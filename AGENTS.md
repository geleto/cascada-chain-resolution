...
The Chain binding, required operation context, and entered-Chain open state are trusted kernel integration facts, even though the kernel exports the corresponding constructors and operations for the higher runtime. Every internal step receives an operation context that already identifies its execution; `runInternalStep` trusts that invariant and has no contextless fallback. A Chain/context execution mismatch or new issuance through a closed entered Chain is fatal before graph or external-authority access because the supplied context still identifies the execution to fail. The higher runtime constructs and passes these facts; absence or arbitrary malformed shape is outside the trusted protocol rather than a supported Error outcome. Application values crossing through it remain ordinary validated inputs. A future general host-facing facade may validate its own invocation before entering the kernel, but must not add a second kernel execution-selection path or turn malformed trusted control into language Error data.
...
- A successful non-Promise language-data result must not expose a callable `then` through native property lookup. Ready assignment returns and publishes `PropertyValidation` for a callable own `then` placement; Promise-backed publication applies the same rule when fulfillment becomes available, and a managed host mutation that creates one fails receiver validation. Managed-class declaration and snapshot adoption reject a callable `then` anywhere on the retained prototype chain, which application code must not later change. Records and Arrays rely on the stable standard prototypes, while exact Functions and external identities rely on their read-only-after-admission contract. Non-callable `then` data remains ordinary. These are source invariants, not another result transport: without them, native Promise settlement would assimilate the same ready object only when an operation happened to complete asynchronously and violate sequential equivalence.
...
- Failure of internal query traversal, refcounting, or indexing is fatal. Supported reflection failure on a user-controlled identity is failure of the query operation and is not an Error already present in the graph: a ready `hasError` or `getErrors` returns `QueryReflectionFailed` poison directly, a pending query rejects with it, `hasError` does not convert it to `true`, and `getErrors` does not collect or return an Array. A supported failure during shared property publication follows that publication boundary instead.
...
- Operation owners are never registered with the execution and carry no fatal-reject action. One sync-first `returnOperationResult` helper in the host-facing facade registers exactly one rejection action for each actually pending execution-bound operation result and removes it when that result settles. Fatal commit rejects and clears only those currently pending operation results. A ready-only execution creates no wrapper Promise or registration. Core operations, internal waits, and detached work do not register. A higher runtime composing kernel operations uses one explicit trusted integration entrypoint for the unwrapped core functions, `returnOperationResult`, poison/compound factories, fatal submission, Error kinds, and precise predicates. It calls `returnOperationResult` only to results it actually exposes to its host; this module-boundary distinction replaces a public/internal mode flag and prevents nested outward wrappers. The integration subpath is a documented trust boundary, not a security capability; do not add tokens or duplicate factories for its callers. Chain construction has an immediate result, checks public entry, and uses the common host and subscription boundaries during initialization; it adds no outward wrapper or constructor-specific final check. Contextless host-configuration APIs stay synchronous and have no execution outcome to observe.
...
- Keep the lifecycle API minimal: direct `owner.open`, idempotent `close()`, lazy local release registration, and one guarded operation-transition entry. It does not classify Errors or provide separate initial/internal/prepared/single/all continuation families. Admission, publication, and complete collection remain semantic helpers at their natural boundaries.
...
- A controlled callback receives only its declared exported inputs. It may mutate or retain exported managed copies, but Functions and observation-only external identities remain exact and read-only. It must not access an unexported managed source or synchronously re-enter Cascada.
...
- A closed execution throws that exact fatal outcome synchronously at execution-bound public entry and otherwise stops local work at centralized common-resumption, host-boundary-exit, subscription-exit, and scheduler-dispatch checks. The host-boundary helper owns the one check immediately after its synchronous host action and before interpreting the returned value or throw; callers add no second post-host helper. One private nullable fatal slot is both the live/failed fact and authoritative outcome; host code observes it through a read-only `fatalError` class getter, and only the execution module's trusted commit path may write it. Deliberate property redefinition or prototype replacement of this trusted control object is unsupported API misuse. Do not add a `hadFatalError` Boolean or public fatal mutator. Every operation result processes synchronously as far as possible. Error and Function precedence applies before ordinary supported-thenable recognition. Core operations consume sync-settling thenables before return, so a remaining semantic thenable is the operation's pending direct result. A ready result returns directly with no result Promise or microtask, while a pending direct result uses `returnOperationResult` and is registered until its outward settlement transition begins. Merely settling the internal source and queuing that transition does not complete the operation result; fatality that commits first still rejects it. The transition removes registration before settling the outward wrapper, after which a later fatal cannot change the delivered result. Registration performs no task cancellation, resource cleanup, dependency walk, or owner notification. An immediate non-blocking API return remains direct even when its internal work continues. Contextless host-configuration APIs remain synchronous and have no execution fatal state. Absence of `fatalError` while work remains is not proof of eventual success.
...
- Contextless declaration and host-configuration APIs create no `FatalError`. Invalid input, conflicts, and supported reflection failure produce ordinary host API Errors and no declaration change; they are not poison because no operation context exists. Re-entrant configuration throws an ordinary host error before registry mutation; if it escapes the user code, the surrounding operation contextualizes it as poison at its host boundary. An unexpected implementation exception escapes synchronously. An existing `FatalError` supplied to configuration is thrown unchanged, but every authentic `FatalError` is originally created by execution-bound `failExecution`.
- A reporter is a synchronous best-effort notification invoked as an unbound function under a protective catch after fatal state commits. The private execution-state record is never its receiver or argument. Its return is ignored without thenability inspection; asynchronous work it starts remains host-owned. Reporter or diagnostic-formatting failure cannot replace the first fatal outcome, revive the execution, or create a second report.
- `PoisonError` and `CompoundPoisonError` directly extend native `Error` and are synchronously detectable, sync-first rejecting thenables; awaiting either rejects with that exact Error whether the operation was ready or pending. `FatalError` directly extends native `Error` and is not language data or a language thenable. There is no common kernel Error base. Kernel construction is trusted through protected factories and private semantic brands; completed wrappers, copied compound child arrays, and the final concrete Error prototypes are frozen. `FatalError.prototype` has its own non-callable `then`, so later modification of `Error.prototype` cannot make native Promise assimilation turn the fatal branch into the language-thenable branch. This is narrow protection of Cascada's own Error protocol, not general support for modified Array, String, Object, or Error primordials. Freeze prototypes once after installing their final methods rather than copying methods per instance or repeatedly checking integrity. Do not copy arbitrary cause properties into them. Recognize Error forms with precise poison, fatal, and native-Error predicates before ordinary thenability recognition; a native Error remains an Error even when it has a callable or throwing `then`, which is never read. A `FatalError` physically received by return, fulfillment, throw, rejection, or graph traversal is submitted to the current execution before admission or success handling.
- One authoritative public `ERROR_KIND` vocabulary names the violated semantic contract. Promise availability never changes a kind: ready failure and rejection at one boundary use one transport-neutral, action-based kind. The opaque source identifies the exact occurrence; it is not a replacement for the stable machine-readable contract kind. Every poison has its exact nonempty registered kind and source context; neither message text nor a generic fallback substitutes for them. `Multiple` is a compound meta-kind.
- Split kinds only when the violated contract, graph/result effect, recovery meaning, or materially useful diagnosis differs. One external property observation therefore uses `ExternalPropertyReadFailed` for a getter or Proxy throw, a ready Error result, and a direct rejection. One selected host invocation and its direct result use `HostCallFailed`. Import, lookup, query, and export reflection keep distinct kinds because failure has distinct operation outcomes; do not collapse them merely because reflection was the physical mechanism. Conversely, do not add a second value kind or a multi-kind host-boundary policy when recovery and graph effect are identical.
...
- Compound poison requires at least one input, flattens nested compounds, and shares semantic deduplication with `getErrors`: raw cause, source-context identity, and kind. Child order and the representative leaf/source are unspecified. Collectors may accumulate arrivals and use identity visited sets without ordered Error summaries, while completing every required branch. Every retained leaf keeps its context and kind. Compound child arrays are frozen after construction; diagnostics derive kinds without a stored `.kinds` projection.
...
- One narrow three-argument host-boundary helper wraps only the exact supported synchronous host action. On either return or throw it first checks the execution's fatal state, then preserves or contextualizes the host outcome only while the execution remains live. Its causal caller applies that boundary's failure effect outside the catch. It uses no callback, policy mode, intermediate thrown marker, paired catcher, or separate post-host helper. A low-level effectful primitive does not catch; its causal caller wraps only that invocation. A direct host-result Promise rejection is handled once in the first boundary continuation already required for import, validation, mirror settlement, or publication. Its closure captures source and kind until it runs; store neither on the source Promise, identity metadata, Chain, property version, or mirror. Ordinary Promise propagation or FIFO mirror publication then preserves that Error unchanged. Preparation, publication, bookkeeping, cleanup, and failures in boundary handling remain fatal.
- Do not route a probe whose documented result includes inability to determine through the host-boundary helper. Admission classification keeps one exact local catch that returns external-category facts, checks execution fatality after the host interaction, and preserves `FatalError`; declaration thenability keeps its separate contextless local recognition and returns ordinary validation when inspection fails. These result shapes and invariants differ, so share no generic probe-result algebra, callback, policy mode, or optional-context helper.
...
- Promise rejection ownership follows creation and transfer, not graph reachability. Observe a host Promise only when selected semantic work consumes it. The kernel owns every Promise it creates or derives until it returns it to an immediate consumer, stores it for a known later consumer while marking it handled, or exposes it as an operation result. `returnOperationResult` consumes its internal source, unregisters fatal delivery on either settlement, and transfers responsibility for the returned Promise to the caller. The higher runtime owns every Promise it creates and every kernel result it schedules, buffers, or discards instead of returning, including discarded-expression observation. Neither layer recursively observes unused host-input graphs. A callback-result contract that admits language Error treats ready poison and poison rejection equivalently; any Error escaping a callback contract that admits no Error outcome is fatal.
- Keep one small machine-checked classification of the actual package export surface so a new outward operation cannot be added unnoticed. Focused route tests own exposure, Error, and Promise semantics. Audit Promise ownership at its producers and transfers, and run tests with strict unhandled-rejection behavior; do not maintain a custom source analyzer or a manifest of semantic claims that syntax cannot verify.
- Support native Promises with standard behavior and the ordered, chainable thenables defined by `docs/data-limitations.md`. The common continuation helper performs an ordinary `then` subscription at each consumer's program position. It preserves a synchronous callback as synchronous work and otherwise returns the pending chain produced by the thenable. The thenable owns first settlement, subscriber storage, chaining, nested assimilation, and FIFO notification across settlement; Cascada neither validates those behaviors nor compensates for violations. A synchronous callback throw escapes the `then` call, while a later one rejects the returned chain. Treat `then` as a trusted scheduling protocol, not a general host callback, so its supplied kernel continuation does not trip the host-code re-entry guard. Keep no execution-wide thenability `WeakMap`, captured-callable record, canonical Promise, first-settlement record, species-defense path, active thenable-cycle set, or kernel subscriber queue. Each causal boundary still interprets its own fulfillment or rejection with its own operation context, while later consumers preserve the contextualized Error.
