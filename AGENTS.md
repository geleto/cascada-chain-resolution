...
- Final assignment creates an absent placement; final deletion of one is a no-op. A newly broken mutation prefix poisons the first failed placement until the selected scope is reached. Include the owning Array if placement-local failure would change its length. Failure within a reached scope belongs to that scope, enlarged only to cover affected intrinsic structure, as specified by [scope transitions](docs/error-handling.md#scope-transitions).
...
A ready mutation needs no gate. Path availability adds neither a lease nor gate when every segment is ready. A managed `!` prefix re-roots the ordinary mutation path there; it does not introduce whole-subtree copying. A managed mutation scope excludes canonical registered mutable external locations; inert aliases elsewhere do not forbid controlled managed work. Mixed entry remains supported and combines managed placement protection with a tree reservation. Native access waits for earlier managed gates; reservation timing must preserve the current entry view and prevent contained work from waiting behind a later outside reservation. If later resolution rejects a dynamic mutable-resource route, publish its local failure at the predetermined managed scope or static prefix; do not poison a guessed external phase. A Promise representing only an independent result is not a gate and extends no receiver or input protection; publish completed state immediately.
...
Two external scopes conflict when they overlap by ancestry/equality and at least one mutates. Observations and disjoint siblings can run concurrently; parent observations exclude descendant mutations, and parent mutations exclude all descendant work. Reserve the selected scope in its current entry view before required input waits or native access; native access also waits for earlier managed gates. Capture prerequisites before publishing the new reservation; a later operation cannot enter an earlier operation's prerequisites.

Propagate outstanding completion membership up the tree, not child counters or scans for unfinished descendants. Distinguish direct and subtree observation/mutation membership and capture fixed waits before adding new work. Each completion removes itself independently; rolling Promise aggregation must not retain completed sibling history behind one unfinished operation. Child access checks strict ancestors' direct conflicts, never their unrelated subtree activity. Completion carries no poison. [External ordering](docs/external-context-ordering.md#external-phases) defines the conflict table and entry ownership.
...
Entry reserves access, not mutation authority or callback-wide rollback. Managed mutating entry installs an ordinary placement gate with ancestor COW as required; managed read entry leases its root. Every read-only entry reserves observation access to covered external resources, including at an external root; mutating entry reserves exclusive access. It uses the common tree algorithm, not a contextual-binding gate or second scheduler. Each contained managed mutation has its own rollback boundary. Explicit unregistered native-property entry fails recoverably; compiler selection chooses the deepest enclosing registered scope without narrowing managed/mixed entry.

Poison at the selected managed or external entry target does not suppress its callback. Transfer complete placement state, including recovery data, so contained inspection, permitted replacement, or repair can run. Strict-ancestor poison still blocks entry. Readonly capability remains unchanged. Successful entry setup adds no poison; failed mutating target selection retains its prescribed scope effect.

Contained work uses the same reservation algorithm in a covered entry view. Outside work waits on the outer reservation; contained work never requeues behind that later outside waiter. Nested entries retain their own work after parent callback closure. External coverage lasts through required contained effects, while managed publication preserves ready-sibling progress. Do not introduce a second operation queue or retain a parallel dependent-gate bypass graph. Entry readonly capability applies independently of its reservation mode.
