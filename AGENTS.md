...
The Chain binding, required operation context, and entered-Chain open state are trusted kernel integration facts, even though the kernel exports the corresponding constructors and operations for the higher runtime. Trust means the kernel does not validate the arbitrary shape or source payload of these compiler/runtime facts; it still checks the minimal routing invariants needed to select one execution safely. A missing context, a Chain/context execution mismatch, or new issuance through a closed entered Chain is therefore fatal before graph or external-authority access. Relying on an incidental `TypeError` would lose the intended operation attribution and could touch state before the execution is known. The higher runtime constructs and passes these facts; application values crossing through it remain ordinary validated inputs. A future general host-facing facade may validate its own invocation before entering the kernel, but must not add a second kernel execution-selection path or turn malformed trusted control into language Error data.
...
- Operation owners are never registered with the execution and carry no fatal-reject action. One common sync-first exposure helper in the host-facing facade registers exactly one rejection action for each actually pending execution-bound public API result and removes it when that result settles. Fatal commit rejects and clears only those currently pending public results. A ready-only execution creates no wrapper Promise or registration. Core operations, internal waits, and detached work do not register. A higher runtime composing kernel operations uses one explicit trusted integration entrypoint for the unwrapped core functions, exposure helper, poison/compound factories, fatal submission, Error kinds, and precise predicates. It applies exposure only to results it actually exposes to its host; this module-boundary distinction replaces a public/internal mode flag and prevents nested outward wrappers. The integration subpath is a documented trust boundary, not a security capability; do not add tokens or duplicate factories for its callers. Chain construction has an immediate result and performs only its public-entry check. Contextless host-configuration APIs stay synchronous and have no execution outcome to observe.
...
Returning a direct Promise object is not successful completion; its outward settlement transition chooses the public outcome. A fatal committed before that transition rejects the result even if its internal source has already settled. Once a ready result returns, or the outward transition settles a pending result, later independent work cannot revise it and reports fatality only through the execution. Every script result uses common export before this public exposure, so reachable nested Promises, Errors, validation, copying, and publication belong to the script-result frontier. A higher runtime must not substitute a raw managed lookup or a fire-and-register issuance outcome when that omitted work is intended to contribute to script completion.
...
- An executionless fatal call throws a contextless `FatalError` synchronously and does not report. If that Error later reaches an execution, that execution reports it normally; no mutable report state belongs on the Error and no global or explicit contextless reporter exists.
