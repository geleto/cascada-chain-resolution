...
A ready mutation needs no gate. Path availability adds neither a lease nor gate when every segment is ready. A managed `!` prefix re-roots the ordinary mutation path there; it does not introduce whole-subtree copying. A managed mutation scope excludes registered mutable external descendants; mixed entry remains supported and combines managed placement protection with a tree reservation. Earlier managed gates precede external reservation. If later resolution rejects a dynamic mutable-resource route, publish its local failure at the predetermined managed scope or static prefix; do not poison a guessed external phase. A Promise representing only an independent result is not a gate and extends no receiver or input protection; publish completed state immediately.
...
A managed mutation scope or managed mutating receiver cannot contain registered mutable external resources. Observation-only external references remain opaque leaves. Reject invalid mixed scopes before invocation and retain their value under an owner-isolated guard. External mutations affect only their selected native subtree; parent scope methods preserve registered child identities. Hidden sibling overlap or access to managed storage violates the host contract. Entry may select mixed branches and grants no exception to these mutation rules.

## External Operation Phases

Two external scopes conflict when they overlap by ancestry/equality and at least one mutates. Observations overlap, sibling scopes overlap, parent observations exclude descendant mutations, and parent mutations exclude all descendant work. Reserve the selected scope after earlier managed gates and before awaiting required inputs or native access. Capture prerequisites before publishing the new reservation; a later operation cannot enter an earlier operation's prerequisites.

Use completion dependencies propagated up the tree, not child counters or scans for unfinished descendants. Distinguish direct and subtree observation/mutation frontiers. Child access checks strict ancestors' direct conflicts, never their unrelated subtree activity. Completion carries no poison and retains no settled history. The detailed dependency table and entry-view ownership are in [external ordering](docs/external-context-ordering.md#external-phases).
...
Entry reserves access, not mutation authority. Managed mutating entry installs an ordinary placement gate with ancestor COW as required; managed read entry leases its root. Mixed entry also reserves the external subtree in the matching access mode. Entry rooted at a registered external node retains exclusive outside protection in either callback mode. It uses tree reservations, not a contextual-binding gate. Explicit unregistered native-property entry fails recoverably; compiler selection chooses the deepest enclosing registered scope without narrowing managed/mixed entry.
...
Managed guards are owning-placement facts, not flags on shared value identities. Installation/clearing uses ordinary version capture and path COW. Preserve retained value/version and publish new guard state without mutating earlier captures. A canonical tree reference may identify the current guard placement but stores no second poison. Ordinary graph queries stop at that guard; live contextual native access still obeys it through older entered managed snapshots. Ordinary Error-valued data remains replaceable/deletable without repair.
...
Repair exclusively clears repairable poison throughout its selected subtree, preserving values and fixed locations. A derived ancestor summary does not block child repair; own poison above the target does. Repair-only invokes no native code. Repair-and-call clears first, then prepares/invokes under the same reservation; new failure poisons again and does not resurrect cleared Errors. Repair creates no authority and is not rollback. It clears scope guards, not arbitrary Error-valued managed properties. [Scope transitions](docs/error-handling.md#scope-transitions) specify ownership and query details.
