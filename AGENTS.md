...
- Final assignment creates an absent placement; final deletion of one is a no-op. A newly broken mutation prefix poisons the first failed placement until the selected scope is reached. Failure within a reached scope belongs to that scope, as specified by [scope transitions](docs/error-handling.md#scope-transitions).
...
A property version is the exact logical state captured at one placement. Installed versions overlay physical storage where the logical value must be retained separately.
...
- A synchronously consumed custom thenable creates no Promise version. Runtime-owned storage receives its final value directly; unchanged imported storage uses a fixed placement version containing that value. Imported native Errors likewise use fixed placement versions containing their contextualized Errors.
- One installed version determines the placement's logical value. Runtime-owned physical storage may be updated, but correctness never depends on writeback; imported physical storage stays unchanged.
- Replacement or deletion detaches the installed version. Captured versions continue serving operations that retained them.
...
A ready mutation needs no gate. Path availability adds neither a lease nor gate when every segment is ready. A managed `!` prefix re-roots the ordinary mutation path there; it does not introduce whole-subtree copying. Earlier managed gates order access before descendant external phase reservation; this operation's own gate then protects its managed publication while the reached phase orders its exact external effect. If later resolution rejects a dynamic mutable-resource route, publish its local failure at the predetermined managed scope or static prefix; do not poison a guessed external phase. A Promise representing only an independent result is not a gate and extends no receiver or input protection; publish completed state immediately.
...
External identities are observation-only by default. An identity may be mutated only when initial synchronous context import reached it along the compiler's mutation access tree and recorded its fixed location. Compiler nodes are property maps with `{}` endpoints; calls contribute receiver prefixes and property mutations contribute containing prefixes, independently of poison scopes. Omission means no requests; `{}` requests only the context root. The compiler owns its unchanged input, and the runtime builds its context-local filtered tree. Every recorded mutable boundary is directly accessible in the original context without crossing any Promise or thenable. The complete tree is ready before ContextChain construction returns; later data delivery creates no authority.

- Initial discovery follows only the compiler tree's static keys. A first external identity ends that branch, even if the compiler node has children. Remove non-external endpoints and newly empty branches. Method-receiver facts and poison scopes are operation-local, not tree markers.
- Build the tree atomically with the initial import segment from directly accessible source values and staged admission facts. Stop at every Promise or thenable, including a synchronous custom one; Errors and Functions also stop discovery, and an external identity terminates its discovered boundary. Delivery creates no tree leaf.
- Preserve explicitly requested distinct occurrences through aliases or context cycles. Two selected locations for one exact external identity produce import-attributed `ExternalLocationConflict`; abandon the segment without changing existing bindings. Unselected aliases make no claim, and later off-path use fails locally. The finite compiler tree bounds discovery.
...
- Each runtime leaf is the location record for one first external boundary, unique to its root ContextChain and canonical path. Entered contextual Chains select that same record and retain the originating route needed for live guard checks. Store no receiver cache; verify a reached boundary value through its execution-local identity entry. Dynamic input keys may continue within a statically selected native boundary, but cannot select a mutable boundary.
...
- Registered mutable external boundaries and their context-path ancestors form a fixed namespace. Reject whole replacement/deletion, structural remapping, or arbitrary managed mutating methods on that namespace before effects, using the affected static-tree path. Ordinary managed children beside it remain mutable. Physical COW and entry/poison overlays preserve its bindings without candidate-graph validation or a namespace-specific receiver copy. Observation-only external identities impose no fixed-namespace restriction.
...
Every mutable external operation validates its static route and current binding before reservation and before deferred native access. It owns one actual boundary; aggregate queries compose ordinary scope observations. An off-path or dynamic route fails locally without modifying another context's scope or phase. A forbidden mutation publishes its local Error at an already-selected managed scope or the longest static managed prefix when its target lies beyond a dynamic key; detection timing does not choose a resource to poison. No candidate reservations or access-time conflict arbitration are needed.

Derive live authority from the shared entry. Invalid leaves remain inert discovery facts; add no tree-pruning mutation, reverse occurrence index, placement poisoning sweep, or exposure history. Native effects stay within the reached owning boundary. Explicit access to an identity with invalid authority returns its binding Error rather than reinterpreting it as unregistered observation-only state.
...
One host resource that Cascada may mutate must have one external owner and one Cascada access location. Hidden access to another mutable owner's state, including reads through an observation-only wrapper, is unordered and violates the host contract. Use one combined owner or explicit detached inputs. Independent host mutation remains unsupported. Protected managed source references may remain read-only; they do not follow later COW updates.
...
- Reach the boundary through earlier managed and entry-binding gates before reserving its external phase. The unique valid context path makes the earlier gate the ordering boundary; an outside phase must not block work inside the entry whose gate it awaits. Publish the actual boundary successor before subscribing to its predecessor.
- Capture path protection at issuance. Resolve dynamic managed prefixes through their ordinary versions, leases, and mutation gates; reaching a mutable external boundary through such a prefix produces the prescribed local validation failure and acquires no external phase. See [external phases](docs/external-context-ordering.md#external-phases).
- Select only actual required mutable external boundaries. Mutable resource selection requires a static source path; computed keys before that boundary are invalid even when ready. Dynamic keys inside an already selected boundary remain supported. Observation-only identities need no fixed path or external mutation phase.
- Compiler/runtime path facts preserve one first-dynamic-segment position, separately from current segment values and Promise readiness. Carry that provenance through entry and path composition; a computed route cannot regain mutable authority by producing ready strings. No candidate reservation, speculative exclusivity, or candidate-readiness aggregate is required.
- Capture explicit inputs and available managed property versions at issuance. Resume a gated managed prefix at its FIFO turn, then reserve the reached boundary and start its selected predecessor wait. Required preparation progresses concurrently. Later work uses captured logical versions and exact boundary identities rather than rereading a newer managed prefix.
...
- Existing waiters keep their captured predecessor when a successor becomes current. A read group stays joinable until the next exclusive reservation seals it and completes only after sealing and its last observation. It tracks completion only, never observation Errors.
- Each exclusive completion Promise fulfills directly with `null` or the exact non-thenable repairable poison; a read-group Promise is a drain signal with no payload. [Guard Poison and Repair](#guard-poison-and-repair) defines how observations and exclusive successors consume it.
...
- A direct operation Promise keeps its phase active through its final boundary processing. A nested result Promise does not. A property assignment reserves the whole external owner before waiting for its input, exports that input completely, and finishes the native write before releasing the phase. Later access to any property of that owner waits; native storage contains no managed gate or Promise placeholder.

Select external phases from a statically selected mutable context boundary. Managed entry gates its selected placement as usual; whole mutable-external entry gates its context binding exclusively, including read-only callbacks, while its contained operations use normal external phases. The entry holds no external phase across its callback and republishes the same binding after issuance closes. Earlier entered routes obey the current binding gate; only its own private work bypasses it. Compiler lowering enters the whole mutable boundary through a static path. A native-property entry request fails recoverably with PropertyValidation at target selection: read-only failure is local and mutating failure poisons the external scope in its phase order, preserving its exact binding. Existing poison takes precedence; invoke no callback and do not widen the requested entry silently. Observation-only external identities remain unlocked.
...
- A marker inside opaque external state clamps to the first external boundary. A managed prefix keeps ordinary isolation; an external host effect selects its reached owning boundary. Managed methods receive no authority over opaque descendants, and external methods receive no raw mutation access to managed ancestors or siblings.
- A selected external phase grants no authority over another external owner. Invalid leaves remain inert without tree deletion; other valid boundaries stay usable. Host mutation of an invalid or unselected identity violates the host contract.

## Guard Poison and Repair

[Scope transitions](docs/error-handling.md#scope-transitions) define the common mutation effect independently of receiver, temporary protection, and result transport. Contextual Error queries also observe ordered mutable-external scope metadata: healthy state gives false/null, poison gives true/the original Error, and ancestor queries include accessible descendant scopes. They read no native properties. A poisoned guard is terminal; retained recovery children are not queried or awaited. Managed index pruning cannot hide required external-scope observations, and phase poison is not copied into managed counters.

An exclusive phase retains one completion Promise, consumed through the common continuation helper in subscription order. Fulfillment carries `null` or the exact poison directly; no transport wrapper or separately exposed ready-state cache is needed. A native completion Promise schedules its continuation even after settlement. An existing supported sync-first completion primitive may deliver directly only under its own FIFO contract across settlement; add no phase-specific ready shortcut or queued-work tracker. Fulfillment-only completion needs no rejection-only observer, while potentially rejecting derived reactions retain ordinary ownership.

Each failed mutation has one selected poison owner. A scope at or inside an external boundary uses that identity's phase poison. A managed scope retaining fixed external bindings records logical poison in its owning context placement's metadata and retains the underlying value for repair; it does not additionally poison its external children. Native ownership and external phase ordering remain unchanged. A child's completion preserves predecessor poison when a managed ancestor owns the new failure. Ordinary managed scopes without fixed bindings keep ordinary Error-value publication.

All lookup, Error-query, export, and mutation routes observe a managed guard's Error without traversing retained recovery data. Captured managed values and Errors retain ordinary immutable-version semantics. Live external access through an earlier entered Chain must nevertheless obey the originating context's current ancestor guards; old managed snapshots grant no permission to bypass live ordering or poison. Keep one authoritative scope record at its context path, shared by the necessary runtime representations, without child poison copies or an alias registry. Publish an ancestor's poison before completing its selected child's phase.

- Existing scope poison makes the attempted consuming action unnecessary: return its exact Error, including a compound, without descendant traversal, native access, or waiting for invocation-only arguments. Capture inputs while scope readiness is unknown, then close action-only work on a blocker; already-owned rejections and shared settlement retain their normal lifetimes. Complete collection applies to executable selected work and accessible queried branches.
- Every observation failure affects only its result. Complete its required Error collection and release its reservation with predecessor state unchanged; it neither adds nor clears phase poison.
- Mutation failure publishes its combined Error at its selected managed guard or external phase only. Blocked descendant operations propagate the original guard Error without adding descendant poison. Completed host effects remain visible.
- Repair-only bypasses and clears the selected managed guard or external phase, performs no host access, and returns `undefined`. A managed guard exposes its retained value again. Repair-and-call bypasses that selected scope's old poison, then clears it on success or publishes the new failure there. No repair sweeps or clears independently poisoned descendants; a deeper repair cannot bypass an unrepaired ancestor.
- Only repair-only and repair-and-call clear retained scope poison. Assignment and deletion naturally remove an ordinary Error value at their final managed placement, but do not implicitly clear a retained managed guard or external-phase poison.
...
- Required preparation poison, receiver validation failure, or method failure publishes through the selected mutation scope; invocation code reports its receiver effect and independent result to that common transition. Observation and independent-result failure affect only their result. A direct-Promise rejection preserves the rejection outcome after applying the corresponding graph effect.
- Import every managed-method result. Returning the mutation receiver selects its published logical value and marks it shared when retained as output. An observation result uses ordinary import. Every other mutation result uses managed mutation-result import, which keeps admitted identities and marks the reached managed graph shared so receiver/result aliases gain ordinary shared ownership. Mutation scopes do not change these rules. Any result that would expose a mutation-capable external identity fails.
