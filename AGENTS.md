...
| Instance of a registered class | Side-effect-free observations and synchronous mutations through methods or class-defined accessors | Host call on a complete prototype-preserving receiver snapshot | Ordinary language writes; host mutation replaces the receiver placement |
...
- Registered instances and their state use ordinary identity ownership. Complete-state isolation is required only when JavaScript receives a registered receiver.
...
- Ordinary COW preserves the admitted prototype when it shallow-copies a registered instance. A registered host mutation instead receives a complete private receiver snapshot because JavaScript may mutate any nested state.
- A registered receiver snapshot preserves aliases and cycles within the receiver. Publishing a mutation may sever an alias between receiver state and another graph placement: that placement retains its old identity and logical value, while the receiver publishes its private copy and mutations. The cross-owner identity relationship is not preserved.
...
- A **lease** is temporary COW protection for an exact logical value still in use when later mutation may reuse its representation. It never delays later operations; a later mutation copies, and the lease ends when its operation completes or fails. Read-only entry and pending host use of an exact COW-managed value require leases. Registered snapshot preparation does not retain its source: each transition copies its complete available frontier before returning and captures every pending branch through a mirror, so later continuations use captured versions. A published registered-mutation receiver is leased while a returned Promise may retain it.
...
- Host arguments are exported. Host code receives no internal representation or unresolved language Promise introduced by Cascada; traversable values become snapshots, while opaque identities and Functions remain exact. Host-call snapshots preserve admitted registered prototypes; public export instead normalizes registered instances to plain data. A snapshot captures logical values at the operation's program position, and later mutation cannot change it.
- Admit results by origin: import new host identities and snapshots not published as runtime state; keep controlled results and identities in a published registered-mutation receiver runtime-owned. An exact unsnapshotted identity retains its existing origin. When result admission reaches an identity already retained under these rules, keep it without reimporting its graph; normal sharing applies if the result adds an owner. Import records an opaque identity without traversing its state.
...
- All semantic state must be rooted in own enumerable string-keyed data properties. Host-ready state contains primitives, records, logical Arrays, and nested registered instances. Functions, opaque identities, Errors, private fields, internal slots, Symbols, non-enumerables, closure state, and own accessor descriptors are not host-ready. The runtime validates inspectable own-property shape and reachable values; private fields, internal slots, and closure state remain trusted registration restrictions that cannot be diagnosed exhaustively.
- A Promise may temporarily occupy runtime-owned or imported state, but registered JavaScript receives only its resolved logical value. Imported physical storage retains its Promise unchanged.
...
- Every registered method or accessor receives one complete prototype-preserving host snapshot. Receiver and argument preparation copy traversable captured values into one private graph, preserving aliases and cycles and materializing internal representations. Host code receives no original traversable graph identity, unresolved Promise, or Error. Exact opaque and Function arguments remain permitted, but the same identity is invalid when also reached through registered state.
- Once preparation is ready, inspection preserves consumed Errors for propagation and produces a validation Error for unsupported registered state; either skips invocation. Snapshot preparation follows the frontier-capture exception under Pending Work, so source leasing is unnecessary.
- An observation uses the same complete snapshot because arbitrary class code may read any receiver state. It runs once and discards the snapshot; preparation poison or a synchronous throw affects only its result.
- A mutation runs once and synchronously on the private snapshot, validates its inspectable own-property shape and complete reachable state, and publishes the root once at the captured receiver placement. Pending preparation uses the ordinary transition gate there. Preparation poison or a synchronous throw poisons that placement; a detected invalid state left by registered host code is fatal and is not published.
- A class-defined setter receives one resolved exported value and its return value is ignored. Like every registered mutation, it must finish changing state synchronously.
- A returned Promise is only the independent API result. Publish successful state before waiting, preserve the Promise unchanged, and lease the published receiver until settlement because host code may retain it. Mutation after the synchronous call returns is a host contract violation.
