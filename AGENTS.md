...
Operation Chains retain graph values and ordinary non-thenable Errors. Public lookupPrimitiveValue reuses path observation and admits only null, undefined, String, Number, Boolean, BigInt, and Symbol primitives. Another selected value produces ExpectedPrimitive without coercion, deep export, descendant traversal, or mutation of the source. Existing Errors keep their identity and source. Operators own accepted primitive combinations and numeric validity.

At that outward boundary, a ready failure returns an immutable non-Error PoisonedValue with one .error reference; a pending failure rejects with that ordinary Error. Its then invokes a supplied rejection callback synchronously and returns the callback result, or returns itself when no callable handler exists. It catches no callback throw and owns no scheduling. The public createPoisonedValue factory also serves failures created by Cascada expressions. The container carries no duplicate attribution and is never stored or admitted in the graph: input consumption converts it to its contained Error through ordinary rejection delivery.

Normalized kernel results are T | PoisonError | Promise<T | PoisonError>. Expression extraction is Primitive | PoisonedValue | Promise<Primitive>. A ready container is a completed outward failure, not pending graph work or a fatal-result obligation. All required work and primitive validation precede final expression settlement. Preserve pending fatal delivery until that final boundary without duplicate wrappers. Graph inspection succeeds with ordinary Error data; explicit propagation and final native script delivery choose rejecting transport separately. Cascada uses only the documented public package API, including its shared Error factories and guarded higher-runtime composition facilities.
