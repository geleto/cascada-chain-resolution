...
Identity declarations and managed-class registration are runtime-wide host configuration applied independently by each execution. Each execution owns its first fatal outcome and coordinates shutdown of its operation work. The fatal reporter and host-code re-entry guard are runtime-wide contracts. Immutable definitions and operation-local work do not belong to an execution.
...
is poison instead. A `RuntimeError` is reported once and rethrown, never
