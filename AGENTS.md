...
- Every phase Promise fulfills with a hook-free, non-thenable state record; repairable poison is a field of that record and never a direct thenable fulfillment. An observation captures already-published poison when it joins and poison from its exclusive predecessor before it runs. The read group keeps issuance-ordered outcomes and its poison known so far. Poison produced by one observation does not retroactively affect peers already issued in the same read phase; later observations and the next exclusive successor see it.
...
- Use one operation-level lifetime mechanism around each operation's open/closed fact and idempotent `close()`. Components of one issued operation share that lifetime: component success does not close it, and the coordinator closes after required final processing and publication. Fatal handling is separate: a detecting transition submits and propagates the execution's authoritative `RuntimeError` to unwind its current call or Promise reaction; a later continuation that merely observes an already-failed execution simply returns. A nested component with independently stored operation-only resources registers one synchronous, idempotent, non-throwing release with the owner and unregisters it on normal completion. Release registration never tracks or cancels work.
- Operation owners are never registered with the execution and carry no fatal-reject action. One common sync-first exposure helper in the host-facing facade registers exactly one rejection action for each actually pending execution-bound public API result and removes it when that result settles. Fatal commit rejects and clears only those currently pending public results. A ready-only execution creates no wrapper Promise or registration. Core operations, internal waits, and detached work do not register. A higher runtime composing kernel operations uses one explicit trusted integration entrypoint for the unwrapped core functions and applies exposure only to results it actually exposes to its host; this module-boundary distinction replaces a public/internal mode flag and prevents nested outward wrappers. Chain construction has an immediate result and performs only its public-entry check. Contextless host-configuration APIs stay synchronous and have no execution outcome to observe.
...
Export never sends an Error to host JavaScript. Each root is one Error-collection domain. After reaching an Error at any depth, export still completes that root's captured frontier and collects every distinct Error occurrence beneath it. One Error becomes the root outcome unchanged; several become one compound Error. A batch export combines failed roots in semantic root order, flattening nested compounds and deduplicating exact leaf identity only. Different occurrence wrappers remain distinct even when they share one cause. Any failed root prevents host invocation or assignment and replaces a script result.
...
Import, storage, assignment, return, copying, and rejected host-input export are not uses. A direct lookup that would expose a mutation-capable external identity fails and records no use. Selecting a supported call or property operation through an external boundary, or selecting a boundary in a broader external mutation scope, is actual use even if member reflection or later preparation fails before host access. The first use must occur at one live location and selects it. Any first use elsewhere, or later use through another path or Chain or through an alias at another location, creates permanent conflict, performs no access through the selected external receiver, and returns an Error explaining the first incompatible use. Selecting two locations for one identity in the same operation also conflicts. Repair cannot clear conflict.

Evaluate the complete actual-use batch from one state and commit it atomically at the first ordered point after the exact boundaries and selected phase predecessors are ready. Ready operations do this synchronously; unresolved paths do it in their already-ordered phase continuation. Commit use before host access and independently of later preparation success. If any conflict exists, commit every discovered permanent conflict but no compatible new location. Otherwise commit every new location together. Report conflicts in deterministic receiver and path order; iteration order never grants partial authority.
...
is poison instead. A `RuntimeError` is reported once by each execution it closes and exposed by every still-pending execution-bound public API operation result; internal and detached paths stop at their next execution check. It is never admitted, queried, combined, or propagated as language data.

- A runtime-only boundary creates a candidate `RuntimeError` from every unexpected escape, including `PoisonError`, and submits it to the execution. A language-outcome transition recognizes expected poison before that lane and applies its defined graph effect. The execution returns its authoritative first fatal outcome, which the detecting call or Promise reaction propagates; a later candidate is discarded. A later continuation that merely observes the already-failed execution instead returns at its entry check. Do not turn a detecting reaction into a successful `undefined` fulfillment: that would make correctness depend on every downstream consumer noticing fatal state before interpreting a successful payload. The first fatal stores the outcome, closes the execution to new operation work and supported host effects, and rejects only public results that are currently pending. It walks no operation owners or internal waits.
- A closed execution throws that exact fatal outcome synchronously at execution-bound public entry and otherwise stops local work at centralized common-resumption, host-boundary-exit, and scheduler-dispatch checks. The host-boundary helper owns the one check immediately after its synchronous host action and before interpreting the returned value or throw; callers add no second post-host helper. One private nullable fatal slot is both the live/failed fact and authoritative outcome; host code observes it through a read-only `fatalError` getter, and only the execution module's trusted commit path may write it. Do not add a `hadFatalError` Boolean or public fatal mutator. Every public result processes synchronously as far as possible. Error recognition precedes thenability; a ready result returns directly with no result Promise or microtask, while a direct Promise result uses the common exposure helper and is registered only until it settles. That registration performs no task cancellation, resource cleanup, dependency walk, or owner notification. An immediate non-blocking API return remains direct even when its internal work continues. If an operation result completes first, its registration is removed; a later fatal is stored and reported without changing the delivered result. Contextless host-configuration APIs remain synchronous and have no execution fatal state. Absence of `fatalError` while work remains is not proof of eventual success.
...
- An executionless fatal call throws a contextless `RuntimeError` synchronously and does not report. If that Error later reaches an execution, that execution reports it normally; no mutable report state belongs on the Error and no global or explicit contextless reporter exists.
- A reporter is a synchronous best-effort notification invoked under a protective catch after fatal state commits. Its return is ignored without thenability inspection; asynchronous work it starts remains host-owned. Reporter or diagnostic-formatting failure cannot replace the first fatal outcome, revive the execution, or create a second report.
- `PoisonError` and `CompoundPoisonError` directly extend native `Error` and are synchronously detectable, sync-first rejecting thenables; awaiting either rejects with that exact Error whether the operation was ready or pending. `RuntimeError` directly extends native `Error` and is not language data or a language thenable. There is no common runtime Error base. Runtime construction is trusted through protected factories and private semantic brands; completed wrappers, copied compound child arrays, and the final concrete runtime Error prototypes are frozen. `RuntimeError.prototype` has its own non-callable `then`, so later modification of `Error.prototype` cannot make native Promise assimilation turn the fatal branch into the language-thenable branch. This is narrow protection of Cascada's own Error protocol, not general support for modified Array, String, Object, or Error primordials. Freeze prototypes once after installing their final methods rather than copying methods per instance or repeatedly checking integrity. Do not copy arbitrary cause properties into them. Recognize Error forms with precise poison, fatal, and native-Error predicates before sampling thenability; a native Error remains an Error even when it has a callable or throwing `then`, which is never read. A `RuntimeError` physically received by return, fulfillment, throw, rejection, or graph traversal is submitted to the current execution before admission or success handling.
...
- One native host Error consumed at one causal boundary becomes a `PoisonError` with the native Error as `cause`. One identity walk for that boundary stages the wrapper in its identity map, so aliases to the raw Error share the wrapper and imported graph topology remains canonical. Reusing the raw Error at another causal boundary creates another wrapper. Imported host storage remains physically unchanged; each aliased logical property version carries the shared occurrence wrapper.
- Preserve an existing `PoisonError` or `CompoundPoisonError` unchanged across later language-outcome boundaries. Poison is not evidence that an escape from runtime-only work is recoverable; such an escape is a fatal trusted-contract violation. Submit an existing `RuntimeError` unchanged to the receiving execution; that execution propagates it unless an earlier fatal Error is already authoritative. Promise delay, copying, and whichever consumer advances shared settlement never reattribute either branch.
...
- Compound poison requires at least one input, flattens nested compounds, preserves semantic logical collection order, and deduplicates exact leaf identity only. Different occurrence wrappers remain distinct even when their cause identities match. Every surviving leaf keeps its own context and kind. Diagnostics derive distinct kinds from the leaves; the compound stores no duplicate `.kinds` projection.
...
- Promise rejection ownership follows creation and transfer, not graph reachability. Observe a host Promise only when selected semantic work consumes it. The kernel owns every Promise it creates or derives until it returns it to an immediate consumer, stores it for a known later consumer while marking it handled, or exposes it as a public result. Exposure consumes its internal source, unregisters fatal delivery on either settlement, and transfers responsibility for the returned Promise to the caller. The higher runtime owns every Promise it creates and every kernel result it schedules, buffers, or discards instead of returning, including discarded-expression observation. Neither layer recursively observes unused host-input graphs. A callback-result contract that admits language Error treats ready poison and poison rejection equivalently; a runtime-only callback treats either escape as fatal.
- Keep one lightweight machine-checked source inventory with rule families for trusted Error construction and kinds, package-export result exposure, and Promise production/settlement ownership. Derive mechanically visible facts such as exports and factory constants directly; use a small manifest only for semantic ownership that syntax cannot decide, and use route tests for behavior. A newly introduced syntactic site must fail verification until classified. Do not build separate analyzers or claim static proof of dynamic graph effects.
- Use one source-neutral cached first-settlement Promise for every captured thenable, including branded native Promises. Invoke the captured `then` once with kernel callbacks and never use the derived Promise it returns as kernel FIFO or settlement state. For a native Promise, intrinsic `then` can still invoke host-controlled `constructor`/`Symbol.species`; that invocation is the supported host boundary, while ignoring its derived result prevents the species result from controlling kernel continuation. Native assimilation has already resolved nested thenables, so native fulfillment needs Error recognition but no thenability resampling. For another thenable, callbacks return immediately when `fatalError` is present and otherwise fulfill the cached Promise with a non-thenable first-settlement record. Each causal boundary interprets the raw record in a continuation whose closure captured that operation context: raw rejection is contextualized separately at each boundary, while non-native fulfillment performs Error recognition and execution-local nested thenability capture. An active assimilation-path identity set rejects self and mutual cycles without rejecting later noncyclic reuse. Later consumers preserve that boundary's Error. The cached record never carries consumer attribution.
