...
This runtime is a correctness boundary for documented use, not a security boundary against a caller deliberately corrupting its programming interfaces. Protect all supported values, timing modes, external failures, and ordinary invalid application inputs. Do not add runtime machinery merely to make intentional contract violations fail neatly.

Treat these as unsupported misuse unless a concrete product or security requirement says otherwise:

- forging, directly constructing, subclassing, or changing the prototype of runtime Error and control types;
- mutating exported constructors, shared prototypes, trusted runtime control objects, or standard primordials that the data contract requires to remain stable;
- calling trusted integration methods directly, fabricating internal control records, omitting required compiler/runtime arguments, or corrupting private-looking runtime state; and
- supplying a custom thenable, Proxy, external identity, or managed class that deliberately violates its documented ordering, stability, mutation, or storage contract.

Unsupported misuse may produce an ordinary JavaScript exception, fail classification, or make that caller's execution unusable. It does not need a token, brand registry, defensive clone, frozen shared prototype, repeated integrity check, compatibility adapter, special diagnostic, or dedicated test. Do not turn a programming API into a tamper-resistant capability system by accident.

Defensive handling is justified when the trigger can occur during documented use: invalid application data, an external call that throws or rejects, ordinary fallible Proxy/reflection behavior on a supported boundary, Promise timing, resource lifetime, cross-execution isolation, or a runtime defect escaping an already-valid operation. Such handling must protect a named semantic invariant and remain proportional to the realistic risk.

Before adding a defensive mechanism, answer all of these:

1. What concrete supported scenario triggers it without deliberate contract violation?
2. Which core invariant or required diagnostic would otherwise be lost?
3. Can the existing language, boundary, ownership, or fatal path handle it naturally?
4. Would a clear unsupported-use rule be simpler and sufficient?
5. Is the added production state, branching, retention, documentation, and test burden proportional to its benefit?

If the only motivating example is malicious, deliberately malformed, or extraordinarily contrived use of a trusted programming interface, default to documenting it as unsupported and delete the defense. Tests should prove supported semantics and load-bearing invariants, not promise graceful behavior for excluded misuse. Revisit this boundary explicitly if Cascada ever becomes a sandbox or accepts mutually untrusted extensions.
...
- Keep `operation-lifecycle.js` minimal: direct `owner.open`, idempotent `close()`, and lazy local release registration. `internal-step.js` owns the one guarded operation-work family: immediate entry, continuation after one result, initial value consumption, complete input collection, and clean-input preparation. These share execution and owner checks without becoming configurable lifecycle or Error-policy variants. Publication remains at its natural semantic boundary.
...
- `PoisonError` directly extends native `Error`, and `CompoundPoisonError` extends `PoisonError`. Both are synchronously detectable, sync-first rejecting thenables; awaiting either rejects with that exact Error whether the operation was ready or pending. `FatalError` directly extends native `Error` and is not language data or a language thenable. There is no common kernel Error base. Semantic recognition first uses native `Error.isError` so an arbitrary Proxy is never subjected to prototype traversal, then uses ordinary `instanceof` inheritance. Standard Error constructors and prototype chains must remain unmodified. This programming API adds no runtime machinery to defend against deliberate construction, subclassing, prototype replacement, or prototype mutation. Factory-produced wrappers and copied compound child arrays are frozen; their shared prototypes are ordinary runtime prototypes. Do not copy arbitrary cause properties into them. Recognize Error forms with precise poison, fatal, and native-Error predicates before ordinary thenability recognition; a native Error remains an Error even when it has a callable or throwing own `then`, which is never read by Cascada. A `FatalError` physically received by return, fulfillment, throw, rejection, or graph traversal is submitted to the current execution before admission or success handling.
...
- Error combination requires at least one input. The trusted `combineErrors` factory expands direct compound inputs by one level and deduplicates by raw cause, source-context identity, and kind before allocating an Error: one distinct leaf returns unchanged; several produce a compound with a frozen, leaf-only `.errors` array. Its constructor trusts the factory's finalized leaves. Recursion is unnecessary because every input compound already has the same leaf-only invariant. `getErrors` uses that same one-level `flattenAndDeduplicateErrors` rule. Child order and the representative leaf/source are unspecified. Collectors may accumulate arrivals and use identity visited sets without ordered Error summaries, while completing every required branch. Every retained leaf keeps its context and kind; diagnostics derive kinds without a stored `.kinds` projection.
