...
The Chain binding, required operation context, and entered-Chain open state are trusted kernel integration facts. The common `runInternalStep` entry checks that the operation context and its execution binding are present before work. It covers Chain construction and initial higher-runtime semantic entry, where omitting the required positional argument or passing a source-only context is a credible integration error. A null or undefined operation context or execution binding throws a privately constructed, branded fatal diagnostic without creating an execution or supplying a fallback. An enclosing valid operation context submits that defect through its own execution. Helpers and continuations trust their supplied operation context and do not repeat entry checks. Initial entry throws an existing fatal, while a continuation observing failed execution or closed local ownership skips its work; both use the same internal fatal-on-escape guard after their own checks. Chain/execution mismatch and new issuance through a closed entered Chain remain fatal at their existing boundaries before graph or external-authority access. Missing source-location fields are harmless; application values still receive their ordinary validation.
...
- One private host-escape protocol separates exact host actions from their causal failure handling. The three-argument `runHostBoundary(operationContext, kind, action)` composes that protocol for direct language-result calls and also contextualizes returned native Errors. `runHostAction` guards only the exact supported host action and checks the execution's fatal state after either return or throw. An ordinary throw carries a private identity brand through shared graph helpers; the owning `catchExternalThrow` consumes only that marker and supplies its operation context and fixed kind. Contextualization and the causal caller's failure effect run outside recovery. Causal recovery encloses each initial or resumed semantic step inside its fatal guard; an outer issuance catch cannot recover an escape from an inner guard. Unmarked internal failures escape to the fatal envelope. Markers are neither language data nor an integration API. No second recovery mode, mutable policy on operation contexts, or separate post-host check exists. A direct host-result Promise rejection is contextualized in its first existing boundary continuation. Its closure carries source and kind until completion; neither belongs on the source Promise, metadata, Chain, or property version.
...
- Verify that export collects synchronous and Promise-revealed Errors at every depth, loses no distinct Error identity, combines complete semantic membership once across all required roots, and never invokes host code with an Error.
