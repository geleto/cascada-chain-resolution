...
Read-only entry setup reuses observation-path capture. If the walker supplies an import boundary, ordinary import preparation makes the captured value a direct attributed root before entry creates its Chain. Entry then increments the tracked value's shared metadata lease; primitives need no metadata. Completion decrements the same lease and deletes its counter at zero. `enter` and `run` share `metadata.updateReadLease`, while each operation owns and releases only its own lease. The mutation walk's single `requiresCopyOnWrite` predicate combines permanent sharing or non-extensibility with a positive `readEnterCount`. Completing the last read entry removes only the temporary COW condition and never clears permanent protection.
