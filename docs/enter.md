...
Create the private Chain and capture the original target's complete placement state before detaching it: presence, logical value/source version, and any recovery baseline. Install the public gate before subscribing to pending-source transfer. An absent target initializes an absent private root placement, not a present undefined value; ordinary reads still return undefined and ordinary assignment establishes presence. A pending source forks its exact captured Promise version into that root. If ancestor COW leaves the source reachable elsewhere, preserve sharing at the transfer boundary. Callback invocation happens after required owning-path reconstruction, not after all descendants become ready.

A gate orders current-owner access. Captured earlier versions keep their ordinary values and Promise frontier. Unrelated object sibling paths do not wait. Entry into an absent property does not itself create that property: publication transfers the private root's final presence as well as its value. A no-op callback preserves absence, while an explicit assignment of undefined creates a present property. Use ordinary placement-state transfer, not an entry-specific dirty flag or a remembered initial value used to guess whether work happened. Successful entry setup does not add language poison: a contained mutation publishes its selected scope effect, while a callback's independent Error result need not poison the entered value.

An out-of-range Array-index mutating entry can change length while its callback decides whether to create the element. Select and protect the owning Array before callback invocation; never grow length merely to install a gate element. Reuse the ordinary structural transition, with the callback's reference still targeting the requested element. Publish the current working Array and the element's final placement state on closure; a no-op preserves length and holes, while a write commits its ordinary structural effect. Existing in-range elements and holes retain placement-local gates: a gate at a hole is pending placement state, not proof that an element exists. Consumers that depend on presence must observe its completed state before enumerating/copying that element or invoking an Array callback for it. The wider structural protection grants no broader reference or native authority, and it creates no callback-wide rollback: successful contained commands remain committed. Later length/whole-Array consumers and structural mutations wait for an out-of-range entry; captured older Arrays remain unchanged.
...
[External ordering](external-context-ordering.md#external-phases) owns the single direct/subtree frontier algorithm. Capture an entry's issuing view and effect completion at issuance. After preceding managed gates and required target selection, reserve its covered subtree in that view before invoking its callback or awaiting external predecessors. An entry creates a covered view in which contained commands use that same algorithm. Outside commands remain in the parent view, where they encounter the entry's reservation. A nested entry creates a nested view under a reservation in its parent's view. Share immutable tree locations and identity binding facts; view-local frontiers are temporary entry ordering state and are released with that view.
...
A mutating entry reserves its native subtree exclusively; a read-only entry reserves observation access. Record one effect completion for each issued command that may reach covered native state, including commands delayed by managed gates, and reuse it when that command reserves. This is entry lifetime membership, not a resource reservation or a second operation queue. Selection failure must publish its required effect before completion. Remove completed membership promptly. At callback closure stop issuance and capture outstanding completions, including already-issued nested entries, before releasing outer coverage. Do not retain independent nested method-result Promises.
...
A reference call is one batch of requested entries. Issue every disjoint selection during the same synchronous turn, without awaiting another argument's selection or external predecessors. Each selection takes its ordinary place in managed FIFO traversal and installs its path protection or external reservation as soon as its own preceding managed gates permit. A ready external argument must reserve immediately even if another disjoint managed argument is pending. Recording the latter's effect completion does not order access to the former. Invoke the function only after every required reference is available; keep the original captures, source provenance, capabilities, and entry lifetimes throughout.
...
An earlier outside gate is still a prerequisite, never owned by the batch. A reference under that gate joins its FIFO selection turn before later overlapping work; a disjoint ready reference reserves now. For f(managedBranch, db) followed by db!.set(2), f must already hold db's reservation while managedBranch waits, so f's eventual db write cannot run after set(2). Conversely, a reference below an unresolved managed prefix cannot reserve native descendants ahead of the operation protecting that prefix. References overlapping within the batch share selection; there is no sequential acquire-and-wait between them. These are ordinary scoped entry rules, not permission for a callback to issue work through an unrelated outside Chain and inherit the batch's ownership.
...
- During an out-of-range Array-index entry, issue length/whole-Array observations and structural mutations. They must wait and observe the completed shape, never placeholder growth. In-range holes retain local ordering; consumers distinguish absent completion from explicit undefined, and Array callbacks skip a no-op hole. Old captured Arrays remain unchanged.
