...
`externalState` and `managedState` return the exact value on success and return an Error argument unchanged without reading `then`. `managedStateClass` returns `undefined`. Invalid or conflicting declarations return a validation Error. Each new declaration validates its complete synchronous input before recording anything; none waits for Promises. It performs ordinary supported-thenability recognition without a sample cache. Identity deduplication belongs only to the declaration graph walk; it retains no thenability facts. A callable result is invalid declaration input; if an unsupported identity cannot be inspected safely, validation fails and the declaration records nothing rather than creating poison, a Promise, or persistent state. Repeating a declaration is harmless. Declarations must precede admission: a late declaration never reclassifies an admitted identity and is unsupported because it can affect admission in a later execution.

Managed class prototypes may contain accessors, but Cascada never selects them as methods. They may contain no callable or accessor `then` before `Object.prototype`; this restriction keeps a managed value from becoming a native thenable when an own shadow is deleted or omitted during copying. All semantic instance state must use own enumerable string-keyed data properties. Managed classes must not require private fields, Symbols, non-enumerable or accessor state, mutable closure or module state, hidden shared mutable storage, or native internal slots. Constructors never run during copying, and declaration does not probe whether the supplied function is constructible because Cascada only retains its object prototype.
