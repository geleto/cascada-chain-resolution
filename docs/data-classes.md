...
Managed class prototypes may contain data methods but no accessors or callable `then` before `Object.prototype`. The callable-`then` restriction keeps a managed value from becoming a native thenable when an own shadow is deleted or omitted during copying. All semantic instance state must use own enumerable string-keyed data properties. Managed classes must not require private fields, Symbols, non-enumerable or accessor state, mutable closure or module state, hidden shared mutable storage, or native internal slots. Constructors never run during copying.
