...
2. **[code] Detached continuations need a terminal handler.** Most `onResolve(...)` chains are discarded (`onResolvedValue`, the walk continuation, the import rescan marker). `settlePromise` handles promise *rejection*, but an exception *thrown inside* a continuation rejects the discarded derived promise with no handler — a host unhandled rejection instead of an Error node. Fix in two layers: (a) `onResolve` (helpers.js:39) adds a terminal rejection handler so no throw ever reaches the host's unhandled-rejection path — it lacks node/key/mirror context, so its job is only swallow + deterministic report; (b) where that context exists (`onResolvedValue`, the walk continuation), catch around the continuation body and write a generic Error value through the normal channel — `mirror.currentValue` and, guarded by `canUpdateMirrorToLive`, the live key — same path a rejection takes. Generic Error values are fine; this kernel is a sandbox test for the real implementation inside the language source.
