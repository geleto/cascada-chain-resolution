...
Check the harness before relying on its evidence. Inspection may establish sharing or leases, admit lazy data, install dependencies, flush work, or wait for settlement. Consistency verifiers and initial index construction count as inspections too. For affected sequences, use fresh equivalent inputs for instrumented runs and controls with only final inspection: no initial indexing, intermediate verifier calls, or optional diagnostic reads. Keep observations required by the scenario, and state which inspections are omitted. Do not await each command in the control merely to assert its result. Compare the outcomes the contract requires to agree, and separately assert any intended differences in timing or ownership. Before implementation, account for these effects in the worked traces and verification plan; an unspecified observer effect is an open obligation.

A harness can also produce wrong evidence without changing execution. Capture each command's inputs once for both the model and runtime; a deferred callback must not read a variable reassigned for a later command. Issue each observation synchronously at its program position and keep its result; an observation issued from a continuation, or after awaiting another result, observes a later position. When comparing failure identity or attribution, use independent causal facts, such as supplied source contexts and causes, never the expected result's identity; validate the required kind, attribution, and reference preservation. Settle to quiescence, including inputs that callbacks create during settlement, before reporting a hang. Classify each run's outcome locally: global handlers must not absorb harness crashes, and a missing or empty report is a harness failure, not a clean result.
