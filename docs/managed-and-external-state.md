...
One execution identity map leaves use state absent before first use, then records either one live tree leaf or permanent conflict with the first stable reason. It recognizes later aliases of every tree identity. Import and storage are not uses. Calls, property access, and host-visible inputs are uses. The first actual use must select one live leaf; use through another path or context Chain, or through an alias elsewhere, conflicts before host access. An entered contextual Chain retains the original leaf, so it remains the same location. One operation validates all proposals from the same identity-state snapshot and commits new locations together only when host access will occur. Repair cannot clear conflict. Identities absent from every tree remain observation-only and may be observed from any location.
