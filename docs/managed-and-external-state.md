...
Managed mutation scopes exclude canonical registered mutable external locations. Controlled managed writes may retain inert external aliases at other locations; those aliases grant no external observation or mutation authority. Native managed receiver preparation still rejects registered mutable external identities anywhere in its consumed receiver graph. Observation-only external leaves remain permitted. Every managed mutation protects its pre-operation scope using versions and COW. Failure discards working changes and records poison with the baseline in owning-placement metadata. Invalid mixed scopes are rejected before mutation and retain their unchanged baseline. Repair reveals the baseline; native external failure instead keeps completed effects.

Entry may select mixed branches. Managed mutating entry uses a placement gate and ordinary path COW; managed read-only entry uses a lease. Covered external resources use the same hierarchical reservations: observation coverage for every read-only entry and exclusive coverage for mutating entry. Private ordering state lets contained commands use that algorithm without waiting behind outside work blocked by their entry. One command completion covers issuance through native effects. Entry does not create a rollback transaction for its callback.
