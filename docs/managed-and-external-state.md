...
Initial synchronous ContextChain import filters a compiler mutation access tree into its static external mutation tree. The compiler supplies only static access prefixes as property maps with `{}` endpoints and no host-category or poison-scope markers; [integration.md](integration.md#compiler-construction-of-the-mutation-access-tree) defines construction. Import follows requested original placements using staged/admitted facts, stops at the first external owner, and prunes non-external endpoints and empty branches. Promises and thenables never contribute authority, including synchronous delivery. Context cycles and aliases are bounded by the finite compiler tree; no managed subtree search or discovery cycle cache is needed. The runtime owns its filtered records and leaves the compiler input unchanged. Admission and unique identity registration commit atomically before context construction returns. Later fulfillment, assignment, COW, Array remapping, aliasing, deletion, and entry neither add nor move locations.

One execution identity map owns context binding state and the phase cursor. An otherwise-valid context import atomically registers its external identities. Competing independent root context registrations invalidate their shared authority without an arrival-order winner; regular-Chain admission, storage, or invalid access never poisons the context binding. External access validates the selected context and path before host work, including after waiting. Off-path access returns a local Error; already-completed results and earlier placements are not rewritten. A context may remain imported with invalid authority. Ordinary repair cannot change registrations or clear invalid authority. Within one root context, discovery of the same external identity at distinct normalized candidate paths rejects the initial import with `ExternalLocationConflict` before any registration commits, including under explicit static managed scopes. Repeated discovery of the same location merges; see [duplicate candidate paths](external-context-ordering.md#duplicate-candidate-paths-within-one-context).

Registered mutable external boundaries and their original context-path ancestors form a fixed namespace. Reject whole namespace replacement/deletion, structural remapping, and arbitrary managed mutating methods on namespace-bearing receivers before effects. Ordinary managed children beside the namespace remain mutable; physical COW preserves the logical bindings. No proposed-graph validation or namespace-specific receiver copy is needed. External references remain exact through managed copying, but an inert reference at another location gains neither authority nor a new fixed namespace. Invalid leaves stay in the static tree without pruning, while explicit access returns their binding Error.

A statically selected mutable external boundary uses one ordinary readers-writer phase handle. Observations wait for earlier mutations and overlap each other; the next mutation or repair waits for preceding observations and mutations. Traverse earlier managed and entry-binding gates before reserving that phase, then publish its successor before subscribing to its predecessor. Dynamic selection of a mutable boundary fails locally without candidate reservations. Dynamic native suffix keys remain supported after static owner selection. Observation-only external identities require no mutation phase or static-path restriction.

A `!` prefix defines the complete mutation scope. A marker inside opaque external state clamps to the first external boundary. A broader managed prefix uses ordinary managed mutation; an external host effect selects only its reached owning boundary, without authority over managed data or other external owners. Managed methods receive no authority over opaque descendants. An entered Chain retains the reached canonical tree node; it creates no derived index. Entry into mutable external state selects only the whole boundary through a static path and gates its context binding while the callback issues work. Inner commands use ordinary phases; the entry itself holds no external phase. Entering a native property is a recoverable PropertyValidation failure at target selection, even when caused by compiler lowering: return local poison for read-only entry or publish through the external scope for mutating entry, preserving its fixed binding and existing poison.
...
Within the native suffix, intermediate property values and call receivers/callables must be ready; no Promise is consumed to continue that path. Final lookup and direct method results may be Promises. Assignment/deletion does not read the old final property, so either may replace/remove a Promise-valued target. These operations use native storage, never managed Promise versions or COW inside the external owner.

The selected mutation scope owns poison once. An external scope uses its phase; a managed scope retaining fixed native bindings keeps its value under a logical scope guard. An external child still uses its phase for ordering but gets no duplicate poison when the ancestor owns the failure. All contextual descendant access, including through earlier entered Chains, obeys current ancestor guards. Repair clears only the selected scope, preserves native bindings, and does not clear independently poisoned descendants or invalid authority.

Contextual Error queries include accessible scope poison in phase order without reading native properties. A healthy external scope gives false/null and a poisoned one gives true/its original Error. Ancestor queries include required managed Errors and external scope metadata; a poisoned managed guard terminates traversal and retains its children solely for repair. No duplicate Error cache or execution-wide recoverable history is kept.
