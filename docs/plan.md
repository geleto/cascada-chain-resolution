...
8. **Implemented: fatal error reporting.** Internal/runtime failures go through the global fatal boundary: `reportFatalError(error)` reports and throws. Fatal errors are never language Error values and must never be confused with rejected data promises, which continue to be converted by `onInitialPromiseResolve` into language Error values at promise-settlement boundaries.
    ...
    - `runFatal` catches only synchronous throws. Every public operation uses it for its synchronous prefix, while `onInitialPromiseResolve` and `onAllPromisesReady` own their respective asynchronous rejection policies without adding another reaction.
...
12. **Superseded by item 17: imported cycle Errors.** This item records the
    ...
    **Coverage.** Integration coverage runs under both metadata modes and includes:
    ...
    - One-stage helper ordering: same-promise value consumers retain registration order, and interleaved `onInitialPromiseResolve`/`onAllPromisesReady` consumers do not invert. Rejection conversion, conversion failure, and synchronous continuation throws retain issue 8's language/fatal taxonomy.
...
13. **Implemented: uniform non-extensible node support.** Frozen, sealed, and otherwise non-extensible objects are ordinary tracked nodes. They are implicitly shared, store runtime metadata through the WeakMap fallback, and participate in the same lazy ref index as extensible nodes. Import prepares cycles and Promise continuations eagerly, but only a counter-based operation pays to index the reached branch.
    ...
    No non-extensible-specific META or mirror field exists. `hasSharedMark` derives their sharedness, `markShared` leaves them unchanged, and inline metadata storage falls back to the same WeakMap used by explicit WeakMap mode. Imported preparation ensures META only as required for preparation; other operations add metadata only for boundaries, counters, mirrors, cycle cuts, or explicit shared marks.
...
15. **Implemented: operation module separation.** `src/index.js` is the small composition root: it owns `Chain`, Promise-mirror initialization, and the public exports. Public signatures and operation ordering are unchanged.
    ...
    - `src/observations.js` owns lookup, export, `hasError`, `getErrors`, observational path walking, indexed Error dispatch, and the counter-fenced Error-search traversal. Its walkers and operation-local state remain private.
...
19. **Implemented: live-property Promise mirrors.** Use one direct
    ...
    **Rejection handling.** The first resolver uses the one-reaction
    `onInitialPromiseResolve` helper, which converts a rejected data Promise to one
...
20. **Implemented: property-state class copy-on-write.** Imported class instances can opt into COW support when their complete state consists of own enumerable string-keyed data properties. This completed kernel work is useful groundwork for class support; class construction and method execution are separate language/runtime steps. `src/mutations.js` exports one host-only `PROPERTY_STATE_CLASS` Symbol for internal tests and owns the private shell factory; step 20 does not add the Symbol to the package-level `src/index.js` API. An instance's exact custom prototype carries its own `PROPERTY_STATE_CLASS === true` data property; certification is not inherited and supplies no copying callback. Shell selection occurs only when COW is required and adds no import preparation, metadata, registry, built-in detector, or generalized classification phase. The shell has the exact sound prototype from creation, the existing shallow-copy loop copies the existing language-visible properties without a second read path, and no constructor runs. The result preserves the class hierarchy, `instanceof`, inherited and overridden methods, Promise mirrors, Errors, aliases, cycles, ownership isolation, and refcounts without changing the language-property model.

    The private factory first handles every real array, including subclasses and cross-realm arrays, by returning the existing same-length local ordinary-array shell; subclass prototypes and methods are normalized away. It then returns `Object.create(null)` for a null prototype, `{}` for a plain object from any realm, `Object.create(proto)` for a certified non-array custom prototype, and no shell for any other prototype. Plain-object shapes are recognized structurally from the intrinsic prototype's own constructor back-reference, preserving pre-step-20 cross-realm behavior without a realm registry or built-in list. Supported class fields must be own enumerable data properties and accept normalization to enumerable writable configurable data properties, but certification is trusted: the existing `readLanguageProperty` path is unchanged and no per-field descriptor validation is added.

    Certification is one predicate: only an own marker data descriptor whose value is exactly `true` opts in. A missing marker, accessor marker, `false`, `undefined`, or any other value is simply unsupported and follows the Error-value path; reflection or proxy throws remain fatal. Accessors in instance data, private fields, required non-enumerable/Symbol state, closures, hidden sharing, and native internal slots are trusted facts rather than detected ones. Marking an incompatible built-in is the same host-contract violation as falsely certifying private state, so no cross-realm intrinsic list is maintained. The caller creates the shell before `shallowCopy` touches `attachmentPath` or properties. No shell produces a language Error that the existing recursive mutation return path installs at the unsupported instance's nested, root, or Promise-resumed placement. An import boundary adds attribution; without one, the same Error is installed without attribution. A false trusted assertion is not promised proactive detection, and any unexpected throw it later causes is fatal without rollback.

    Prototype methods are preserved for certified non-array classes, but step 20 does not make opaque method execution part of the kernel. Direct mutation of tracked state outside Cascada path operations remains a host/compiler contract violation, and export remains deliberately plain data. Comprehensive tests import the certification Symbol directly from its owning module and cover nested and root instances, at least three independently certified inheritance levels with new fields at each level, ordinary siblings, `instanceof`, inherited and overridden pure methods, Promise/Error fields, aliases, cycles, refcounts and verification, null-prototype records, local and cross-realm arrays and plain objects, sparse arrays, normalization of marked and unmarked array subclasses, external isolation, exact nested/root/Promise-resumed Error placement for every unsupported certification shape, unexpected-throw fatal paths, documentation of undetectable trusted violations, callable `then`, and both metadata storage modes. The complete internal contract is [`property-state-classes.md`](property-state-classes.md). Deferred array-subclass prototype preservation, Map, Set, other built-ins, and virtual properties are isolated in [`future/keyed-containers.md`](future/keyed-containers.md). The entered-receiver groundwork for possible compiled methods is specified in [`enter-leave.md`](enter-leave.md) and [`run.md`](run.md); arbitrary mutating native methods remain out of scope.

21. **Planned: enter/leave path ownership.** Once the target's owning parent is available, `enter(chain, path, true)` captures the target value—including an Error, Promise, or missing `undefined`—into a private Chain and immediately installs a fresh gate Promise at the public placement. The compiler-facing `enter` validates its exact Boolean analysis fact and selects an encapsulated mutating or read-only path inside `src/enter-leave.js`, preserving one simple call surface without exposing either implementation path or requiring named helpers. Every consumer, including `run`, calls only `enter` and `leave`. The existing mutation walk changes its boundary to `walkMutationPath(chain, ...)`, asserts Chain mutation access there, derives `chain._state` internally, and accepts an optional one-shot acquisition sink. Its ordinary return channel continues to reconstruct graph nodes, while a direct terminal stores `Entry | Error` synchronously and the first pending ancestor lazily allocates the one acquisition Promise whose resolver the terminal calls inside its FIFO continuation. No second walker, `{ node, result }` record, persistent location capability, or access-check bypass through a raw root holder is added. A target Promise never delays mutating acquisition: a transfer wrapper samples the source mirror's latest prepared logical value at `enter`'s FIFO position instead of independently seeding from raw settlement, preserving all pre-entry effects even when they COW. Transfer and COW fork share one private source-mirror sampling core and use ordinary destination `PromiseMirror` instances; no transfer kind or lifecycle state is added. When owning-path COW leaves the target version reachable from an old world, acquisition derives `retainedInOldWorld` from the existing attachment path; a direct tracked target is marked shared before the Entry escapes, and a Promise transfer marks only its prepared sampled tracked value synchronously before later private consumers. It never marks the raw Promise. Only pending ancestors delay a mutating Entry.

    By contrast, `enter(chain, path, false)` installs no gate and waits through a target Promise before protecting its resolved snapshot with a temporary read lease; it propagates a target Error. A root already explicitly shared, imported, or non-extensible needs no redundant lease. Native raw-snapshot work must finish before release, while kernel continuations issued before closure retain their ordinary captured-mirror FIFO semantics after release, so no outstanding-operation counter is added. Later traversals through a gated path wait through the existing mirror FIFO while unrelated paths continue; a direct later replacement creates a newer version immediately, and another mutating Entry acquires immediately while its data version serializes through transfer-mirror sampling.

    `leave(entry)` fatally rejects lifecycle misuse, registers gate completion, captures the current private root, internally registers its one required root-mirror wait if pending, and only then closes the private Chain to new issuance. The mirror registration is internal rather than a public lookup through the private Chain. Chain access is the sole active/closed lifecycle authority; an Entry retains only its private Chain, gate/resolver when mutating, or optional leased tracked value when reading—not a mode kind, source Chain, captured placement, or duplicate state. Gate presence distinguishes the two Entry forms. Mutating leave always returns publication completion; read-only leave releases only its optional leased value and returns `undefined`. `setMirrorValue` globally rejects a Promise before import preparation, ref indexing, descriptor mutation, or publication; new Promise values must use `replaceProperty` and a fresh version. Leave nevertheless keeps a local non-Promise assertion immediately before the gate resolver, protecting that assimilation boundary if raw compiler or host mutation bypassed the property transition. It waits at most once on the captured private-root mirror. No suspended earlier-issued operation can install a later root version: root replacement targets the always-available private holder and runs synchronously when issued. Already-issued private continuations may remain suspended and operate on the now-public graph through their existing mirrors, so no Chain-wide completion queue or recursive root readiness tree is added. Nested Entries need no LIFO stack.

    Import attribution remains version-local: pending targets carry it in source/transfer mirrors and prepared tracked values carry their own META boundary; the private Chain receives no sticky inherited boundary. The gate mirror carries the owning walk's normal imported-attachment preparer. Its ordinary Promise assignment permanently pins a fresh COW attachment root because private work may later publish imported data even when the current ancestry is clean. Repeated sequential Entries on such a path may therefore re-COW it, although awaiting publication still keeps one live gate and no accumulated Promise tail. Avoiding that pin would require either a static no-import guarantee or a temporary pin spanning every recursive import-preparation frontier.

    Chain access is checked only at the shared observation- and mutation-walker issuance boundaries; already-issued inner continuations do not recheck. `acquireReadLease` returns the tracked value only when it increments the private META count, making that value both release token and acquisition flag. The `enter`/`leave` APIs initially remain internal to `src/enter-leave.js`; a non-Boolean `mutates`, read-only/closed Chain misuse, and invalid descriptor or publication contracts remain fatal without changing existing path-operation result shapes. Every compiler-generated mutating Entry must reach leave; abandonment and dynamic self-wait are trusted lowering violations that the low-level primitive cannot make productive. The first implementation adds no active-Entry registry; step 22 structured lowering owns normal completion, and a development-only diagnostic remains optional later work. The complete lifecycle, ownership, ordering, failure, deadlock, nesting, and test design is [`enter-leave.md`](enter-leave.md).

22. **Planned: data operations through run.** `run` builds on enter/leave rather than the discarded recursive-proxy design. Pure standalone functions have no effect path, and trusted read-only calls use a read-only Entry. A statically direct-safe mutating data operation runs directly when its receiver and arguments are synchronously ready, avoiding a gate, Entry, and microtask. Any operation that may suspend enters its receiver before invocation, then operates only through the private Chain with normal kernel reads/writes and leaves on every completion path. One relative-path operation ABI serves both direct and entered execution. Mutating native JavaScript methods remain unsupported because raw writes bypass mirrors, import preparation, refcounts, and cycle bookkeeping. The finished design is [`run.md`](run.md); the discarded native proxy analysis remains in [`future/run-draft-proxy-archive.md`](future/run-draft-proxy-archive.md).
