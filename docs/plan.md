...
1. **Implemented: subtree counters replace the CLEAN flag.** `src/refcounts.js` owns exact `promiseCount`/`errorCount` totals for ref-indexed branches. Ref-indexing is lazy and per branch; never-checked data pays zero counter bookkeeping. Writes, deletes, and live Promise-property resolution use atomic property transitions, while indexed COW copies reconstruct counters from their own placements.
    ...
    - Non-extensible nodes enter through import and follow the same counter rules as other imported nodes.
...
2. **Implemented: single META record and accessors.** `src/meta.js` owns the one logical metadata record. Runtime-owned metadata uses an inline non-enumerable Symbol by default; imported metadata and all metadata when `CASCADA_META_STORAGE=weakmap` use the WeakMap. Shared marks, read-entry counts, promise mirrors, and subtree counters all use that record, and `shallowCopy` never copies it as language data. The default test command runs the complete suite once in each mode.
    ...
    - `metaOf(value)` returns only the node's own metadata. Inline mode uses an own Symbol on runtime-owned nodes; imported nodes use the WeakMap.
    - `requiresCopyOnWrite(value)` combines an explicit shared mark and any active read-only entry.
...
4. **Superseded by items 11–12: recursive import marking.** Imported cycles and nested Promises are discovered eagerly without marking every descendant as a direct boundary; all newly created imported metadata uses the WeakMap in both storage modes. Full design: `docs/import-preparation.md`.
...
7. **Implemented: `hasError`.** `hasError` uses subtree counters, Promise
    ...
    - One call-local `visited` WeakSet is shared through the complete promise tree. Every traversed node is checked and added, so an imported identity repeated across synchronous or promised paths is visited once without adding query-specific ownership marks. It does not retain settled branches while another captured promise remains pending. A trusted identity cannot recur inside one branch under the compiler's single-owner/COW contract.
...
11. **Implemented with item 12: eager imported-subtree cycle preparation.** Import marks every newly reached host identity imported and shared, stores its attribution, and immediately walks the detached external graph to discover cycle-closing properties and nested Promises. Subtree counters remain lazy. Trusted language data follows the compiler's single-owner/COW contract and uses the ordinary tree-shaped ref-index walk without identity or cycle bookkeeping.
    ...
    **Import boundary.** `import(value, errorContext)` rejects every falsy context. Every newly imported tracked identity receives a direct boundary and shared ownership; the graph is prepared synchronously before import returns. A promise root uses one `resolveInitialValueOrPoison` wrapper that performs the same preparation before language consumers observe its resolved value. Preparation owns the imported-only identity/path state and nested Promise registrations, but allocates no counters.

    **Before ref-indexing.** Every imported identity protects itself through direct provenance and shared ownership, including when independently rooted. Mutation COWs before entering it; the shallow copy carries no META and is owned. A Promise property's mirror carries the source container's provenance until COW creates a runtime-owned property version. Aliases and cycle cuts are already known even when no counter operation is ever issued.

    **Imported preparation and ref-index commit.** `prepareImportedData` starts the graph walk once at public import. Each synchronous segment has a current path and weak visited set. A current-path repeat receives a cycle Error; another same-segment repeat is skipped. Every first-prepared tracked identity receives direct imported/shared META. A META hit identifies an already prepared or runtime-owned identity globally: it is marked shared and checked only against the copied current ancestry by the private fixed-path scanner instead of receiving full preparation again. A synchronous match propagates to the property or mirror that entered this prepared island; it is never stored on an inner shared node where revocation could leave it as a phantom. Pending properties receive mirrors plus preparation consumers. `buildRefIndex` later indexes only the reached branch, expanding through each cycle-cut target as a separate component.
    ...
    **Attribution and external metadata.** Every newly imported tracked identity receives a direct boundary and shared mark in WeakMap metadata. Its next shallow copy omits that metadata and becomes owned again. A Promise mirror on an imported placement retains property-level provenance; forks sample it in FIFO order and pass it transiently into preparation without putting it on their runtime-owned destination mirrors. Fresh nested Promise results receive the same direct provenance during preparation. Non-extensible values enter through import and follow the same rules.
...
12. **Superseded by item 17: imported cycle Errors.** This item records the
    ...
    **Imported metadata.** Both storage modes have identical behavior. Imported values use `META_MAP` for attribution, cycle cuts, mirrors, and logical counters without adding properties to host data; `metaOf` and `ensureMeta` are the only access boundary. An existing inline metadata record remains authoritative when a runtime-owned identity is later reached through import. Non-extensible nodes use ordinary exact counters because they enter through import, and Promises and Errors inside them are valid language data.
    ...
    **Coverage.** Integration coverage runs under both metadata modes and includes:
    ...
    - Imported non-extensible cycles, Promises, Errors, and enumerable own `__proto__` keys in both storage modes, including external metadata, live/revoked delivery, exact logical counts, and removal of every stale extensibility shortcut.
...
13. **Implemented: uniform non-extensible node support.** Frozen, sealed, and otherwise non-extensible objects are external data and enter through import. Their boundary explicitly protects them through copy-on-write, their metadata lives in the WeakMap, and they participate in the same lazy ref index as other imported nodes. Import prepares cycles and Promise continuations eagerly, but only a counter-based operation pays to index the reached branch.
    ...
    No non-extensible-specific metadata, mirror field, or COW condition exists. Imported preparation stores newly needed metadata externally; other operations add fields only for boundaries, counters, mirrors, cycle cuts, active read entries, or explicit shared marks.
    ...
    **One counter invariant.** After `buildRefIndex` succeeds, every raw-reachable tracked value has a counter regardless of extensibility. A cycle cut contributes its own count and omits the crossing reverse edge, while its raw target starts an independently indexed component. Error queries remain counter-fenced across every component, and export remains a counter-free raw walk.
    ...
    Ownership selects metadata storage and copy-on-write; `Object.isExtensible` has no semantic role. A write-through Promise publication validates the property immediately before writing, while an imported mirror preserves the property and needs no extensibility or writability branch.

    The demand-driven tradeoff is deliberate: querying an imported non-extensible branch allocates counters, mirrors, and reverse edges just like querying another imported branch. Imported Promise properties preserve their physical Promises and store settled logical values in their mirrors, so frozen, sealed, and extensible imports use one path. Coverage under both metadata modes includes frozen data, synchronous Errors, Promise properties, arrays, aliases, cycles, lazy indexing, exact counter transitions, COW assignment/deletion, and verification.
...
19. **Implemented: live-property Promise mirrors.** A mirror is live only while
    ...
    **Write-through requirement.** Only a mirror that updates its physical
    ...
    data enters through import, whose metadata already uses the WeakMap.
