...
20. **Implemented kernel experiment; language integration on hold: property-state class copy-on-write.** The implementation and tests remain, but Cascada variables are data-only for now, the certification Symbol is not public, and class/function execution is not planned work. Imported class instances can opt into the internal COW experiment when their complete state consists of own enumerable string-keyed data properties. `src/mutations.js` exports one host-only `PROPERTY_STATE_CLASS` Symbol for internal tests and owns the private shell factory; step 20 does not add the Symbol to the package-level `src/index.js` API. An instance's exact custom prototype carries its own `PROPERTY_STATE_CLASS === true` data property; certification is not inherited and supplies no copying callback. Shell selection occurs only when COW is required and adds no import preparation, metadata, registry, built-in detector, or generalized classification phase. The shell has the exact sound prototype from creation, the existing shallow-copy loop copies the existing language-visible properties without a second read path, and no constructor runs. The result preserves the class hierarchy, `instanceof`, inherited and overridden methods, Promise mirrors, Errors, aliases, cycles, ownership isolation, and refcounts without changing the language-property model.
