...
2. **Implemented: single META record and accessors.** `src/meta.js` owns the one logical metadata record. Storage is selected by `STORE_META_IN_WEAKMAP`, derived from `CASCADA_META_STORAGE`: inline non-enumerable Symbol property by default, or WeakMap when set to `weakmap`. Shared marks, promise mirrors, and subtree counters all use that record, and `shallowCopy` never copies it as language data. The default test command runs the complete suite once in each mode.
    ...
    - COW copies receive no copied META object. If counters are live, `indexCopyIfSourceIndexed` creates fresh metadata and reconstructs counts from the copy's logical placements.
...
12. **Superseded by item 17: imported cycle Errors.** This item records the
    ...
    **Coverage.** Integration coverage runs under both metadata modes and includes:
    ...
    - One-stage helper ordering: same-promise value consumers retain registration order, and interleaved `onValueResolve`/`onInternalResolve` consumers do not invert. Rejection conversion, conversion failure, and synchronous continuation throws retain issue 8's language/fatal taxonomy.
...
13. **Implemented: uniform non-extensible node support.** The special frozen-subtree validation and counting model is removed. Frozen, sealed, and otherwise non-extensible objects are ordinary tracked nodes that are implicitly shared, store runtime metadata through the existing WeakMap fallback, and participate in the same lazy ref-index as extensible nodes. Import eagerly prepares cycles and Promise continuations, but only the first counter-based operation on a branch pays to index its non-extensible nodes.
    ...
    **One counter invariant.** After a successful `buildRefIndex`, every raw-reachable tracked value has a counter, regardless of extensibility. A cycle cut stops count propagation and omits its reverse edge, but its target starts an independently indexed component. Error queries stay counter-fenced across every component. Export is always raw and does not require counters anywhere in its captured branch.

    Error queries, mirror continuations, verification, and live reverse-edge bookkeeping consume that invariant with required-counter checks. `addParentEdge` and `removeParentEdge` remain no-ops for untracked values but require a counter for every tracked child; component indexing records each successfully indexed tracked child without a second optional check. A missing counter on an ordinary tracked edge is fatal downward-closure corruption, not a clean result. Promise observation always creates or reuses a mirror and registers through `PromiseMirror.onResolve`; an indexed owner must already have the matching mirror, while an unindexed raw export owner may create one.

    Imported preparation cuts fresh DFS back-edges and the entering placements of prepared islands that reach a captured fixed path. One `buildRefIndex` call at the reached branch builds all of its raw-reachable cut-separated components without indexing unrelated imported siblings. For imported Promises, the mandatory consumer stores private raw state; each following imported consumer resumes only its full-preparation or fixed-path work and indexes the value for its next FIFO consumer. No imported-record commit loop is needed. A private draining Promise-placement cut remains invisible to attached counts until final drain commits it safely. Every resulting live transition continues through item 12's atomic edge commit.
