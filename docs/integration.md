...
Compiler reference/control-flow entry must allow a poisoned final target so the callback can test, repair, or replace it; only a poisoned strict ancestor prevents reaching it. Use the ordinary entry transfer of complete placement state, not a compiler-only recovery bypass.

Entry preserves placement presence: a conditional body that does nothing cannot create an absent property or fill an Array hole. An explicit undefined assignment does create a property. Runtime structural ordering protects the owning Array when an out-of-range mutating entry could alter its length; the compiler need not predict whether the callback takes that branch. The entered reference remains at the requested target.
