...
The compiler need not predict whether an element-entry callback creates a placement. Runtime [structural publication](enter.md#structural-publication) orders possible and committed growth, independently published Array-method results, and length assignment. [Array shape questions](array-view.md#captured-shape-and-pending-placements) preserve each observer's program position.
