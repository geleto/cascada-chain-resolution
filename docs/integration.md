...
For multiple Cascada reference arguments, use the [batch entry protocol](enter.md#multiple-reference-arguments): start all disjoint selections in one issuance turn and reserve each ready argument without waiting for another argument's managed selection. Merge overlapping paths and unresolved selection prefixes into shared entered state so a batch cannot wait behind itself. Each path still passes its own preceding managed gates at its FIFO turn before external reservation. Wait for all required references only before invoking the function. Do not nest acquire-and-wait callbacks, reserve all resources only after the slowest selection, or widen disjoint references to a common parent. Phase 13 defines the public batch-selection handoff over the common kernel; generated code must not reach private tree state.
