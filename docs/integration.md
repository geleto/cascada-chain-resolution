...
Compiler reference/control-flow entry may receive poisoned or presently unreachable targets. Unused targets do not suppress callbacks; actual commands encounter the original poison and ordinary validation, or use permitted repair/replacement. Complete placement transfer preserves recovery without a compiler-only bypass.

Array-index entry protects its element, including holes and out-of-range indexes. Protection creates no physical slot and commits no growth. Register a possible index + 1 contribution at issuance, before predecessor waits. A contained creation, including pending data or poison, commits growth immediately while the element gate remains closed; later deletion or repair to absence does not shrink length. Complete no-growth only after required contained transitions and final publication finish, including queued entries inheriting presence. Array methods publish independent placements carrying retained element transitions, so only required arguments, shape, and consumed values delay their publication. Removed-element results may remain pending independently. Direct length assignment waits for transitions in its truncated suffix before changing live placements. The compiler need not predict whether the callback creates an element.

The current entry API supports one path and one mutable/readonly mode. Phase 13 adds mutation and observation path lists through the [batch entry protocol](enter.md#multiple-reference-arguments), covering multiple Cascada reference arguments and control-flow dependencies. Establish the coordinated entry before evaluating its callback, including a slow condition inside it. A callback that mutates `x.child` and observes `x.other` declares both paths and uses the supplied references. Start all disjoint selections in one issuance turn and reserve each ready argument without waiting for another argument's managed selection. Merge overlapping paths and unresolved selection prefixes into shared entered state so a batch cannot wait behind itself. Each statically selected external coverage reserves at issuance while managed path capture separately passes preceding gates at its FIFO turn. Wait for the required reference captures and predecessors only before invoking the callback, without consuming unused data merely for selection. Do not hoist condition evaluation before entry, nest acquire-and-wait callbacks, reserve all resources only after the slowest selection, or widen disjoint references to a common parent. Phase 13 defines the public handoff over the common kernel; generated code must not reach private tree state. This future coordination is not a prerequisite for the single-path implementation.
...
It invokes no native method and clears selected managed placement poison to reveal its baseline, plus repairable poison in the covered external tree. Healthy or absent managed targets need no managed change; ordinary Error data without a retained baseline returns its original Error and suppresses repair-and-call. Repair-and-call uses run with mutation and repair: true; clear first, then prepare and invoke under the same required ordering. Each managed mutation rolls back its own failed private changes. Compiler-generated enter controls access, not a transaction; higher-level guard/recover owns rollback across multiple commands. Preserve ordinary Error replacement and independent result failures.
