...
`createPoisonError` submits an existing `FatalError` and throws the receiving execution's authoritative fatal, preserves existing poison, or creates a frozen leaf containing the exact raw cause, source-context handle, and kind. The post-host checkpoint runs before interpreting the action's outcome, including when host code swallowed a fatal re-entry exception. Protocol probes select their semantic result inside the action: a non-callable native Error-valued `then` is a successful probe, while throwing that Error is `ThenAccessFailed`.
...
Application callbacks and effectful reflection are supported host code. Captured intrinsics on runtime-owned data, including runtime-owned remap traps, remain trusted work. Their predictable Array index/length failures are validated before invocation; any exact reflection on host-controlled storage uses the private host-action producer. An application comparator is its own `ControlledCallbackFailed` boundary. A Promise returned in that unsupported callback-result position is rejected as `InvalidCallbackResult` and its rejection is owned. “Native code” alone is not a recoverable category.
...
These are explicit probe semantics, not another host-failure policy. Keep their exact `try`/`catch` at the probe that owns the fallback. Do not add a generic completion record, callback, policy mode, or second host-boundary framework merely to share their different result shapes. Other effectful reflection whose failure is observable uses the exact host-action producer and its causal consumer; a complete language-result call uses their `runHostBoundary` composition.
...
Required preparation uses one complete-collection body for ready and pending inputs; a ready failure does not bypass already-selected sibling work. Complete collectors preserve semantic membership, not arrival-independent Error order. They may accumulate arrivals and suppress repeated graph-identity inspection with a visited set. No ordered child sequences or second captured-graph reduction is required solely for Errors. Collection still finishes every required input and preserves normal property-version captures and effect ordering. `hasError` retains its first-proof short circuit. The allowed Error-output and detection races are enumerated in [data-limitations.md](data-limitations.md#allowed-nondeterminism-in-error-handling).
...
`failExecution` is the sole execution-failure and commit entry. It preserves an existing fatal or constructs the private branded fatal with the supplied operation source, commits first, and throws the authoritative outcome. A detecting `runInternalStep` or continuation uses this path; a continuation observing an already failed execution simply returns. The common initial `runInternalStep` checks that the operation context and its execution binding are present before work, covering Chain construction and higher-runtime entry. Omission or accidentally passing a source-only context is diagnosed here. Initial entry throws an existing fatal; a continuation observing failed execution or closed ownership returns without work. Both use one internal `runWithFatalGuard` body after those checks, so a continuation repeats neither entry validation nor the fatal-state check. If routing is missing, private Error construction throws a frozen fatal diagnostic identifying the missing operation context or execution binding; it creates no execution, fallback context, reporter, or global fatal state. An enclosing valid operation context, or a later execution receiving that fatal, submits it normally. Declaration/configuration validation stays outside execution: expected validation returns an ordinary native Error and unexpected implementation exceptions escape synchronously. Missing optional file/line information never makes a supported application failure fatal.
...
The package-internal `commitFatal(execution, candidate)` performs one synchronous transition for the first candidate:
...
| Convert supported host failure | One exact-action marker producer and fixed causal consumer; `runHostBoundary` composes them for direct language-result calls | Shared helpers retain ordinary returns, internal defects stay fatal, and complete boundaries also contextualize returned Errors. |
...
The trusted [integration surface](integration.md) re-exports these implementations, including causal method-result import. [Integration tests](../test/causal-failures.test.js) cover standalone calls and iterator advancement without nested outward-result registration.

- `execution.js` owns the private nullable fatal slot, its public read-only `fatalError` getter, the currently pending operation-result rejection Set, the captured reporter, and package-internal `commitFatal(execution, candidate)`. Internal commit and registration exports serve the Error and facade layers without exposing a public mutator or requiring another token.
- `error.js` owns trusted Error construction, `ERROR_KIND`, precise Error predicates, the single host-action producer/consumer behind the three-argument `runHostBoundary`, hook-free contextualization, compound construction, and fatal submission.
...
- `thenable-subscription.js` owns the shared subscription-exit fatal check and rejection ownership, including exceptional exits. Its scheduling scope keeps the generic host-code re-entry guard inactive during supplied callbacks.
...
- `input-collection.js` joins values through `collectInputs`, keeping poison outside readiness Promises. `prepareInputs` uses that same collector to combine all required input failures before invoking a success continuation. Structural payload joins retain individual Error values; there is no failure-policy flag or second subscription mechanism.
...
- Convert raw failure at its causal boundary through the kernel's host-escape protocol. Direct language-result calls use `runHostBoundary` to preserve or contextualize thrown and returned Errors; shared kernel reflection uses the same exact-action producer with its fixed causal consumer. Apply the graph effect outside recovery and classify raw rejection in the existing boundary continuation. Cascada does not keep a separate marker or catcher implementation.
