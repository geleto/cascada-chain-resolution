...
Canonical namespace poison belongs to the canonical owner's current logical placement. Derive it during the existing ordered path capture where possible. Any retained tree reference must follow ordinary ancestor COW, authorized entered-root handoff/publication, and repair as part of canonical publication; it must not remain attached to a superseded physical container. Copying or repairing an unrelated retained graph never retargets that reference or clears canonical protection. Store no second Error or alias registry. Earlier managed snapshots keep their captured state. Native operations capture canonical managed namespace state at their ordered path turn before external reservation; resumption rechecks binding authority and ordered external scope poison, not later managed gates or namespace versions. Ordinary lookup, export, and Error queries stop at the visible Error, without traversing hidden recovery data. The ordinary graph index counts that logical Error instead of both it and the hidden value, while retained source versions keep settlement bookkeeping needed after repair.
