...
- **Runtime-owned work** (formerly **structural owner**) is trusted runtime machinery such as traversal, continuation, Promise version update, publication, scheduling, or cleanup.
...
| External state | `ExternalLocationConflict`, `ExternalPropertyReadFailed`, `ExternalPropertyWriteFailed`, `ExternalPropertyDeleteFailed`, `ExternalCapabilityEscape`, `InvalidExternalSnapshot` |
...
- A pending Chain root installs its exact initial version and Promise version during initialization. The resolver continuation captures the initialization operation's source until settlement.
- Contextualize in the first import, Promise version, validation, or publication continuation already required by the boundary. Its closure retains source and kind until it runs. Add no forwarding Promise merely for attribution.
- For a direct operation result, later work consumes the Promise returned by that boundary continuation. For a graph placement, later continuations use FIFO only as readiness and read the contextualized logical value already published to the Promise version; they never reinterpret the raw settlement payload.
- A copied or derived pending placement gets a new Promise version at its program position and preserves the source Promise version's eventual contextualized Error by reading it after the source resolver's FIFO position.
- Store no attribution on the source Promise, its identity metadata, the Chain, or the Promise version. Once settlement succeeds, the value needs none; once it fails, the contextualized Error itself carries source and kind.

This removes sideband attribution, not state with a different purpose. A Promise's settlement is immutable, while later FIFO consumers may transform a placement's logical value, so the Promise version still stores that current value. Fixed imported logical overlays preserve host storage whose physical Error or synchronously consumed custom thenable differs from its final language value; complete Error collection retains every required logical failure; and `execution.fatalError` provides synchronous fatal state. None is an alternate place to store Promise attribution.

Every representation boundary consumes logical placement versions, whether fixed or pending. Managed receiver preparation materializes the affected containers and ancestors when their physical slots differ from the prepared graph; it preserves aliases and cycles without copying the whole receiver by default. Derived ArrayViews retain fixed logical values as well as fresh versions for actually pending placements. Neither route rereads a raw Error to attribute it again or resubscribes to a synchronously consumed thenable. Placement replacement and removal complete fallible physical storage work before detaching the old version or committing the prepared refcount change, so a rejected storage operation that leaves physical data unchanged preserves every surviving alias and index.

The [managed-storage Proxy contract](data-limitations.md#proxies-in-managed-storage) requires readable baseline descriptors along a managed mutation path, and each primitive property write, definition, or deletion to implement the requested operation on success and leave the represented graph unchanged on failure. Supported required storage failures use ordinary causal poison and managed mutation rollback; the runtime discards failed working state rather than reversing Proxy traps. It does not detect Proxies or guarantee recovery from a trap that violates this contract. Native managed methods may change their isolated working receiver before failure; that receiver is discarded. Authorized external mutation may leave completed native effects and poison its selected external scope. Internal Array remapping Proxies retain their existing role.
...
Each causal boundary interprets the fulfillment or rejection delivered to its own subscription using its own operation context. A raw rejection reused at two causal boundaries therefore creates two contextualized poison occurrences; later propagation of either occurrence preserves it. Promise versions remain per placement and version because later FIFO consumers may transform that placement's logical value; they do not duplicate thenable settlement state.

Consume a possible placement thenable before choosing its representation. If its resolver transition returns directly, publish the final value directly into runtime-owned storage; when imported or non-writable physical storage must be preserved, reuse a fixed placement overlay for that final logical value. Install a changing Promise version only when the resolver transition actually remains pending. The resolver may write the logical value into unpublished staging state, but its returned direct-or-pending shape, rather than whether that write occurred, selects the representation. Initialize callback-visible unpublished staging and captured versions before subscribing, without installing a changing Promise version or other pending-only machinery. Once the resolver returns pending, JavaScript run-to-completion prevents later asynchronous delivery from interleaving before its pending Promise version is installed.
...
Ordinary payload settlement advances an already committed logical placement. Synchronizing its physical Promise slot is optional: supported preflight or write failure leaves the settled logical value intact, with no additional Error or rollback baseline. Synchronous thenable normalization follows the same rule using a fixed overlay when necessary. Validation, index preparation, structural publication, and actual mutation writes remain required and retain their ordinary failure effects. A fatal or unmarked internal failure still propagates.
...
A recoverable failure in required publication of an already failed value retains both Errors. Required property preflight, index preparation, and mutation writeback retries combine the current diagnostic with each newly discovered failure before publishing the Promise version. Mutation coordination retains the existing receiver/result outcome through target publication and every enclosing writeback, including gate-installation failure. Gate completion records the committed placement after required shared settlement. Its publication signal releases later replacement without waiting for ordinary pending data; captured value consumers follow the logical version and receive normalized poison. A raw gate fulfillment alone does not establish publication success. A mutation that has an independent result keeps receiver publication separate: publish a receiver failure immediately and combine it with an already selected result Error, waiting only for that independent direct result if necessary. Release completed receiver protection while result collection continues. Do not scan nested result payload or add another gate for it.
...
Combination deduplicates equivalent wrappers without merging distinct source contexts or kinds. A new compound's message names the failed boundary without enumerating children; reuse preserves the existing message, source, and identity. Presentation may sort, group, cap, or summarize a separate diagnostic view; it never reorders or changes an already-created compound.

### Graph and operation effects

A script reports recoverable failure only through poisoned returned data. There is no execution-wide recoverable-error history and no required sweep of ignored mutation results. Repair and replacement with fresh data may remove poison before output is consumed. Complete collection applies to the current required data frontier, including every Error that still contributes to that frontier; it does not resurrect discarded or repaired failures. A mutation's graph or external-phase effect must carry its failure without depending on a separate script-level check of the kernel operation result.

#### Scope transitions

A mutation selects one poison owner covering its required logical effects. A managed mutation scope cannot cover canonical registered mutable external locations; selecting such a mixed scope fails before mutation and poisons that managed root while retaining its value. An inert external alias elsewhere does not invalidate controlled managed work; full native managed receiver preparation still rejects registered mutable identities. An external mutation selects a registered external scope and may affect only that native subtree. Entry reserves access and may be mixed; it does not mutate data, create a new poison owner, or exempt contained operations from these restrictions.
...
| Managed mutation fails with E | Discard working changes; expose E at the selected scope and retain its pre-operation value/version for repair |
| External mutation fails with E | Store E at its selected tree scope; retain native identities and any completed native effects |
...
| Valid repair-only | Clear selected managed scope poison to expose its baseline, and covered external subtree poison without native rollback; return undefined |
| Repair-and-call | Clear first, then prepare and invoke under the same reservation; a new failure poisons again |

#### Managed rollback and scope poison

Every managed mutation preserves its selected scope's pre-operation logical value at its ordered turn, after earlier conflicting work and before any write. Protect the captured value using existing ownership, versions, and COW. Controlled mutations copy affected paths; arbitrary native managed methods receive a complete receiver graph isolated from that baseline. Preparation, invocation, receiver validation, and required publication must all succeed before the working state replaces it. On failure discard private working changes, retain the baseline, and publish the original contextual Error. No whole-operation undo log, permanent pre-call identity history, or failed-receiver reconstruction is required. A failure before reaching the requested scope retains the normal first-failed-placement effect, widened only when required to cover structural effects as below; it never invents an unreachable deeper owner.

Store scope poison in the owning placement's existing execution-local version metadata. Its visible logical value is the ordinary Error; private retained state identifies the pre-operation value and captured source version needed by repair. Do not put data inside the Error, add an identity-wide poison flag, copy a failing value merely to poison it, or create a separate guard class/value transport. Native storage and fixed external identities remain unchanged. A gate is only temporary ordering; poison persists independently until repair or permitted replacement.

Selection blocked by existing poison at the selected managed scope or a strict ancestor propagates that Error without entering new-failure publication. Preserve the complete existing placement state and its original recovery baseline; do not retain the visible Error as a replacement baseline or nest recovery records merely because another command was attempted. Ordinary Error data at that scope gains no invented baseline. Permitted final replacement/deletion and explicit repair retain their own rules. Distinguish blocked selection from failure after selecting a healthy scope: an existing Error consumed from arguments or required preparation still causes that healthy scope's normal failure effect. Error identity or contextualization alone cannot distinguish these cases.

Rollback capture preserves placement presence and existing source versions without awaiting or consuming an old value solely to snapshot it. Ready replacement/deletion remains nonblocking for a never-settling old target. Recovery state belongs to the placement: lookup/getErrors return the unchanged Error alone, and storing that Error elsewhere does not transfer rollback state or authority.

Forking or transferring recovery state preserves ownership of its retained baseline as well as the visible Error. If two independently mutable placements retain that baseline, repairing and mutating one must not change what the other later restores. Reuse ordinary sharing, leases, and captured-version ownership; an Error extracted as data still transfers no recovery state.

Installation and clearing use ordinary owner-isolated placement transitions and path COW. Publish a new placement state without changing earlier captures. Copies preserve the retained value/version and logical Error; captured pending versions continue normal settlement and original attribution. Successful publication releases operation-only baseline protection; failure transfers required baseline retention to the poisoned placement. Old logical values and previously returned Errors remain unchanged. Failed working graphs are not exposed, including when native code left unsupported receiver structure. Fatality retains its existing stop contract rather than triggering a rollback sweep.

Canonical namespace poison belongs to the canonical owner's current logical placement. Derive it during the existing ordered path capture where possible. Any retained tree reference must follow ordinary ancestor COW, authorized entered-root handoff/publication, and repair as part of canonical publication; it must not remain attached to a superseded physical container. Copying or repairing an unrelated retained graph never retargets that reference or clears canonical protection. Store no second Error or alias registry. Earlier managed snapshots keep their captured state. Native operations capture canonical managed namespace state at their ordered path turn without first waiting for external predecessors; resumption rechecks binding authority and ordered external scope poison, not later managed gates or namespace versions. Ordinary lookup, export, and Error queries stop at the visible Error, without traversing hidden recovery data. The ordinary graph index counts that logical Error instead of both it and the hidden value, while retained source versions keep settlement bookkeeping needed after repair.

A successful managed mutation may have an independent Error result, such as a removed Error-valued Array element; it remains committed. Ordinary Error assignment is a data write, not an instruction to roll back. Scope poison does not turn every Error into recovery storage. Final replacement/deletion at a replaceable managed placement can discard scope poison and its retained baseline without consuming it; fixed resource namespaces still require repair. An entered callback and a sequence of operations are not one rollback unit. Earlier completed operations remain in the baseline; higher-level Cascada guard/recover owns rollback across a sequence.

#### Managed structural effects

Ordinary Array index creation commits growth at the placement's ordered publication, including creation of poison. A failed mutation prefix at index 5 of an empty Array creates a local Error and length 6; healthy siblings remain accessible. Repair restores the absent baseline as a hole, and deletion also preserves that committed growth. An explicitly broader selected scope retains its own baseline. Intrinsic length writes, truncation, and remapping belong to the Array and can poison that whole scope.

An ordinary containing object does not acquire its own poison merely because one child is poisoned. Internal path COW, replacement of backing storage, and shared settlement/bookkeeping are not additional logical mutation effects. One owning poison hides the affected scope; Error queries see it through ordinary graph traversal, without duplicating poison across ancestors or aliases. Committed logical length remains authoritative when a version has no physical slot. Final presence can become absent while that length remains; copies and native delivery preserve the resulting trailing holes.

Supported refusal of the selected placement's physical write or deletion keeps poison and recovery at that placement, with the same semantics for ready, pending, and entered operations. Publish through its captured logical version, or isolate the containing node and install a logical overlay when no version exists; unaffected aliases remain unchanged. This logical creation commits ordinary Array growth even if storage remains absent. A separate failure while copying or reconstructing an ancestor has its own publication effect and must retain earlier mutation failures and any independently required result Error. Publishing graph failure never waits for an independent result solely to combine its Errors.

Select and protect the structural owner at the operation's ordered turn, before the first write or placeholder that can affect its structure. If that structural transition remains unfinished, gate that owner until publication completes. Failure discards working changes and retains the complete Array baseline, including length, holes, values, and captured versions. Repair restores that baseline through the ordinary placement transition; it never truncates a live Array back to an old length after later writes have been allowed to proceed. Use no Array-specific undo log, per-element duplicate poison, or affected-object registry.

Entry protection commits no mutation, physical gate write, presence change, key reorder, or Array growth. An Array-index entry protects its element and registers possible growth at issuance. A contained creation, including poison or pending data, commits growth before releasing dependent length/range questions, while the element gate remains closed. Deletion or repair to absence keeps that committed length. No-growth completes only after contained transitions and final publication. Logical placement commits also preserve record deletion/recreation order. Each command retains its own scope, causal operation context, and baseline; an independent callback Error does not poison the reference. See [mutating entries](enter.md#mutating-entries).

Structural ownership does not extend work merely because stored data or an independent result remains pending. A successfully installed Promise-valued element follows ordinary property availability and Error-data rules; a completed structural publication is not held open for unrelated settlement. Ordinary Error assignment remains distinct from failed-operation poison. External Arrays use their selected external scope and retain completed native effects, with no managed rollback. A wider poison/rollback owner grants no wider mutation authority: ordinary sibling writes remain valid when they preserve fixed resource locations, while an explicit mixed managed bang scope remains invalid.
...
Repair selects its target under ordinary managed placement ordering and, where external scopes are covered, the common exclusive reservation. Strict-ancestor own poison blocks selection; derived external summaries do not block repairing their child. Clear the selected managed placement's scope poison and reveal its retained baseline. Clear repairable own poison in the covered static external subtree and update summaries without reading native contents. Restoring a managed baseline does not recursively rewrite it, erase ordinary Error data, or clear independent earlier managed failures; those retain their own repair/replacement targets.

Repair-only has the following target outcomes after required selection and ordering:

Repair selects the exact requested placement. It cannot widen an absent Array index to the owning Array: restoring that absent placement has no structural effect and grants no access to sibling external scopes. Healthy intrinsic length values likewise need no restoration.

Ready and deferred repair finish when the restored placement commits, even when its value is an unresolved Promise. Restoration does not wait for that value to become available. An unused raw baseline need not already have a Promise version; an existing captured version preserves its settlement and attribution. A supported restoration failure returns the new boundary Error while preserving the prior visible poison and complete baseline.

| Selected state | Repair-only result and effect |
| --- | --- |
| Healthy managed value or absent managed placement | Return undefined without changing managed contents or creating a placement; still clear covered repairable external metadata, if any |
| Managed scope poison with a retained baseline | Restore that complete placement state, including original absence or source version; return undefined |
| Ordinary Error with no retained baseline | Return that exact Error unchanged; invent no prior value and clear nothing behind it |
| Valid external scope | Clear its covered repairable metadata, preserving native state and descendant binding conflicts; return undefined |
| Poisoned strict ancestor or invalid selected binding | Return the existing blocker without reaching or clearing the target |

Repair-and-call proceeds only after successful repair selection/clearing. An ordinary Error without a baseline or an inaccessible target skips preparation and invocation. A healthy target requires no managed restoration; repair does not search its managed contents for Errors. Supported preflight failure occurs before clearing and preserves the existing recoverable state.

Restoring a managed baseline and removing its poison are one owner-isolated placement-version transition. Retain the old visible Error and complete baseline until required preparation and publication succeed. A supported failure leaves that state unchanged and does not clear covered external poison. Under the same protection, commit the restored placement and covered external metadata before releasing gates/reservations or notifying dependent work; there is no observable clear-then-restore interval. Invalid external repair authority uses ExternalLocationConflict; supported preparation/reflection failure keeps its causal boundary kind. An ordinary Error without a baseline remains that Error, not a new repair diagnostic.

Successful repair-only invokes no native method and returns undefined once valid selection/preflight and its metadata transition finish; an unrecoverable target returns its unchanged blocker as specified above. It does not await hidden managed contents solely to clear poison; their captured versions remain available normally. Ordinary managed selection, COW, and required publication/index preflight keep their supported-reflection boundaries; finish that preflight before clearing poison. Invalid selection and permanent binding conflict retain ordinary validation outcomes. Repair-and-call clears first and then runs normal preparation and invocation under the same required ordering. Its managed rollback baseline is the repaired state, so a new failure never resurrects deliberately cleared poison. Previously returned Errors remain unchanged. Repair creates no authority and performs no native rollback; managed changes were discarded when their operation failed.
...
A missing required Promise version is a known runtime defect at publication. Use the existing required-Promise version invariant and fail immediately; do not wait for that corrupt value to settle. Valid entry callback results and pending gated publication keep their independent lifetimes.
...
- Causal boundaries contextualize ready failures and raw rejection in their first existing import, Promise version, validation, or publication continuation.
...
- Imported occurrences commit atomically, and copied or derived versions preserve origin.
