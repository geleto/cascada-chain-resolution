...
Required preparation uses one complete-collection body for ready and pending inputs; a ready failure does not bypass already-selected sibling work. This includes preparation before the final collector: Array placement capture retains per-descriptor failures alongside known placements, and sorting carries capture/read failures through required conversion or comparator export before deciding whether invocation can proceed. Flat continues each branch required by its depth. Atomic import, failed queries, and structural operations that merely carry payload retain their distinct failure contracts. Complete collectors preserve semantic membership, not arrival-independent Error order. They may accumulate arrivals and suppress repeated graph-identity inspection with a visited set. No ordered child sequences or second captured-graph reduction is required solely for Errors. Collection still finishes every required input and preserves normal property-version captures and effect ordering. `hasError` retains its first-proof short circuit. The allowed Error-output and detection races are enumerated in [data-limitations.md](data-limitations.md#allowed-nondeterminism-in-error-handling).
...
- `PoisonError` supplies synchronous detection and sync-first `.then`; `CompoundPoisonError` inherits that behavior. Both preserve missing-handler behavior, exact rejection identity, and native re-assimilation. `FatalError` is non-thenable under the required stable standard Error prototype contract.
