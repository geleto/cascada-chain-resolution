...
Entry protection commits no mutation. Each contained command retains its own scope, causal operation context, and baseline; an independent callback Error does not poison the reference. Array-index creation commits length even when its published value is poison; repairing it to absence does not shrink the Array. See [structural publication](enter.md#structural-publication) for the shared presence, order, and growth rules.
