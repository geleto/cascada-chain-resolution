...
`ExpectedPrimitive` identifies a selected non-primitive at `lookupPrimitiveValue`; it is a new extraction failure at that operation's source. Existing Error propagation never changes kind.
...
Public lookupPrimitiveValue(chain, path, operationContext) reuses ordinary path observation and accepts only null, undefined, String, Number, Boolean, BigInt, and Symbol primitives. It preserves a selected Error. Another value produces ExpectedPrimitive at the extraction's source without coercion, deep export, descendant inspection, or source mutation. Existing path, reflection, and external-authority checks still apply. Cascada operators own operand combinations and numeric validity. The compiler retains graph and call results in operation Chains and uses this API whenever a result enters primitive expression evaluation.

Normalized kernel results have shape T | PoisonError | Promise<T | PoisonError>. Primitive extraction has shape Primitive | PoisonedValue | Promise<Primitive>, with pending failure rejecting with the ordinary Error. Convert only at the final expression-facing result boundary after required kernel work. A ready failure returns its container synchronously, with no pending-result registration or graph protection. A pending expression operation owns exactly one final outward fatal obligation; conversion cannot detach it from required work or allow fatality before final settlement to appear successful.

Cascada also creates failures during expression evaluation. Those producers use the same public attributed-Error factories and expression wrapper factory. Input consumption in the opposite direction unwraps ready expression poison by its ordinary rejection callback or handles a rejected expression Promise, then stores only the ordinary Error. Existing attribution is preserved; wrapping is never a new causal event.

Error inspection stays successful: getErrors returns or fulfills with a complete Array of ordinary Errors, empty when healthy. Query failure is a separate ordinary Error result. hasError is existence-only and may finish early. A caller explicitly propagating a successful collection combines it and uses the expression factory; a sole distinct Error stays a leaf. Diagnostic views remain immutable safe data. Final script delivery completes export first and converts a failed exported result to native rejection at the higher-runtime outward boundary.
...
- Supported host reflection failure during either query returns or fulfills with QueryReflectionFailed, separately from a successful Error collection. hasError never converts it to true; getErrors never inserts it into the leaf Array.
...
- expression or native outward Promises that assimilate a PoisonedValue after required kernel work.
...
- The public package facade owns normalized operation return and primitive expression extraction. Core modules register no intermediate results. Public Error factories and guarded composition facilities reuse the same implementations; they do not expose an alternate unwrapped graph API or accept a public/internal mode.
...
- hasError exits early and otherwise returns Boolean; getErrors completes its captured traversal and otherwise returns an Array. Supported query reflection failure returns or fulfills with the ordinary QueryReflectionFailed Error, separately from found data. Trusted traversal or bookkeeping failure is fatal.
