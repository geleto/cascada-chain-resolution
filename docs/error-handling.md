...
Poison from a failed mutation must cover every logical effect of that mutation. Select the smallest owning scope that covers the requested mutation scope and any affected intrinsic structure. Preserve an explicit broader scope. For an Array, length changes, truncation, and element remapping belong to the Array; publishing an Error at an out-of-range index would itself extend length, so failure there also poisons the Array at its owning placement. The same rule applies to a broken mutation prefix at such an index. Never expose a healthy Array with length growth caused only by installing failure poison.

An in-range element or hole can retain its ordinary placement-local failure when Array structure is unaffected. An ordinary containing object does not acquire its own poison merely because one child is poisoned. Internal path COW, replacement of backing storage, and shared settlement/bookkeeping are not additional logical mutation effects. One owning poison hides the affected scope; Error queries see it through ordinary graph traversal, without duplicating poison across ancestors or aliases.
...
Entry must preserve shape without implying a mutation. An out-of-range Array-index mutating entry protects the owning Array while its callback decides whether to create that element; installing its gate must not grow length. In-range holes retain local placement protection, including pending presence. A no-op keeps the original hole/length, while publication of an assigned value applies its normal structural effect. Transfer presence with the logical placement state, including after a contained mutation fails and is repaired. This structural reservation is not callback-wide rollback: each contained mutation keeps its own baseline and earlier successful commands remain committed. A failure before element creation poisons the Array; element replacement or repair cannot cross that poison. After successful creation, a later mutation confined to the element has an element-local failure owner. The entry's wider gate does not widen every contained command's poison scope. See [mutating entries](enter.md#mutating-entries).
