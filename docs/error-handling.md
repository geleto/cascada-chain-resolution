...
| External state | `ExternalLocationConflict`, `ExternalPropertyReadFailed`, `ExternalPropertyWriteFailed`, `ExternalPropertyDeleteFailed`, `ExternalCapabilityEscape`, `InvalidExternalSnapshot`, `ExternalRepairFailed` |
...
The fixed context tree may reference the current canonical poisoned placement for live external access; it stores no second Error. Earlier managed snapshots keep their captured state while native access obeys the live canonical namespace poison at its ordering turn. Ordinary lookup, export, and Error queries stop at the visible Error, without traversing hidden recovery data. The ordinary graph index counts that logical Error instead of both it and the hidden value, while retained source versions keep settlement bookkeeping needed after repair.
