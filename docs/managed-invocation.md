...
- A mutation keeps its private receiver behind the ordinary transition gate. Fulfillment imports the value, validates the receiver, and publishes one mutation outcome. Rejection contextualizes the same way and publishes that occurrence at the selected mutation scope.
...
- Any recoverable mutation failure normally poisons the selected scope and becomes the operation result; pending transport fulfills with that ordinary Error after the required effect is published. If the scope is a fixed mutable namespace, retain it beneath its logical poison guard; forbidden structural work is rejected before invocation. Lookup, queries, and export observe that poison until repair; the result is not a healthy preserved scope plus a separately checked Error.
