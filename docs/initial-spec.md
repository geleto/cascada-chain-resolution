...
Immutability is evaluated only from the point of the target language. Promises and resolved values are interchangeable (promises are implicitly resolved), none of the markings are visible, etc… The immutable mark is a non-enumerable Symbol property ignored by the language. Non-extensible external objects, such as frozen/sealed objects, are treated as already immutable without adding Symbol metadata.
