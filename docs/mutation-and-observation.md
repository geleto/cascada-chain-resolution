...
For a native managed mutating call that reaches invocation, full receiver isolation has graph-sized work and allocation even on success. This is an accepted cost of automatic rollback. Preparation already visits the receiver; combine preparation/materialization with isolation where the existing copier can produce one independent working graph. Controlled writes retain path COW and Array sharing. Do not make recovery depend on whether the receiver happened to be shared, and do not copy unrelated scope descendants.
...
Capture explicit arguments, the issuing view, and one required effect completion at issuance. Pass preceding managed gates and required managed path selection, then reserve in that view before invocation-input waits, export, or native-suffix work. Reuse the completion for the reservation; its entry lifetime starts at issuance even while selection is pending. With no preceding wait, reservation remains synchronous. Snapshot reads hold observation coverage through direct availability and ready-only detached copying. Method calls hold access through direct asynchronous work and required result import, not independent nested result Promises. Property writes export the RHS before writing and retain their reservation through publication of poison or success. Observation-only external identities remain unlocked.
...
A mutating managed entry installs its placement gate before invoking the callback. Ordinary path COW isolates gate installation from shared or leased owners. It need not copy unprotected ancestors or eagerly deep-copy the entered value. Entering object property a.x leaves a.y available. Transfer complete placement state, including absence: a no-op entry cannot create a property with undefined. An out-of-range Array-index mutating entry uses the ordinary owning-Array structural gate until its final presence/shape is known; the reference still selects the element, and no placeholder grows length. In-range elements and holes keep local gates, with consumers respecting final placement presence rather than treating a pending gate as a present undefined element. Managed read-only entry retains its counted lease and permits outside writers to COW; lookup results establish permanent sharing as usual.
...
Contained external work uses the same reservation algorithm in an entry-scoped view covered by the outer reservation. It does not wait for later outside work already waiting for the entry. Nested views inherit authority and use the same algorithm, without graph copies, an unrelated scheduler, or ownership of outside reservations. The entry waits for captured predecessors and rechecks selected binding and strict-ancestor external poison before invoking its callback; target/descendant poison still permits inspection and repair. Outer coverage completes after callback closure and required contained external effects. Independently pending results do not extend those effects.
