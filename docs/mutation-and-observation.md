...
At a consuming read, `readLanguagePlacement` derives presence and value from one descriptor or installed version, normalizes raw data once, and captures any version normalization creates. `readLanguageProperty` uses that same read when only the value is needed. Reuse the returned placement within that transition instead of rereading storage. Baseline `capturePlacement` remains non-consuming, and presence-only scans remain separate so they do not start unused thenables or move value consumption ahead of its ordered turn. These are local captures, not an identity-level descriptor cache.
...
Every logical consumer uses the common property layer, including for ArrayViews. Candidate enumeration merges physical keys and installed versions; an undecided candidate may later become absent. Bounded Array operations filter both sources to their consumed range. A version can therefore protect an absent element without creating a physical slot or a second kind of hole. Possible growth lives in the ArrayView length sequence; it does not resize backing storage.
