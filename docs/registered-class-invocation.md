...
Before adding registered-class invocation, consolidate record, Array, String, registered-class, and opaque calls into one lifecycle. It owns boundary dispatch, input readiness and protection, dynamic member resolution, invocation, ordered Error handling, mutation publication, result admission, and cleanup. Internal dispatch first selects the receiver category and supported mode without application code. Controlled Array table lookup and native String data-method lookup remain early under the stable-primordial contract; String accessors are never invoked. An internal constructor, controlled-name, or mode rejection performs no boundary-specific receiver or argument preparation. After required input preparation succeeds, record and registered-class members resolve and validate exactly once; failed preparation performs no member-resolution getter, Proxy trap, descriptor access, or callable test. Replace `run`'s Array-specific mutation Boolean with a category-neutral observation-or-mutation request interpreted after receiver classification. Route every supported mutation through `transformProperty` and remove superseded paths.

Each invocation prepares only the inputs its category consumes. A pending selection leases ready managed argument roots until the selected preparation synchronously captures, leases, or ignores them. Export captures host inputs without source leases. Other continuations lease only managed identities they must read later and release them after their last access. External identities instead require the ordering and source-use validation supplied by their call boundary. Host calls consume every explicit argument, while controlled Array methods keep their per-method input selection.

After registered-class boundary dispatch succeeds, the call consumes its complete receiver graph and required arguments before resolving the prototype method. A pending observation holds its receiver-source leases through result copying and admission; mutation preparation holds the same sources until receiver isolation begins. An Array observation leases its receiver only when a pending continuation will reread it. A String receiver is a primitive.
...
Prepare every explicit argument and the complete receiver graph under the invocation's common operation-lifecycle owner, preserving aliases and cycles across all materialized inputs. Recursively capture and resolve every Promise through existing property-version continuations, including Promises revealed by resolution. A fatal failure abandons receiver and argument work together, while a language Error keeps required Error collection open. After closure, late continuations complete shared settlement but perform no further admission, traversal, reflection, materialization, Error collection, or lease acquisition. After clean preparation, resolve the prototype method from the prepared receiver's admitted prototype chain. Resolve it before mutation isolation, isolate when required, then invoke once. A clean missing method is therefore reported only after argument preparation; do not add an early lookup or a separate preflight. Reuse common protection and any selected external identity phases.
