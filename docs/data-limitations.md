...
A managed mutation scope must contain no registered mutable external resources. Observation-only external identities remain permitted opaque leaves. Reject a mixed managed bang scope before invocation and poison that selected scope, retaining its value for explicit repair. The same restriction applies to managed mutating method receivers, including aliases reached outside the context tree. Managed lookup, import, assignment inputs, and retention still use ordinary sharing, leases, versions, and COW; a bang never promotes managed storage to native-writable data.
...
Repairable scope poison is stored separately from the retained value. Managed guard storage is owner/placement-local, preserves captured versions, and does not poison another owner's shared identity. External poison is stored in the external tree. Ordinary Error-valued properties keep normal assignment/deletion semantics. [Scope transitions](error-handling.md#scope-transitions) define the distinction.
...
- A direct Error always reports method failure, whether returned, fulfilled, thrown, or rejected. A mutating call applies its selected scope's failure behavior; returning an Error as successful payload is unsupported. An invalid mixed managed scope retains its value under an owner-isolated poison guard so repair can restore access without moving resources. Ordinary managed scopes without such bindings keep their normal Error-value behavior.
...
Registered identities and connecting placements cannot be replaced, removed, or moved by reset, Array remapping, assignment, deletion, or any native method. Ordinary unregistered child objects and Arrays may be replaced within the selected authority. Managed namespace parents may retain ordinary mutable siblings, but cannot be selected as bang mutation scopes when they contain registered resources. COW, entry, guard poisoning, and repair preserve fixed identities and locations.
...
- An external mutation may mutate only state owned by its selected registered external scope. A native receiver inside that scope uses its ordering; a managed ancestor or separately owned external sibling remains outside it. Exact external arguments grant no mutation authority and remain read-only; exported managed argument copies are independent host data.
...
Entry supports Cascada reference arguments and delayed control flow. It may select a mixed branch and grants no exception to mutation scope rules. Mutating managed entry gates its selected placement, using ordinary ancestor COW where necessary; entering a.x does not block a.y. It also reserves exclusive access to any external descendants. Managed read-only entry leases its captured value and reserves observation access to external descendants. Entry on a registered external scope uses exclusive outside protection in either callback mode; its read-only callback still cannot mutate.
...
Repair-only reserves exclusive access and clears repairable scope Errors throughout the selected subtree, preserving values and fixed external identities. Once the scope is valid and accessible it invokes no fallible native action and returns undefined. It cannot bypass own poison above its target or clear a permanent binding conflict. Derived ancestor summaries do not prevent repairing their poisoned child. Repair does not traverse/erase ordinary Error-valued managed properties or prove physical native state repaired.

Repair-and-call clears the same subtree first, then performs ordinary call preparation and invocation under the same exclusive reservation. Thus apis!!.reset() clears child scope Errors before reset runs; a preparation error or thrown/rejected/direct method Error poisons again with the new failure. Old cleared Errors are not combined back in. Outside operations cannot observe the intermediate clearing while the reservation is held. Entry may be mixed, but repair-and-call still obeys the ordinary managed/external mutation-scope restriction.

Repairable managed guards retain their values in placement-local metadata. Repair exposes that retained state without modifying earlier captured values or returned Errors. Ordinary final Error-valued properties remain replaceable or deletable without repair; guard poison requires explicit repair. No repair creates authority, changes tree structure, transfers resource identity, or rolls back completed host effects.
