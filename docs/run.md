...
String methods use the ordinary observation path without String-specific overload checks. Callable arguments such as replacement callbacks are allowed, subject to exported-argument and result-admission boundaries. Controlled Array intrinsics support the methods declared by the Array method table; `sort` and `toSorted` additionally support the comparator contract below. Array callback methods such as `map`, `filter`, `reduce`, and `forEach` are deferred. Array `keys`, `values`, and `entries` are outside the controlled method table; direct Array iteration and spread use the runtime iterator path.
...
Array `toLocaleString` is deferred because it invokes executable element methods. A direct or settled method result is admitted before entering the graph: opaque and primitive values pass through, while tracked language-data objects receive normal ownership and indexing. Controlled Array and standard String results are runtime-owned; a tracked result from an ordinary trusted object method or Array override is imported because it may retain external state. A returned method Promise uses the canonical helpers, converts rejection to a language Error, and applies the same admission rule to fulfillment.
...
`sort` and `toSorted` retain the logical values and follow Sort comparison. `sort` leaves holes after explicit `undefined`; `toSorted` follows its standard read-through-holes result. Intrinsic Array `toString` uses intrinsic logical joining regardless of properties named `join`. Pending or erroneous non-index properties are otherwise irrelevant.
...
After an observation target is reached, `run` starts argument preparation. A direct operation returns directly without lease bookkeeping. If an operation on a tracked receiver remains pending on arguments, Array elements, coercion, or an asynchronous method result, it holds one read lease until result preparation completes. The lease protects the resolved receiver from later in-place mutation. Immutable String receivers and materialized temporary views need no lease.
...
Observation always preserves the receiver. Mutation preserves it when `attachmentPath` is defined or `metadata.requiresCopyOnWrite(receiver)` is true. `attachmentPath` is the mutation walk's COW record; a defined value means the old world still retains the receiver. `requiresCopyOnWrite` decides whether the current logical version must be preserved; it does not perform copying.
...
`push`, `pop`, `shift`, and `unshift` use [`array-view.md`](array-view.md) when endpoint sharing is eligible. Otherwise the native algorithm operates on a sparse `arrayMutationRemap` whose existing elements identify their source owner and old key. Writing an origin records its destination `newIndex` as a move; writing a raw argument records an addition; indexed deletion and length writes remain deletion and length operations. Before in-place replay changes the receiver, it captures the value, Promise mirror, import boundary, and cycle cut of every moved origin. Replay then uses the ordinary assignment, deletion, and Array-length mutations in native order, preserving partial-failure behavior. A removed native result is materialized before replay so it retains its original property version. Default and comparator `sort` produce the same remap through native stable `toSorted`. An owned mutation receiver receives the completed shape back under the copying rule above.
...
Array `length` is a visible, non-enumerable, non-configurable virtual property of every logical Array. Assignment is an ordinary mutation and does not use `run`: it removes the final `length` segment, uses the ordinary mutation walker, and applies `transformProperty` to the owning Array property. Setting length follows `ArraySetLength`. Each successfully truncated property detaches its mirror and removes its edge contribution and placement-specific state. Shared, imported, non-extensible, or read-protected Arrays copy before the change. ArrayView rules are in [`array-view.md`](array-view.md).
...
`src/run.js` owns dispatch, observation coordination, mutation policy, result admission, ordinary invocation including String methods, and path walking. Its mutation path composes receiver validation, argument preparation, and Array invocation with `transformProperty`, allowing preparation to overlap receiver readiness. `src/mutations.js` owns `transformProperty`: terminal property readiness, assigned-Promise gates, publication, and independent results. Array length assignment uses the same target helper after its ordinary mutation walk. `src/array-methods.js` owns the method table, specialized argument preparation, and the implementations needed for concat, flat, identity search, text conversion, and sorting. `src/array-invocation.js` consumes that table and owns generic argument preparation, observation and mutation invocation, ArrayView endpoint selection, mutation commit, and native result handling. `src/array-remap.js` owns property-origin remaps, deferred property placement, native-mutation tracing, application, and materialization. `src/property-origin.js` owns origin capture and FIFO sampling of one property state. `src/observations.js` owns path export and native-argument export. `src/resolution.js` owns canonical Promise resolution, operation-result coordination, and Error poisoning. `src/language-coercion.js` owns Promise-aware scalar conversion required by logical wrappers. The surrounding runtime owns argument capture. `src/array-view.js` owns ArrayView representation, backing preflight, endpoint transitions, and bounds. `src/refcounts.js` and property transitions supply logical-edge accounting; `src/meta.js` owns reusable read leases; `src/promise-mirrors.js` owns mirror recreation.

Logical-Array and virtual-`length` access is routed through `src/language-properties.js` for traversal, mutation, import, export, refcounting, Error search, mirrors, and iteration. The observation walker reports final-property presence to `run`, while existing consumers may ignore that additional result. Traversal recognizes virtual String `length` without treating a primitive String as a tracked object; its read-only write and deletion outcome is handled without replacing the String receiver. These adapters may be adopted incrementally while they are identity operations for ordinary Arrays, but attachment must remain disabled until every consumer uses them. `src/index.js` exports `run`; ArrayView and derived-assignment helpers remain internal. Array callback methods other than sort comparison, Map/Set support, and proxy-backed mutating class methods are outside this step.
...
- validation before walking, intrinsic shadowing, ordinary overrides, rejected constructors, String callbacks, deferred Array callbacks, and opaque results;
...
- Cascada scalar coercion of primitives, logical Arrays, supported records and instances, null prototypes, opaque objects, direct and nested Promise frontiers, and Errors, including ignoring protocol-named data properties;
...
- native structural delegation through `arrayMutationRemap`, including duplicate Promise identities, argument omission, Promise mirror recreation, aliases, Errors, and copy-on-write identities;
