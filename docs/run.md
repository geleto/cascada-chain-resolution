...
Array `toLocaleString` is deferred because it invokes executable element methods. A direct or settled method result is admitted before entering the graph: opaque and primitive values pass through, while tracked language-data objects receive normal ownership preparation and remain lazily ref-indexed. Controlled Array and unmodified standard String intrinsic results are runtime-owned; a tracked result from an ordinary trusted object method, Array override, or custom or replaced String method is imported because it may retain external state. A returned method Promise uses the canonical helpers, converts rejection to a language Error, and applies the same admission rule to fulfillment.
