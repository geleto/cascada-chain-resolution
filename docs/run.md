...
The exact host function/method uses `InvocationFailed`; a controlled comparator uses `ControlledCallbackFailed`. Complete input collection suppresses invocation until every required receiver and argument outcome is known. Captured Array intrinsics run as trusted algorithms over runtime remaps after predictable index/length validation; their graph traps keep internal failures fatal and wrap only exact host reflection. Each initial or resumed Array step consumes its exact external escapes before leaving the guarded continuation. Scalar coercion and nested logical Array string conversion use `ScalarConversionFailed`. Required preparation completes every selected input before combining its failures; structural payloads retain their separate value semantics. Unsupported callback outcomes use `InvalidCallbackResult`, and a selected callback Promise retains rejection ownership. A direct method Error poisons a mutation receiver; failure importing an independent result graph leaves valid receiver state available.
...
Retained Array payloads keep their logical identity. This includes Error and Promise values inserted by `push`, `unshift`, `fill`, `splice`, `toSpliced`, or `with`, and values retained by `concat` or `flat`. A controlled structural intrinsic may receive them directly; its wrapper prepares ownership, versions, and bookkeeping.

Call poisoning reaches only consumed inputs. Export consumes every Error reached inside each native-code input. One contextual occurrence is preserved; several are flattened and deduplicated by raw cause, source-context identity, and kind. Error order is unspecified; successful argument positions remain ordered. Any failed input prevents invocation. If every required input is ready, invocation is synchronous. Otherwise common preparation returns one operation Promise.

Array `toLocaleString` is deferred because it invokes executable element methods. Controlled Array results are runtime-owned. Host-call results are imported because external code may retain them; a direct returned Promise preserves fulfillment while a raw rejection is contextualized to the call boundary. Managed result rules are described in [`managed-invocation.md`](managed-invocation.md).
...
Flat and sorting capture placement presence before consuming values, retaining a descriptor failure per candidate and continuing accessible siblings. Sorting also preserves failures reading a captured value until required default conversion or comparator export completes. Candidate discovery respects sparse ArrayView bounds; a failed key-list reflection makes only that interior undiscoverable. Flat depth zero and structural remaps retain payload without inspecting nested contents.
...
Moving a Promise-valued property never moves its Promise version. The destination receives a fresh Promise version at this operation's program position, while the old Promise version detaches or remains with an unchanged source identity. Copying one Promise into several properties creates one Promise version per destination. Structure-only operations do not inspect nested Promises. Sort and text conversion continue only into logical values that scalar conversion actually inspects.
...
`includes` first compares every direct value and consumes each reached possible Promise through the common helper at its logical position. A ready custom thenable therefore contributes its comparison synchronously. Only returned pending branches enter the outstanding set; they are compared as they resolve. The operation settles on the first match and makes remaining registrations cleanup-only, and returns `false` only after every candidate misses. A synchronous match or absence of returned pending branches returns directly.
...
Pending controlled arguments lease an exact runtime-managed receiver until invocation. The common lifecycle extends that receiver lease when a controlled method resumes reading it, such as ordered search, or when delayed `flat`, observation-mode `sort`, or `toSorted` has captured unpublished placements. The common invocation also owns one open/closed operation fact. A final local result closes unfinished Array conversion, search, flattening, export, and remap work; a late continuation in the still-live execution completes required shared Promise settlement and bookkeeping first. If the execution is fatal, the common check returns before settlement, admission, publication, or invocation work.

`run` in mutation mode calls `walkMutationPath` directly. Its terminal callback passes the reached target to `transformProperty`, which captures that property version and invokes the operation callback with the resolved receiver and mutation work. The common invocation lifecycle owns argument preparation. The operation callback contains no parent, key, Promise version, gate, publication, or path-walking logic.

Ready work transforms and publishes synchronously, even when an earlier path segment delayed the walk. If the final receiver or required post-target preparation is pending, `transformProperty` replaces the receiver with an assigned-Promise gate before returning and exposes a separate result Promise. Receiver and argument readiness begin together. A Promise-valued receiver is sampled from its captured live or detached Promise version after earlier FIFO continuations update that version; raw settlement is never consumed.

Once preparation completes, the callback performs one synchronous transition. `transformProperty` publishes the receiver before its independent result, applying the normal assignment, import, Promise version, and copy-on-write rules. If the receiver property was superseded, its detached gate Promise version no longer publishes there, but the result Promise still completes. An assignment-style payload Promise is data rather than readiness. A Promise removed by `pop` or `shift` may delay only the independent method result after the synchronous receiver transition.

Target publication and every enclosing writeback retain the operation result and combine each newly caused publication failure. Failed gate installation still preserves required in-flight result work. A successfully installed gate's completion subscribes at issuance, after shared settlement and before later operations, and reads that captured version's published logical outcome. This captures publication failure without including changes caused by later work. Receiver protection ends with publication; independent-result collection can continue after healthy replacement of failed state.

The common continuation helper performs each required ordinary `.then` subscription at its program position. The native Promise or supported custom thenable owns FIFO delivery across settlement and chaining; a later ready subscription cannot overtake an earlier undelivered callback. A ready custom callback returns its normalized transition result directly, while a pending subscription returns its chain. A transition consumes any further possible thenable before returning, so only an actually pending chain remains in that result position. That result identifies pending work for aggregate waits; pending-only protection also follows the selected receiver and unfinished publication or capture dependency. An independent result cannot extend completed receiver publication or path selection. Callback execution and receiver, remap, result-slot, or Promise version backwrites are not readiness flags. Cascada keeps no canonical Promise or parallel subscriber queue. `async`/`await`, `queueMicrotask`, or a proxy Promise must not split an ordering-sensitive transition or add a microtask to already-ready custom work. Returning an independent result Promise does not replace the source's settlement mechanism.
...
An unpublished working Array may be completed and indexed once before publication or return. Controlled structural methods and length assignment publish such a candidate while preserving their baseline. Direct changes to an existing ref-indexed logical Array use ordinary placement publication without a before/after reconciler. An entering edge is prepared and indexed before its contribution is added. A leaving edge removes its reverse-parent multiplicity and Promise/Error/cycle-cut totals. A replacement applies only the old-to-new edge delta.
...
Holes have no contribution. Placement-specific state, including cycle cuts and Promise versions, is cleared, retained, or recreated for the corresponding logical property even when aggregate counts do not change. ArrayView endpoint contraction removes the changing identity's logical edge even when another view retains the physical element. Storage and bookkeeping steps form one synchronous method-specific transition.

## `length`

Array `length` is a visible, non-enumerable, non-configurable intrinsic property of every logical Array. The mutation walk classifies a terminal property before treating it as a graph edge: ordinary properties target that edge, while intrinsic `length` targets the receiver's enclosing placement. `transformProperty` gates that placement when conversion or conflicting transitions remain pending. The payload is converted only after the receiver is known to be a logical Array; another object's `length` remains an ordinary property and may hold a Promise. Assignment uses the ordinary protected scope baseline and publishes a new Array of the requested length with only its retained placements. It waits for transitions in the truncated suffix without consuming their ordinary payloads or delaying retained-prefix entries. [ArrayView](array-view.md#materialization-and-length) describes representation and backing reuse.
...
`src/run.js` owns path routing and top-level receiver classification. `src/invocation.js` owns common preparation, protection, invocation, result admission, and lease cleanup, together with ordinary host-call selection and native calls. Managed invocation selects its member after complete preparation and before isolation, returning the required mutation outcome when selection fails. `src/operation-lifecycle.js` owns operation closure and release registration for pending nested resources; `src/internal-step.js` owns immediate and deferred guarded operation work, value consumption, complete input collection, and clean-input preparation. `src/export.js` owns `exportValue`, `exportManyValues`, copying, Error collection, and output lifetime. `src/mutations.js` owns path copying, receiver replacement, Promise gates, `transformProperty`, and Array length assignment including truncated-suffix transition waits; `src/language-properties.js` owns terminal-property classification, descriptors, validation, and physical language-property access. `src/property-versions.js` owns exact property versions, placement, publication, and validated deletion. `src/array-methods.js` declares and implements supported Array methods, while `src/array-invocation.js` owns Array call selection and interprets those declarations. `src/array-remap.js` owns property-placement remaps, transfer, and materialization. `src/observations.js` owns observation-path behavior. `src/error.js` owns Error conversion and combination. `src/language-conversion.js` owns Promise-aware scalar conversion required by logical wrappers. `src/array-view.js` owns ArrayView representation, backing preflight, endpoint transitions, and bounds. `src/refcounts.js` owns logical-edge accounting. `src/data-declarations.js` owns data declarations, `src/import-processing.js` owns the inbound admission walk, and `src/meta.js` owns identity facts and read leases.

Logical-Array and intrinsic-`length` access is routed through `src/language-properties.js` for every graph operation. Traversal recognizes String `length` without treating a primitive String as a traversable object. `src/index.js` exports `run`; ArrayView and derived-assignment helpers remain internal. Array callback methods other than sort comparison, Map/Set support, and proxy-backed mutating class methods are outside this step.
...
- native structural delegation through property-placement remaps, including lazy in-place reads, duplicate Promise identities, argument omission, Promise version recreation, aliases, Errors, and copy-on-write identities;
...
- Array and String length reads, growth, truncation, invalid values, restricted representations, Promise version detachment, attached views, and logical iteration; and
