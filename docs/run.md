...
Call poisoning reaches only consumed inputs. Export consumes every Error reached inside each native-code input. One contextual occurrence is preserved; several are flattened and deduplicated by exact leaf identity only. Different occurrence wrappers remain distinct even when they share one cause, and failed roots retain semantic argument order. Any failed input prevents invocation. If every required input is ready, invocation is synchronous. Otherwise common preparation returns one operation Promise.
