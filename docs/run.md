...
`method` must be a string and `mutation` must be exactly `true` or `false`. Both are supplied before the receiver or any argument settles. A non-string method or invalid Boolean returns a language Error before path traversal. After the receiver is classified, internal dispatch rejects `constructor`, an unsupported controlled method, or an unsupported mode without preparing arguments. An observational rejection leaves the receiver unchanged; a mutation rejection publishes the Error through the normal receiver transition.
