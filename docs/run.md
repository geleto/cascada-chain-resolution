...
Ordinary lookup preserves JavaScript shadowing. An own enumerable data property shadows the prototype but is never an executable method. A record observation may use its supported host surface, including a getter, and remains trusted read-only and non-retaining. Managed-class dispatch selects a data method from the prototype captured at admission and its inherited chain up to, but excluding, `Object.prototype`; declaration rejects accessors on that chain.

After managed-class method selection succeeds, the call resolves the complete receiver graph and every explicit argument through captured property versions. An observation leases its prepared receiver without a gate. A mutation uses the ordinary receiver gate while pending, leases the complete argument graph through finalization, selectively isolates receiver identities protected by COW or runtime bookkeeping, and publishes only a valid completed receiver. Managed-class methods execute synchronously and follow the trusted class contract in [`registered-class-invocation.md`](registered-class-invocation.md).
...
Comparator readiness precedes element collection. With fewer than two sortable values, neither default-key conversion nor comparator export and invocation occurs. Otherwise native stable sort permutes internal origin records, while the published result retains the original logical values. Comparator snapshot export consumes every Error it reaches before host code runs.
...
The search value for `includes`, `indexOf`, or `lastIndexOf` is not exported, but it is a consumed input: a direct Error or top-level rejection poisons the call. Other values compare only by identity or primitive value. A `concat` item is likewise consumed only to classify it as a logical Array or scalar, so an Error or rejection poisons. Each logical Array item is immediately captured as a sparse property-origin remap and its root remains leased until publication, so delayed siblings cannot expose later mutation through captured origins. Successfully classified non-Array items are retained exactly. `Symbol.isConcatSpreadable` is ignored.
...
`push`, `pop`, and `shift` use [`array-view.md`](array-view.md) when endpoint sharing is eligible. `unshift` uses the remap path when its receiver must be preserved. Each unchanged Promise property in a derived view receives a mirror forked at the derivation's FIFO position even though the views share its physical backing slot. Otherwise a captured intrinsic plans a sparse property-origin remap. Copying operations initialize present origins because the result retains them all; in-place mutation obtains an origin only when the intrinsic reads that index. Writing an origin records a move, writing a raw argument records an addition, and indexed deletion and length writes remain explicit operations. `concat` combines captured remaps and internally wrapped retained items. Default and comparator sort share one origin-record pipeline.
...
- syntactic validation before walking, category validation after classification, controlled Array dispatch, rejected constructors, String protocols and callbacks, deferred Array callbacks, and external results;
