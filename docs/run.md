...
Call poisoning reaches only consumed inputs. Export consumes every Error reached inside each native-code input. One contextual occurrence is preserved; several are flattened and deduplicated only when their causes have identity, while equal primitive causes remain distinct and failed roots retain argument order. Any failed input prevents invocation. If every required input is ready, invocation is synchronous. Otherwise common preparation returns one operation Promise.
