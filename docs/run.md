...
Managed-record and managed-class member lookup happens only after their required inputs are clean. A record method is an own enumerable Function-valued placement; inherited properties, accessors, and non-enumerables are unavailable. Managed-class lookup selects a data method once from the prepared receiver's admitted prototype chain up to, but excluding, `Object.prototype`; declaration rejects accessors and callable `then` methods on that chain. If lookup later safely detects an accessor, callable `then`, or another invalid prototype change before invocation or publication, the call returns `InvalidManagedReceiver` and preserves the original receiver; only already-compromised runtime state is fatal. Failed preparation performs none of this application-controlled reflection.
