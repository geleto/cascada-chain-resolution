...
Logical-Array and virtual-`length` access is routed through `src/language-properties.js` for traversal, mutation, import, export, refcounting, Error search, mirrors, and iteration. The observation walker reports final-property presence to `run`, while existing consumers may ignore that additional result. Traversal recognizes virtual String `length` without treating a primitive String as a tracked object; its read-only write and deletion outcome is handled without replacing the String receiver. `src/index.js` exports `run`; ArrayView and derived-assignment helpers remain internal. Array callback methods other than sort comparison, Map/Set support, and proxy-backed mutating class methods are outside this step.
