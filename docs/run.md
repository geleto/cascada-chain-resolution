...
`push`, `pop`, `shift`, and `unshift` use [`array-view.md`](array-view.md) when endpoint sharing is eligible. Otherwise the native algorithm operates on a sparse `arrayMutationRemap` whose existing elements are branded property captures. Writing one records the captured property and destination `newIndex` as a remap operation. Writing a raw argument records an assignment; indexed deletion and length writes remain deletion and length operations. Replay places captured properties and uses the ordinary assignment, deletion, and Array-length mutations. Operations retain native order and partial-failure behavior. Default and comparator `sort` produce the same remap through native stable `toSorted`. An owned mutation receiver receives the completed shape back under the copying rule above.
