...
Lowering captures every argument position before issuing `run`, preserves omission and arity, and passes the captured logical values unchanged. After dispatch, `run` exports only arguments consumed by native code. One batch export preserves aliases across argument positions and produces a ready host snapshot or one Promise for it; inputs need not originate from a Chain. An Error anywhere in an exported argument fails that argument, all failed arguments are combined in order, and host code is not called. A sort comparator occupies a separate executable slot and is passed directly or as a control Promise without import, export, result admission, or storage in the Chain.
...
- syntactic validation before walking, category validation after classification, intrinsic shadowing, ordinary overrides, rejected constructors, String protocols and callbacks, deferred Array callbacks, and external results;
