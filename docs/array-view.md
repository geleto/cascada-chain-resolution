...
**Planned and internal.** `ArrayView` is the optional endpoint representation used by [`run`](run.md). It is not a package API and does not add Map or Set support.
...
The internal indexed helpers are:
...
They are module-level or symbol-keyed runtime helpers, not string-named prototype methods or executable methods exposed by `run`. Indexed access follows Array semantics through safe language-property transitions; `has` preserves the distinction between a hole and stored `undefined`. Other keys use the logical non-index property adapters.
...
Array-producing methods and materialization return native Arrays. Array species and subclass preservation are outside the language data model. `run` owns marked publication and unmarked transformed-Array results.

The view is directly iterable. Its internal `[Symbol.iterator]`, `values`, `keys`, and `entries` adapters read logical length on every step; value iteration yields `get(index)` from zero upward, including `undefined` for holes. Cascada iteration and spread resolve a native Array's attached view before selecting this path, so physical indexes outside the logical range never escape. These stateful iterators are consumed by lowering and are not language-data results returned by `run`.

## Implementation boundary

`src/array-view.js` owns the brand, internal helpers, coordinate validation, attachment, materialization, endpoint transitions, length handling, and iteration. It exports only internal logical-Array adapters. `src/run.js` owns invocation mode and results; no ArrayView helper is callable through ordinary method dispatch or exported by `src/index.js`.
...
- non-enumerable fields, private Symbol branding and base index, valid and invalid coordinates, repeated base-index updates, and attached versus standalone identity;
...
- direct iteration, live length, holes, attached native iteration and spread, standard method results, deferred callback rejection, Promise-aware sort comparison, and receiver identity.
