...
Phase 9C's fatal-execution work and supported-thenable addendum are complete. Phase 9D-0 is the next implementation step: close the recovery and successful-result gaps before changing recoverable boundary handling in Phase 9D-A.

## Current checkpoint and agreed changes

Phase 9C fatal execution and the supported-thenable addendum are implemented. The addendum removes `Execution._thenables`, callable capture, canonical Promises, declaration probes, the separate result observer, and refcount mirror-installer callbacks. `thenValue` performs ordinary subscriptions; normalized returned results determine pending work. Final Error representation/kinds remain Phase 9D-A/9D-B work, and external entries remain inert until Phase 9E.
...
| Concern | Phase |
| --- | --- |
| Execution fatal authority, per-execution reporting, centralized stop checks, bounded pending-operation-result delivery, and the sync-first operation-result contract | 9C |
...
- Public entry, guarded continuations, host-boundary exit, and scheduler dispatch observe the same nullable `fatalError` value; operation return needs no redundant final check after a core operation returns.
...
**Implemented.** This addendum replaces the transitional thenable implementation installed by earlier phases without changing Phase 9C fatal semantics. Validation: 918 tests pass with strict unhandled-rejection behavior, including the refcount consistency oracle and focused custom-thenable tests.

### Problem

The pre-addendum implementation cached a callable `then` per execution, invoked a source once, created a canonical native Promise, recorded derived Promises back into the same map, and used that producer record to distinguish ready from pending operation results. This duplicates settlement and subscriber behavior already owned by a well-behaved Promise-like value. It also turns Cascada-style synchronous readiness into native-Promise microtask readiness, adds per-execution identity state, and creates machinery for hostile thenables whose behavior is outside any useful data contract.
...
- require a synchronous callback throw to escape the `then` call and a callback that throws after pending delivery to reject the returned chain. A host implementation unable to provide this sync-first chain behavior must expose a native Promise instead;
...
- When an earlier pending subscription has settled but its callback has not yet been delivered, a later subscription cannot run synchronously ahead of it.
