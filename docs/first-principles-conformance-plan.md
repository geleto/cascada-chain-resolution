...
Phase 9C, its supported-thenable addendum and completion step, and Phases 9D-0 and 9D-A are implemented. Phase 9D-B is next: install rejecting poison transport and finish its publication/collection cutover on the causal Error implementation.

## Current checkpoint and agreed changes

The current kernel has ordinary FIFO subscriptions, atomic import and index publication, successful-output native-then validation, immutable causal Error wrappers, complete semantic Error collection, one local close transition, and a trusted integration surface. Poison transport remains non-thenable until Phase 9D-B; external entries remain inert until Phase 9E.
...
| Exact causes contain diagnostic data without protected managed or external identities | 9D-A contracts and kernel producer audit; 9F external capabilities and 12 host integration |
| Native assimilation preserves successful ready/pending equivalence | 9D-0 native-then validation and exact-value host restriction; preserved by 9D-B/9F/12 |
| One shared host-action and guarded-continuation composition surface | 9D-A integration exports and examples; 12 calls, callbacks, iterators, loading, and standalone-call receivers |
...
| Fatal observation permits only already-owned local iterator finalization | 12 finalization, its failure/Promise ownership, and scheduler abort removal |
| Collection identifies semantic cause/source/kind classes | 9D-A factories and queries, preserved by 9D-B and 12 compiler source reuse |
| Diagnostic routes and safe inspection retain information with a successful formatting fallback | 12 immutable route capture, diagnostic views, and fallback verification |
...
| Immutable native poison hierarchy, precise brands, causal kinds, diagnostic-only causes, and complete semantic collection | Implemented by 9D-A; cause contract integrated in 9F/12 |
| Causal continuations, private exact-action host escapes, unified local closure, and separated declaration/missing-operation-context handling | Implemented by 9D-A |
...
Causal source and kind live in the first required boundary continuation. The host-action producer preserves the post-host fatal check, and the common owner transition preserves live-execution releases.
...
- All four deliverables pass. Phase 9D-A is next; the recoverable and higher-runtime work above remains required.
...
- Capture exact placement versions. Later synchronous subscriptions may advance an earlier captured mirror; discover that newly available graph before counting, repeating only while available work advances the captured frontier. Reuse ordinary normalization and shared property publication; those are valid independent transitions and are not rolled back. Do not reread physical slots, resubscribe, add callback-ran flags, or defer a ready commit to a microtask.
...
- The same file exercises ready and deferred query-triggered index abandonment, captured mirrors advanced during later synchronous subscription with and without an existing index, and an adjacent internal parent-DAG corruption that remains fatal. Query-reflection tests retain the current fatal outcome until 9D-A changes that boundary; index publication is already safe in both cases.
...
**Status: Implemented.** The causal and lifecycle contracts below are in place. All **1,088 tests** pass under strict unhandled-rejection handling, including native-equivalence, refcount-oracle, causal-attribution, query-recovery, integration-route, ready/resumed Array-preparation, complete descriptor-failure collection, mutation-completion Error combination, and rejected-selection receiver-preservation coverage. Poison remains non-thenable at this checkpoint: pending export may fulfill with poison, while query failure uses its required local rejection adapter. Existing export tests identify this transitional transport; Phase 9D-B changes them to rejection assertions while retaining complete Error membership and attribution. Rejecting transport and final poison-prototype freezing belong to that atomic cutover.

Implementation ownership: `error.js` supplies branded factories and the single host-escape mechanism; `operation-lifecycle.js` supplies local closure and `continueOperation`; `language-values.js` supplies initial causal admission; `input-collection.js` supplies complete value collection and clean-input preparation through one collection engine. Array semantic steps consume exact host escapes inside their guarded continuations; query outcomes settle through one finish transition. Property versions and staged import retain source attribution without sideband origin state. `export.js` uses one visited set and Error accumulator for the whole batch. The package integration subpath and [integration tests](../test/causal-failures.test.js) reuse those implementations.
...
Use one private identity-branded host-escape marker, created only around an exact supported host action by `runHostAction`. That producer owns the re-entry guard and the post-host fatal checkpoint. `catchExternalThrow` at the causal operation consumes only this marker and supplies the fixed operation context and kind. It performs contextualization and the operation-specific failure effect outside its catch; adjacent unmarked runtime failures remain fatal. The marker is neither language data, an Error kind, nor an integration export.
...
Invalid host output is recoverable when the boundary can reject it without compromising runtime invariants, such as an unsupported callback result or invalid completed managed receiver. In particular, if managed-class member selection detects that an admitted prototype now contains an accessor before invoking it or publishing state, return `InvalidManagedReceiver` and preserve the original receiver; the forbidden prototype change is not by itself fatal while the runtime can reject it safely. Managed invocation owns selection after complete preparation and before isolation, and carries this rejection through the existing `{ mutatedValue: receiver, result: failure }` mutation outcome. No common member-selection hook, new result wrapper, or change to other invalid-call publication rules is needed. Host behavior is fatal when it has already made runtime state, ownership, ordering, publication, or cleanup untrustworthy.

Make query-reflection failure an explicit query-operation outcome. Classify each exact effectful reflection failure as `QueryReflectionFailed` through the selected host-boundary mechanism; never recover an arbitrary traversal exception. Bubble poison through the query body, close its local owner, and discard any partially collected query-only set; do not call `found`, convert it to Boolean, append it to `getErrors`, or throw it through the fatal-on-escape envelope. Phase 9D-0 is a prerequisite: discarding a query-local collection does not repair invalid persistent indexes. Early proof, failed reflection, and exhausted traversal all settle one query outcome through its finish transition. Own the readiness continuation without racing it against the outcome it already settles. A ready query returns poison directly. While poison remains non-thenable in 9D-A, the existing language-outcome Promise reaction deliberately rejects the direct query Promise with that poison outside the fatal-on-escape catch; after 9D-B, returning the thenable poison has the same rejection result and the transitional rejection adapter is removed. Update the ready/deferred query-reflection cases in test/index-recovery.test.js to expect QueryReflectionFailed, then query and mutate the retained aliases after disabling the trap and run the refcount oracle in the still-live execution. Add no query-result wrapper or general result algebra.
...
Do not add a forwarding Promise used only for attribution, Promise subclass, overridden chaining method, parallel continuation system, or runtime-wide Promise brand. Contextualize in the first import, mirror, validation, or publication continuation the boundary already needs. A direct result flows through the Promise returned by that continuation; a placement continuation uses FIFO readiness and reads the logical value published by the earlier resolver instead of interpreting the raw payload again.
...
Keep settled semantic rules in the architecture and principles, and outstanding code-removal and implementation tasks in this plan. Document the private exact-action marker rule and shared-helper rationale. Phase 12 owns the local iterator-finalization and diagnostic route/view requirements.
...
- Every Error form precedes thenability inspection, including hostile Error values with throwing `then` properties. Precise predicates distinguish raw native Error, poison, and fatal state, and native `Error.isError` passes the package's Node 24 and cross-realm tests; Phase 12 verifies the containing Cascada runtime's browser matrix.
...
- At each justified operation-context entry check, a missing operation context or execution binding causes `FatalError` before graph/host access even when the requested work would otherwise succeed or immediately return. Exercise omitted/null context and missing/null execution, including a source-only context; do not add an arbitrary malformed-shape matrix. Cover Chain initialization and the common semantic integration entry without duplicating tests for every internal helper. A valid enclosing execution receives its authoritative fatal outcome; a root entry with no execution throws the branded missing-operation-context fatal without manufacturing state or reporting globally. Passing that fatal into another execution uses ordinary fatal submission. Missing optional source-location fields on a valid operation context remain nonfatal.
