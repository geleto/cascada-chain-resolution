...
Phase 9C's fatal-execution work and supported-thenable addendum are complete. Phase 9D-A is the next implementation step.
...
| Bounded Error-output/detection nondeterminism; complete required membership and unchanged successful effect order | Existing 9C fatal semantics; 9D-A collection/query tests; 9E/9F invalid-binding tests |
...
| Replace Cascada's legacy poison wrapper, thrown-poison interception, duplicated fatal routing, and source sorting with the kernel contracts | Phase 9D-B kernel surface; Phase 12 Cascada integration |
...
- A hidden pending Array-length continuation is observed and remains active after `assignPath` returns. It normally closes after publication; if it detects fatal failure it closes its local owner, and if an unrelated fatal occurs behind a never-settling blocker it may remain pending without delaying any operation result.
...
- `error.js` changes the transitional operation fatal entry to `runFatal(operationContext, work)`, and every operation-bound caller passes its registering operation context. Keep current report-and-rethrow behavior until Phase 9C replaces it with execution-owned `failExecution` and fatal-on-escape transition helpers. State declarations temporarily use a separate contextless fatal entry; do not make the operation context optional. Phase 9C gives that transitional path an explicit source sentinel and synchronous throw behavior without an execution reporter. Phase 9D-A removes it when declaration probes receive their final local host-validation handling.
...
Phase 9C retains `runContextlessFatal(work)` only as transitional support for state declarations and the defensive missing-context branch in `runInternalStep`. It uses an explicit contextless-source sentinel, converts an unexpected defect to `FatalError`, and throws synchronously without reporting or closing an execution. Do not generalize it with a reporter, mutable global routing, or report state. Phase 9D-A removes both uses: internal steps always receive an execution by trusted construction, while contextless declaration validation uses ordinary host API Errors and unexpected implementation exceptions escape synchronously.

Remove `reportFatalError`, mutable module-global reporter routing, `FatalError` report state and methods, and every competing reporter or fatal-state path. Retain `CascadaError` only as the temporary poison base and public compatibility export until Phase 9D-A removes both together. Apart from expected-poison/fatal-on-escape separation required for sound fatal handling, do not change poison representation, kinds, contextualization, or boundary recovery in this phase.
...
Phase 9B carries the final operation context to every boundary, Phase 9C provides one authoritative fatal lane, and the 9C addendum installs the final supported-thenable mechanism. Recoverable Error attribution and boundary handling still use the transitional model.
...
The Error membership and unspecified-order contract in this phase supersedes historical cause-only deduplication and receiver/root Error-order wording in earlier completed phases. Preserve ordering of successful values and effects.
...
Name the trusted causal factory `createPoisonError(reason, operationContext, kind)`: it submits an existing `FatalError`, preserves existing poison, and otherwise creates one branded occurrence. `combineErrors` remains the trusted compound factory. Phase 12 re-exports both through `./integration`; the host-facing root exports only recognition/data surfaces. This is package-composition trust, not a security boundary, so add no caller capability or duplicate higher-runtime constructor.
...
4. Deduplicate by raw cause, source-context identity, and kind using collection-local state. Share this rule with `getErrors`. Different contexts or kinds remain distinct. A leaf without a cause uses its own identity for that component; compare primitive causes by ordinary Map equality without coercion.
...
Replace `runUserCode`, `UserCodeFailure`, `catchUserCodeFailure`, and `catchRawUserCodeFailure` with one private `runHostBoundary(operationContext, kind, action)`. Its `try` contains only the exact synchronous host action. On either return or throw, it first checks `operationContext.execution.fatalError`; if attempted re-entry or reporter work already closed that execution, it throws the authoritative Error without inspecting the host outcome. Otherwise a thrown reason or ready Error result is handled outside the action's `try`: an existing `FatalError` is submitted and the authoritative fatal outcome is thrown, existing poison is returned unchanged, and another reason becomes a new poison occurrence. The causal caller therefore sees only success or poison and applies that boundary's graph or result effect outside the catch. A failure in contextualization or failure application escapes to the surrounding fatal-on-escape transition and is fatal. The helper has no callback, recoverable/fatal flag, or preparation, export, import, publication, bookkeeping, or cleanup behavior. It owns the post-host check; do not add a second helper or caller check. A lower-level effectful primitive does not catch; its causal caller wraps only the primitive invocation. Application callbacks and effectful host reflection use this boundary. A captured built-in is runtime-owned only on runtime-owned, hook-free inputs; applying it where a Proxy or host trap may run is supported host code. “Native code” alone is not a recoverable category. Preserve the re-entry guard: permitting nested work would require an ordering barrier before otherwise-ready host mutations and would undermine the sync-first contract.
...
Remove `runContextlessFatal`, `CONTEXTLESS_ERROR_CONTEXT`, and the missing-context branch in `runInternalStep` in this phase. Every internal step receives an operation context that identifies its execution by trusted construction. Absence or malformed shape is outside the kernel protocol rather than a second fatal path. Every authentic `FatalError` is therefore created by `failExecution`; tests that need an existing fatal first fail an explicit execution and then pass that Error to the receiving boundary. Remove tests whose only purpose is contextless fatal construction.
...
Make query-reflection failure an explicit query-operation outcome. Wrap each exact effectful reflection action with `runHostBoundary(..., QueryReflectionFailed, action)` rather than catching the whole traversal. Bubble the returned poison through the query body, close its local owner, and discard any partially collected query-only set; do not call `found`, convert it to Boolean, append it to `getErrors`, or throw it through the fatal-on-escape envelope. A ready query returns it directly. While poison remains non-thenable in 9D-A, the existing language-outcome Promise reaction deliberately rejects the direct query Promise with that poison outside the fatal-on-escape catch; after 9D-B, returning the thenable poison has the same rejection result and the transitional rejection adapter is removed. Add no query-result wrapper or general result algebra.
...
- `PoisonError`, `CompoundPoisonError`, and `FatalError` now have the final direct native-Error hierarchy, with `FatalError` retained from Phase 9C and the transitional runtime `CascadaError` removed exactly once here. Poison remains non-thenable until Phase 9D-B.
...
- Declaration APIs preserve existing Errors and use direct contextless thenability recognition without cached state. Contextless configuration failure uses the explicit sentinel.
...
- Do not cancel or specially settle native Promises, gates, phases, or synchronous host code. A never-resumed internal wait may remain pending without delaying any pending operation result.
...
- Add one explicit diagnostic formatter adapter that accepts immutable kernel Error facts, optional async diagnostic routes, and exact causes. It produces the non-thenable plain view consumed by `peekError`/`#`, including recursively safe child and cause summaries, bounded compound presentation, source-formatted messages, and diagnostic stacks outside graph transitions under protective catches.
...
- Cascada starts no command or host effect after closure, does not cancel or specially settle source Promises, gates, or phases, and ignores late operation work at common continuation checkpoints. An internal wait may remain pending without delaying any operation result.
