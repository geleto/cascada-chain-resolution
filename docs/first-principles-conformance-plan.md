...
Phases 9C through 9E-A supply the implemented graph/Error, admission, and binding foundations. Phase 9F-A is the next pending completion: automatic managed mutation rollback, owning-placement scope poison, hierarchical external reservations, external subtree repair, and mixed-entry ownership. Existing managed and external public routes require this cutover. Phase 10 follows with Promise-valued path segments; Phase 13 integrates Cascada.

## Current checkpoint and agreed changes

The current kernel has ordinary FIFO subscriptions, atomic import and index publication, successful-output native-then validation, immutable causal Error wrappers, complete semantic Error collection, one local close transition, and the public higher-runtime API. Poison remains ordinary non-thenable Error data. Expression lookup returns a ready PoisonedValue or a directly rejecting pending Promise; getErrors returns null or a combined Error, and query failures fulfill with ordinary Error data. Context import validates unique external locations and commits execution-local bindings. Phase 9F provides the public boundary baseline. Phase 9F-A adds managed rollback and replaces scope coordination and poison storage; its source-file replacement map below defines the work rather than treating existing tests as proof of the new design.
...
- **External boundary/location:** a registered external scope on a static context route, including nested native scopes. Its Symbol-held record identifies its canonical context/path and binding; first native crossing and selected mutation scope are separate facts.
...
The atomic admission, binding, compiler-input, and public-context foundation is implemented. Hierarchical scope discovery and reservation replacement are pending in Phase 9F-A. Keep this phase's transactional and identity invariants; the end-state tree and scheduling mechanisms are specified by Phase 9F-A and external-context-ordering.md.
...
**Status: not implemented; next implementation phase after the 9F baseline.** Deliver hierarchical external reservations, automatic rollback of each failed managed mutation, owning-placement scope poison, and retained-state repair together. Existing public routes, snapshots, and borrowed-result handling provide implementation material but do not implement this end state. Complete this phase before Phase 10 and Phase 13 integration. Use existing architecture documents; add no separate replacement architecture or permanent audit document.

### Replacement strategy and source boundaries

This is a replacement phase, not an additive compatibility layer. Start from the final invariants and rewrite the coordination modules whose state model changes. A full rewrite of those modules is preferred to retaining branches that translate between old and new state. For larger mixed-purpose modules, remove the affected section and reimplement it while keeping independent algorithms intact. No wholesale source revert is required: it would discard useful boundary and ownership fixes.

| Current source area | Required treatment |
| --- | --- |
| external-mutation-tree.js | Rewrite nested discovery, node metadata, and scope-selection queries. Preserve atomic identity proposals, duplicate rejection, static-source filtering, and compiler-input ownership. |
| external-operation.js | Replace ReadGroup, reservePhase, exclusive/read cursor state, and poison-carrying completion. Use one hierarchical completion kernel. Retain supported binding/capability validation at its natural boundary; no wrapper preserving the old phase API solely for callers. |
| path-context.js | Rewrite contextual routing around canonical nodes and covered entry views. Delete scopeRecord recovery/gate maps, contextGuards/descendantGates scans, dependsOnOwnedGate traversal, and entryLocation's search for an owned binding gate. Preserve path provenance and public entry-selection behavior required by the final contract. |
| path-operation.js | Centralize mutation selection and publication while keeping requested authority, receiver/container, and structural rollback owner distinct. Rewrite routing from these facts and inherited reservation coverage. Remove successful managed-bang/native-effect composition, mutation-only gate registration hooks, and duplicate guard/phase state. Keep OperationOwner and result-boundary responsibilities. |
| enter.js | Rewrite mixed/external reservation setup and release. Retain path COW and callback closure; transfer complete placement presence/version/recovery state through the private root and latest-root publication. Reuse structural-owner protection for out-of-range Array entry. Remove external contextual-binding gates and owned-gate maps. |
| observations.js | Replace external metadata-query composition and owned/dependent-gate bypass branches. Keep ordinary managed path capture, expression validation, and graph-query pruning. Parent metadata queries use one covering reservation, not child reservations reacquired behind their own waiter. |
| mutations.js | Make managed mutation preserve its ordered baseline, mutate private state, and publish success or scope poison retaining that baseline. Replace lengthExternalMutationBranch and its extra walk with common structural-owner selection. Evaluate removing observeTarget through shared failure publication, not a duplicate failure walker. Remove namespace-only recovery side writes and target.onPublish if it only served the deleted gate index. Preserve ordinary gates, Array structure, source/result Error combination, and nonblocking publication. |
| managed-invocation.js, array-invocation.js | Route native and controlled mutation failure through common rollback publication. Native receivers must be completely isolated from the baseline; replace selective mutation-isolation discovery/reconnection when the protected root makes it redundant, reusing the complete-graph copier. Preserve observation materialization, validation, and independent result import. |
| array-remap.js, array-methods.js | Evaluate removal of sourceSurvives branches, requiresCopy preflight, and in-place ArrayMutation.apply replay made redundant by baseline protection. Retain sparse remap production, captured placements, efficient views, and any replay still needed solely on independent working storage. Remove obsolete helpers and callers together; do not replace them with an adapter. |
| property-versions.js, language-properties.js, meta.js, refcounts.js | Extend ordinary placement versions with presence, visible poison, and private retained baseline/version. Preserve absent completion through captures, transfer, logical enumeration, normal settlement, ArrayView placements, and visible-edge indexing; no public absence sentinel. No separate guard class/transport, identity poison flag, Error-owned data, or second graph index. |
| external-access.js, external-snapshot.js, import-preparation.js, import.js, export.js | Reuse native boundary conversion, snapshots, complete collection, and borrowed-result isolation. Update selected-scope/binding plumbing for hierarchy and preserve completed placement presence in managed traversals; do not rewrite these algorithms merely to reduce file count or unify different readiness contracts. |
| run.js, index.js, chain.js | Replace affected integration points directly. Remove forbidden mixed mutation routes and repeated mutation-outcome reconstruction; evaluate one managed traversal retaining the selected scope through receiver selection. Remove obsolete namespace/phase options when their implementation is gone. Preserve public signatures unless an explicit integration requirement needs a change; no compatibility overloads or duplicate public fatal boundaries. |

Introduce no new module merely to wrap the deleted interface. A small dedicated reservation module is appropriate if it owns the complete common algorithm; choose its final boundaries during implementation and early integration. Entry views and ordinary reservations must call the same implementation, not two implementations sharing terminology.

Implement in dependency order: replace tree registration and selection with the common reservation kernel; add placement poison, managed baseline protection/rollback, and repair; cut over routing and entry; migrate queries and remaining public consumers; delete obsolete code. Build ordering and placement recovery directly as the first foundational pieces, integrating representative operations and difficult entry/publication cases early to verify their representations before spreading them across callers. No separate preliminary phase or code-size prediction exercise is required. Evaluate total state, execution paths, cleanup obligations, and runtime costs as well as code size; reconsider a representation if integration introduces unnecessary machinery. Any justified experiment code must be removed before completing the phase. Do not maintain two production algorithms as an incremental migration strategy.

Perform mutation-route and outcome simplification inside this replacement, where final placement and rollback ownership are known. Do not first normalize the old implementation in a separate cleanup phase or build an adapter accepting both old and new mutation contracts. Evaluate traversal candidates together across path-operation.js, mutations.js, and run.js; moving the same publication logic into another walker is not a reduction.

The common mutation coordinator, complete isolation of native managed mutation receivers, and shared logical placement transfer are required replacements. Compare optional preparation/copy fusion and attachment-protection changes only after their owning mechanisms use the final representation. Evaluate each candidate within this phase against the simpler correct implementation: keep it only if it reduces independent rules, execution paths, or maintained state without weakening semantics or imposing copying, traversal, or retention costs that outweigh the benefit. Revert unsuccessful candidates and delete experiment code; evaluating a candidate is required, adopting it is not. These comparisons need no separate phase or permanent audit artifact.
...
- Internal COW, Array placement movement, and managed entry reuse common transfer of placement presence, source version, and recovery state. A no-op creates no property or Array element; structural gates prevent placeholder length changes, and presence-sensitive traversal/callbacks distinguish holes from explicit undefined.
...
- Mutation selection and publication have one coordinator. Remove the separate length route and post-selection scopeDepth edits, repeated selected-outcome discrimination/reconstruction, and every Array branch proved unreachable under the chosen baseline/working-storage representation. Keep operation-specific fixed-location checks and ordinary preselection Error handling.
- No reader counter, old phase cursor, poison-completion payload, contextual-binding gate, recovery side store, or dependent-gate bypass remains active or exported. Search for ReadGroup, reservePhase, ownedGates, dependsOnOwnedGate, registerScopeGate, record.recovery, and their structural equivalents; renaming them is not removal.
...
Extend compiler-guided initial discovery to requested native child data placements below an external node. A runtime node can contain both child keys and a Symbol-held external scope record. Keep canonical path/context, binding, reservation, and poison facts inside internal metadata rather than reserving property names. Continue through the finite compiler paths only; do not enumerate native interiors or add an endpoint-used flag.
...
Distinguish first native crossing from selected external scope. Explicit external bang selects its node; default operations select the deepest registered scope covering their receiver/container, and unregistered suffixes remain under that scope. Registered descendants require static source provenance even below an external parent. Dynamic ordinary suffix keys remain supported but cannot select registered nodes. Root and entered routes use the same location facts. Unexpected replacement of a fixed identity remains fatal; supported invalid selection stays poison.

Update tree queries for exact registered entry targets, deepest covering scopes, mixed namespace branches, and native crossing without adapters for leaf-only records. Update compiler integration and public examples. No source operation needs to scan an unrelated tree branch just to select its scope.
...
Capture explicit inputs, source provenance, the issuing entry view, and one command effect completion at original issuance. Pass preceding managed placement gates and required managed selection at their FIFO turn. Then capture external predecessors and reserve synchronously in that view before invocation-input waits, export, native-suffix key waits, or host reflection. Reuse the same completion at reservation; never reserve again after an input or predecessor wait. Revalidate authority before deferred native access. Completion carries no Error state, and fatality rejects public results without settling or cancelling internal reservations.
...
Use this one reservation turn for all routes. A ready path reserves during issuance; a gated path reserves only after preceding managed gates. Gate provenance includes pending path selection, structural transitions, poison/repair, and entry, so do not infer reservation ownership from gate presence. The issuing Chain determines the view. Use the operation's one effect completion for entry lifetime from issuance through required selection-failure publication or native completion; retain only unfinished membership and no second scheduling queue. Implement the pending managed mixed-entry ordering described in external-context-ordering.md without provisional candidate reservations or reservation transfers. Phase 10 adds Promise-key traversal; 9F-A must establish the compatible gate/reservation handoff now.
...
Implement structural poison ownership in the common managed mutation selection/publication path. Select the smallest owning scope covering both the requested scope and affected intrinsic structure, retaining an explicit broader bang scope. Array length changes, truncation, and remapping require the Array owner. Failure at an out-of-range index, including a broken prefix, must poison that Array rather than grow it by installing an Error element. An in-range element or hole may retain placement-local poison when structure is unchanged. Select and protect the structural owner before the first structural write or placeholder, using the same captured baseline, COW, gate, and placement transition as other managed work. Never widen recovery after later conflicting writes have been allowed through.
...
Check fixed locations against the operation's actual structural effects at the selected owner. Array length conversion still determines whether truncation removes a registered index; a remap still verifies that registered placements neither move nor disappear. Share the fixed-location invariant without adding a whole-graph diff or a generic mutation-effects framework. Array growth and truncation above all registered indexes remain permissible; the mere presence of a registered descendant is not grounds to reject a controlled structural write. An explicit managed bang covering registered mutable resources remains invalid even when an implicitly widened structural owner would be permitted.
...
Evaluate one managed path traversal that captures the selected scope as it passes it and continues to the receiver, with the same coordinator owning publication. Remove the forbidden managed-scope-to-native mutation route first. Prefix selection and suffix traversal visit different segments; the candidate must reduce coordination, temporary holders, or publication paths rather than merely rename those two calls. Evaluate observeTarget removal in the same change: a successful external-prefix selection should not COW managed parents, while a failed mutating prefix must still publish at its prescribed placement through the common failure/COW mechanism. An observation walker alone supplies no recovery writeback. Preserve captured versions, first-failed-placement behavior, attribution, and attachment ownership across deferred failure. Keep a split or combined traversal only if its integrated implementation is simpler; revert an unsuccessful candidate without restoring forbidden routes or duplicate outcome contracts.
...
Compare separate receiver preparation followed by complete copying with preparation directly into the private working graph. The latter can copy available placements immediately and capture exact pending versions, following export's capture pattern without using public export or inheriting its different capability rules. Preserve aliases/cycles, Array shape, admitted prototypes, complete preparation Error collection, causal attribution, and method-selection ordering. Remove a source lease only when every later read is satisfied by independently captured state; baseline protection through publication has its own lifetime. Retain post-invocation validation of arbitrary native changes. Keep fusion only if it removes more coordination and traversal than it introduces; otherwise retain the straightforward prepare-then-copy path and its necessary leases.

Evaluate Array mutation across array-invocation.js, array-remap.js, and array-methods.js under the chosen baseline/working-storage representation. When a path derives its candidate directly from the protected Array, sourceSurvives is invariant there: remove its constant branches and any now-unreachable in-place apply/physical preflight, including helpers and callers. An already independent working Array may instead be changed in place, so rollback alone does not prove ArrayMutation.apply or requiresCopy globally dead. Any surviving replay must target that independent storage, never the baseline. Retention can legitimately determine both receiver copying and removed-element sharing; split facts only when the chosen representation makes their meanings differ. Do not add duplicate flags for two consequences of the same ownership fact. Compare a single remap/view publication path with existing tracing/materialization for sparse Arrays, pending captured elements, callback methods, large mostly unchanged inputs, and independent removed-element results. Views may avoid full copies, and intrinsic tracing may preserve native behavior without hand-written method algorithms. Keep only paths that still earn their cost; revert an unsuccessful unification while retaining rollback correctness and deleting branches proved unreachable.
...
Once placement transfer and mutation publication are integrated, evaluate whether attachment protection can be established at the actual retaining capture instead of using containsPromise to scan assigned descendants. First account for pending settlement, captured Error queries, cycles resolving back to an attachment, ancestor replacement, and deferred paths; protection must exist before any later conflicting mutation. Reuse ordinary leases, sharing, and captured versions. Do not replace the scan with new Promise subscriptions, an eager ref index, another descendant summary, or unconditional permanent sharing of private roots. If no simpler correct ownership rule covers these cases, retain the existing counter-pruned scan. Compare both fully ready admitted inputs and pending/cyclic inputs, including subsequent COW cost, not just the assignment scan.

Use the same mechanism for ordinary managed mutation failure, invalid mixed bang, forbidden fixed-namespace structural work, and supported ancestor reflection/publication failure. Keep all independently required Errors without changing their source/kind. Remove namespace-only record.recovery storage, success paths that mutate the preserved baseline, and retention of partially modified failed receivers. Earlier completed operations remain committed; entry is access ordering, and higher-level guard/recover owns rollback across a sequence. Fatality does not trigger a rollback/cancellation sweep.
...
Retire ExternalRepairFailed with its obsolete repairPath branches: remove it from ERROR_KIND, public vocabulary fixtures in test/error-attribution.test.js, and any remaining docs/callers. Healthy or absent managed repair returns undefined; ordinary Error data returns that same Error; unsupported external authority uses ExternalLocationConflict; preparation/reflection uses its existing causal kind. Introduce no replacement generic repair kind.
...
Preserve placement presence through entry setup and publication. Initialize a missing target as an absent private root placement; normal reads yield undefined, while assignment of undefined establishes presence. A delayed no-op callback must not create a missing record key or Array element. Reuse ordinary placement/version transfer for presence, value, source version, and recovery state; add no entry dirty flag or comparison against the initial value. Extend capture/fork/transfer, logical traversal, publication, and index maintenance together so absent completion cannot become a present undefined edge. Preserve existing root-operation semantics; do not reinterpret root deletion as removal of the original entered property.

Select structural protection before installing a mutating entry gate. An out-of-range Array index uses its owning Array's ordinary structural gate while the callback decides whether to create the element; existing in-range elements and holes keep local protection. Keep the private reference at the requested element and publish its final presence into the current working Array. Never grow length merely by installing a gate. Length/whole-Array observations and later structural mutations wait for the out-of-range entry and see its completed shape. For an in-range hole, ordinary version transfer must carry pending presence: export, logical enumeration, complete receiver preparation, and controlled Array callbacks must distinguish absent completion from an explicit undefined value. No callback runs for an element that remains absent. Reuse path COW and structural publication without an entry-specific Array representation, callback-wide baseline, or undo log. Preserve the relationship to the structural owner through contained mutation and repair so required Array poison is not published as a length-growing Error element. The wider gate grants no broader mutation/reference authority and must preserve fixed external locations.

An entry reserves its external subtree in the current ordering view. Every read-only entry uses observation coverage, including an external root; every mutating entry uses exclusive coverage. Managed read-only entry additionally leases its captured root where needed. Poison at a selected managed or external target does not suppress the callback: entry reserves access and transfers complete placement/recovery state, allowing ordinary inspection, permitted replacement, and explicit repair. Strict-ancestor poison and invalid binding block selection. Explicit unregistered native-property entry remains recoverable PropertyValidation; compiler selection chooses the deepest registered enclosing scope. Successful entry setup adds no poison, while failed mutating selection retains its prescribed effect. Entry grants no mutation authority or callback-wide rollback.
...
Use section 3's reservation turn. Record each potentially external command's one effect completion with the entry at original issuance, before managed gates or selection can wait, then reuse it in the eventual reservation. Complete selection failure only after required publication and selected native work after required effects/boundary processing. Remove completed membership immediately. Callback closure stops issuance and captures unfinished completions, including already-issued nested entries. Independent nested results do not extend access. Publish the managed root separately through ordinary versions, preserving ready sibling access while native child effects remain pending.
...
Keep ExternalAccess responsible for native suffix traversal and result conversion; PathOperation owns route selection, reservation, and final scope effect. No second reservation is acquired while walking a native suffix. Reads use logical snapshots when they reach managed sources; native methods cannot directly use those sources as managed storage.
...
Keep ordinary method-result import nonblocking for nested availability. Existing managed result graphs use captured logical versions, selective borrowed pending copies and ancestor propagation where result policy differs, source Error attribution, unchanged source settlement, aliases/cycles, and unchanged-branch sharing. The host relinquishes mutation of returned managed data. No new ready-only result limitation, whole-result await, or automatic native receiver snapshot is introduced.
...
- Reject replacement/deletion/remapping of registered nodes and connecting paths, while ordinary native child replacements and managed sibling changes remain valid. A violating host replacement at a committed location retains its fixed-identity failure classification.
...
- Verify preceding managed gates are passed before external reservation for entry, managed structural publication, namespace poison/repair, and failed selection. Preserve FIFO issuance order and the issuing view; waiting for an outside gate grants no entry ownership. No invocation-input wait precedes reservation after valid selection.
...
- While an out-of-range entry is pending, issue length/whole-Array observations and structural mutations. They wait for the final shape; no placeholder length is observable. In-range elements and holes keep placement-local ordering. Check no-op hole completion versus explicit undefined in export, logical enumeration, indexing, and controlled Array callbacks; no callback runs for a hole that remains absent. Exercise shared/imported ancestors, ArrayViews, fixed external siblings, and deferred root publication. Read-only absent-target entry creates no placement or structural gate.
...
- Fail at a missing out-of-range Array index, including a broken prefix, and verify Array-wide poison with unchanged baseline length/holes/values after repair. Cover empty and nested Arrays, ArrayViews, captured pending elements, explicit broader scopes, and in-range holes that require only local poison. Unaffected siblings and aliases remain usable; no ancestor receives duplicate poison. Ordinary sibling appends preserving fixed external locations remain valid; failure and repair preserve those locations under the wider owner.
...
- Preserve nonblocking fresh and already-managed nested method results, borrowed-result source version/attribution, unchanged branches, source/result isolation, and deferred capability rejection without extending the completed receiver reservation.
...
- A mutation gates that managed prefix and continues against its private value. Later contextual access through the prefix, including mutable external access, passes preceding managed gates before reserving in its issuing entry view. Reuse one effect completion captured at issuance for entry lifetime and eventual native reservation; do not reserve uncertain candidates.
...
- Entry follows Phase 9F-A: managed/mixed targets retain their requested depth; native-property lowering selects the deepest enclosing registered scope. Explicit unregistered native-property entry remains invalid. Rebasing cannot grant static authority.
...
- Enter a managed mixed target through a pending key, then issue a later static native access beneath a possible target. Resolve the key to that managed parent: the entry reserves before the later access, its callback can finish, and no reservation/gate cycle occurs. Preserve narrow sibling availability after selection and reject any dynamically derived attempt to use a registered identity. Repeat within nested entries and with an ordinary managed mutation selecting a non-resource sibling.
...
- Use the public entry-target handoff after context import: leave managed/mixed targets unchanged and select the deepest enclosing registered external scope for native-property targets. Emit relative suffixes and static provenance without guessing host categories. Supply nested mutation access paths; preserve fixed registered identities under native reset. Test the same compiled source with managed and external host shapes.
- Implement the [batch entry protocol](enter.md#multiple-reference-arguments) through one public handoff. Start all disjoint selections at issuance, passing each path's own preceding managed gates at its FIFO turn; reserve a ready external argument even while another disjoint argument awaits managed selection. Merge equal/ancestor routes and unresolved selection prefixes into shared entered state before acquiring overlapping entries, then derive relative references without duplicate mutating roots or writeback. Preserve per-reference readonly capability and managed lease snapshots; retain actual observation/mutation coverage for mixed modes instead of widening a read-only parent to exclusive access because a child is mutable. Repeat capture-before-publish when a prefix advances. Wait for all references only before invoking the function. Do not nest sequential acquire-and-wait callbacks, wait for every selection before publishing ready reservations, reserve guessed native candidates, or lock a common ancestor of disjoint arguments. Generated code never accesses private tree state. Phase 9F-A supplies composable path/version and reservation primitives; Phase 13 owns this batch selector and public API.
