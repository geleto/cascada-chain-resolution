...
Phase 9C fatal execution and the supported-thenable addendum are implemented. The addendum removes `Execution._thenables`, callable capture, canonical Promises, declaration probes, the separate result observer, and refcount mirror-installer callbacks. `consumeValue` performs ordinary subscriptions; normalized returned results determine pending work. Final Error representation/kinds remain Phase 9D-A/9D-B work, and external entries remain inert until Phase 9E.
...
| Synchronous callback staging; pending-only mirrors, gates, leases and public-result registration; ordinary custom-thenable rejection observation | Phase 9C addendum |
...
`failExecution(operationContext, reason)` becomes the sole operation fatal entry. `runOrFailExecution(operationContext, work)` is its synchronous fatal-on-escape envelope:
...
Phase 12 adds one documented trusted integration subpath containing the same unwrapped core operations, exposure helper, poison/compound factories, fatal submission, Error kinds, and precise predicates needed by Cascada. It is not another implementation, dynamic mode, or security boundary; its calls register no outward results, the higher runtime must not expose it as its host API, and no second Error factory or authorization token is added.
...
- or the public-result reactions installed through the common guarded continuation path.
...
- `commitFatal` stores one Error, rejects and clears only current pending public wrappers, reports, and walks no internal task, gate, phase, aggregate, or owner. The public `Execution` surface exposes only the own, non-configurable, read-only query; assignment, redefinition, shadowing, and prototype replacement cannot hide or replace the stored outcome, and no fatal commit or registration operation is package-exported.
...
The pre-addendum implementation cached a callable `then` per execution, invoked a source once, created a canonical native Promise, recorded derived Promises back into the same map, and used that producer record to distinguish ready from pending public results. This duplicates settlement and subscriber behavior already owned by a well-behaved Promise-like value. It also turns Cascada-style synchronous readiness into native-Promise microtask readiness, adds per-execution identity state, and creates machinery for hostile thenables whose behavior is outside any useful data contract.
...
6. Audit every pending-only decision together. `continueInternalAll` runs each component and adds only its returned pending chain to `Promise.all`; a synchronously consumed custom thenable may fill its value slot but must not contribute `undefined` or force a microtask. Export determines pending lifetime from the normalized aggregate result rather than pre-scanning its inputs. Root and Chain initialization, `enter` publication, mutation gate selection, operation finalization and result observation, managed-preparation release registration, invocation lease retention, Array preparation and `includes`, Error-query wait collection, path work, derived-chain handling, and public-result exposure likewise allocate pending machinery only from the normalized returned result. Mark only returned pending derived chains handled. JavaScript run-to-completion permits registration first and pending protection immediately after `then` returns: a callback not delivered synchronously cannot interleave before that protection is installed and the operation returns.
7. Make `exposeResultOrFatal` preserve Error, Function, and fixed admitted-category precedence before ordinary supported thenability. Remove producer recording as its pending-result signal. Core direct-result paths must consume a synchronously settled custom thenable before returning, so a thenable that reaches exposure is pending and receives the one outward native wrapper and fatal-reject action.
...
- Ready custom thenables in root and Chain initialization, `enter`, `continueInternalAll`, mutation preparation, invocation preparation, export, `includes`, and Error queries remain direct and acquire no pending-only wait, gate, lease, release, outward wrapper, observer, or microtask. Pending forms acquire exactly the protection required by that route before the issuing stack returns.
...
- Contextless declaration thenability recognition preserves an Error first and directly rejects a reached callable thenable. Inability to inspect an unsupported identity returns an ordinary declaration-validation Error and atomically records no declarations; an escaping `FatalError` remains the contextless fatal outcome. Alias deduplication belongs only to the declaration identity walk, not to a thenability cache. The probe creates no fake rejecting thenable, Promise, poison, kind, operation context, or persistent thenability state.
...
- Focused Error, result-exposure, and Promise-ownership audits and route tests cover both repositories. The actual-export classifier fails on unclassified package exports, while no custom source analyzer or semantic manifest is introduced. Discarded expressions and every stored or fire-and-forget higher-runtime Promise have one exact owner without recursive Promise scanning.
