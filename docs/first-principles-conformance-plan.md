...
| path-operation.js | Rewrite routing/coordinator orchestration from selected access scope, poison owner, and inherited reservation coverage. Remove successful managed-bang/native-effect composition, mutation-only gate registration hooks, and duplicate guard/phase state. Keep OperationOwner and result-boundary responsibilities. |
| enter.js | Rewrite mixed/external reservation setup and release. Retain managed source-version transfer, path COW, callback closure, and latest-private-root publication where their invariants are unchanged. Remove external contextual-binding gates and owned-gate maps. |
...
| mutations.js | Make managed mutation preserve its ordered baseline, mutate private state, and publish success or scope poison retaining that baseline. Remove namespace-only recovery side writes and target.onPublish if it only served the deleted gate index. Preserve ordinary gates, Array structure, source/result Error combination, and nonblocking publication. |
...
| property-versions.js, language-properties.js, meta.js, refcounts.js | Extend ordinary placement versions with visible poison and private retained baseline/version. Preserve captures, normal settlement, ArrayView placements, and visible-edge indexing. No separate guard class/transport, identity poison flag, Error-owned data, or second graph index. |
| external-access.js, external-snapshot.js, import-preparation.js, import.js, export.js | Reuse native boundary conversion, snapshots, complete collection, and borrowed-result isolation. Update selected-scope/binding plumbing for hierarchy; do not rewrite these algorithms merely to reduce file count or unify different readiness contracts. |
| run.js, index.js, chain.js | Replace affected integration points directly. Remove obsolete namespace/phase options when their implementation is gone. Preserve public signatures unless an explicit integration requirement needs a change; no compatibility overloads or duplicate public fatal boundaries. |
...
- Every public external access path and contextual query uses the new kernel, including work issued through entered Chains and work delayed before native selection.
...
One fulfillment-only completion represents an operation's required native effects and boundary processing. Keep capture and publication synchronous and composable for Phase 13 reference-argument batching. Capture and deduplicate required predecessor completions before adding this completion to its selected direct set and matching subtree sets on ancestors. Form a fixed wait from that snapshot. Remove each completed membership independently in the completion transition, using removable sets or an equivalent bounded representation. Do not roll a new Promise aggregate over the previous pending frontier on each issuance: one indefinitely pending child must not retain arbitrary completed-sibling history. Retain only current pending membership and dependencies actually captured by unfinished operations; completions carry no receiver/result payload.

Empty dependency sets mean no prerequisite, not Promise.resolve(). Ready native work remains synchronous and removes its own reservation membership in its completion transition without removing any other pending operation. Reuse completion state; add no second ready-result transport. Compare wide concurrent readers and long serialized write streams as well as sibling concurrency; eliminate redundant captured dependencies only where doing so preserves the conflict relation.

Native access must wait for earlier managed gates and captured external predecessors. Reserve in the correct ordering view before input waits, export, suffix-key waits, or native reflection. Capture explicit inputs at original issuance and revalidate authority before deferred native access without recapturing later ordering state. Completion carries no Error state. Fatality stops resumptions and rejects public pending results without settling or cancelling internal reservations.

Evaluate reservation at original issuance for a statically selected scope. Entry-local views may already prevent the deadlock that otherwise motivates waiting for managed gates before reservation. Integrate this candidate with real entry and path operations before introducing separate tracking of commands awaiting native selection. Verify nested child entry continuing after a later ancestor waiter, mixed-root publication, pending managed gates, deterministic selection failure and its poison publication, canonical namespace poison, and source-static authority. Retain early reservation only if all invariants hold and it removes state or execution paths without unacceptable work or retention. If it does not improve the implementation, revert that candidate and use delayed reservation with one issuance-time effect completion reused at native selection. Remove the rejected implementation, its options, and experiment-only fixtures; never retain both as modes.
...
Perform assignment, deletion, controlled Array work, and native managed calls on working state independent of the baseline. Controlled operations copy affected paths and reuse Array structural sharing. Native methods can write anywhere in their receiver, so provide a complete isolated receiver graph preserving aliases/cycles and admitted structure. Prefer the existing complete-graph copier over a selective isolation/reconnection walk whose root is always protected. Do not add copy-history state, intercept native writes, or deep-copy unrelated scope descendants. Remove candidate replay into the protected baseline solely to preserve native identity; replay may target independent working storage, followed by one publication. Keep observation preparation/materialization separate.

Evaluate the Array mutation implementation under the protected-baseline invariant as one change across array-invocation.js, array-remap.js, and array-methods.js. Determine which sourceSurvives/copiesReceiver choices, physical-storage preflight, and source replay are now unreachable or duplicate candidate publication. Compare a single remap/view publication path with the existing tracing/materialization behavior for sparse Arrays, captured pending elements, callback methods, large mostly unchanged inputs, and independent removed-element results. Keep a simplification only if it reduces total mechanisms without losing required behavior or imposing an unjustified cost. Otherwise revert that candidate while retaining rollback correctness; any surviving replay must target independent working storage, never the baseline.
...
Use the reservation timing selected in section 3. Every external-bound command participates in entry lifetime from original issuance: use its ordinary reservation directly if early reservation is safe, or reuse one issuance-time effect completion when native selection is delayed. Do not track the same command through two completions or retain both timing mechanisms. Complete selection failure only after its required publication, and selected native work after its required effects and boundary processing. Callback closure stops issuance and captures outstanding completions, including already-issued nested entries. Independent nested results do not extend access. Publish the managed root separately through ordinary versions, preserving ready sibling access while native child effects remain pending.

Transfer complete placement state into and out of the entered root, including a preexisting Error, retained baseline, and captured source version, through ordinary version transfer. A visible Error alone is insufficient. Cover nested entry and deferred publication without replacing a contained operation's baseline with the entry-start value; canonical namespace references must follow the same publication. Keep recovery data outside Errors and outside the public gate payload.

Remove contextual external binding gates, owned-gate dependency/bypass traversal, and parallel unfinished-scope indexes only once entry views and live guard references replace their behaviors. Do not combine managed root publication with the external view's all-effects completion. Reuse normal Chain representation; readonly/closed checks remain at common issuance boundaries.
...
- Lower multiple Cascada reference arguments without sequential acquire-and-wait. Collapse overlapping entries under a covering reservation and derive relative reference views; publish disjoint entry reservations in one synchronous batch before awaiting any predecessor. Use the common kernel's capture-before-publish protocol and a documented public handoff, not private tree access. Test reversed argument order, shared ancestor/descendant arguments, pending predecessors, and unrelated sibling progress. Phase 9F-A must leave capture/publish composable for this batch; do not add speculative public overloads during the kernel replacement.
