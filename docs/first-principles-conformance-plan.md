...
Phase 9C, its supported-thenable addendum, and its completion step are implemented. Phase 9D-0 is the next implementation step: preserve valid state after recovery and safe successful outputs before the recoverable boundary changes in Phase 9D-A.

## Current checkpoint and agreed changes

Phase 9C completion establishes subscription-exit fatal propagation, immediate required-mirror failure, independent execution-binding coverage, and aligned contracts. The implemented addendum removes `Execution._thenables`, callable capture, canonical Promises, declaration probes, the separate result observer, and refcount mirror-installer callbacks. `thenValue` performs ordinary subscriptions; normalized returned results determine pending work. Final Error representation/kinds remain Phase 9D-A/9D-B work, and external entries remain inert until Phase 9E.
...
| Synchronous callback staging; pending-only mirrors, gates, leases and operation-result registration; ordinary custom-thenable rejection observation | Phase 9C addendum |
| Atomic import staging/commit/abandonment and staged logical reads for initial external-tree discovery | Phase 9C addendum |
| Subscription exit propagates the subscribing execution's authoritative fatal, including one committed by an older queued reaction during that subscription | Phase 9C completion; preserved by 9D-A/9D-B and 12 |
| A detected missing required Promise mirror fails immediately at publication; fatal detection never waits for the corrupt source to settle | Phase 9C completion |
...
| Successful ready and pending outputs obey the same native `then` surface restriction | Phase 9D-0; 9D-B assimilation verification; 9F/12 boundary integration |
| Raw failure payloads exposed through `Error.cause` are diagnostic-only host data and cannot expose unexported managed state or mutation-capable external identities; source attribution remains the originating operation's source-error context | Phase 9D-A contract and kernel producer audit; 9F/12 integration |
| Immutable Error wrappers propagated by reference; cause/context/kind deduplication in queries and compounds; unspecified Error order; no persistent wrapper interning | Phase 9D-A, preserved by 9D-B and 12 |
| Operation context is mandatory for execution-bound work; its absence is a fatal runtime defect even on an otherwise successful path, with only justified entry checks | Phase 9D-A kernel entry audit; Phase 12 compiler/integration verification |
| No required Error is lost: complete collection is part of determinism; Error order and only the enumerated reporting/detection races may vary | Existing 9C fatal semantics; 9D-A collection/query tests; 9D-B assimilation cutover; 9E/9F invalid-binding tests; Phase 12 integration |
...
| Reject distinct discovered candidate paths to one external identity within a root context at import; merge duplicate discovery of one normalized location; retain only one binding location | Phase 9E import validation, with 9F integration and Phase 10 conservative-scope coverage |
| Keep phase completion Promises and non-thenable poison records; no direct-record shortcut or redundant rejection observer on fulfillment-only completion | Phase 9E |
| One trusted integration surface for core operations, exact host-action handling, guarded continuations, Error factories, and outward completion | Phase 9D-A surface and route tests; 9D-B transport cutover; Phase 12 consumers |
| Replace Cascada's legacy poison wrapper, thrown-poison interception, duplicated fatal routing, and source sorting with the kernel contracts | Phase 9D-A/9D-B kernel surface; Phase 12 Cascada integration |

The supported-thenable addendum preserves current inert tree entries and changes how import consumes availability, not the future authority mechanism. The external authority policy is also settled: Phase 9E rejects distinct candidate paths to one external identity within a root context during import. No first-use path selection remains. The agreed lazy-rejection policy and context-versus-regular-Chain distinction are unchanged.

### Error-handling review disposition

The [error-handling review](error-handling-review.md) supplies the evidence and Cascada comparison. The following assignments cover all eight findings; the phase instructions below are the implementation contract. Implement each requirement in its owning phase, updating `AGENTS.md`, `error-handling.md`, and `data-limitations.md` where specified. Record completion status separately from the final design.

| Finding | Required disposition and implementation owner |
| --- | --- |
| R1: recoverable reflection can leave a retained cyclic index invalid | 9D-0 stages new index facts through fallible work and publishes only valid downward-closed regions; 9D-A query recovery relies on this invariant |
| R2: raw `Error.cause` payloads can expose protected managed or external identities | 9D-A specifies diagnostic-only failure payloads and audits kernel-created reasons; 9F covers external capabilities; 12 applies the contract to higher-runtime host calls and safe views |
| R3: native assimilation differs for ready and delayed successful values | 9D-0 validates the completed managed receiver's full native `then` surface and states the exact-value host restriction; 9D-B/9F/12 preserve it |
| R4: factories alone are insufficient for shared host-boundary semantics | 9D-A implements and tests the trusted host-action/continuation integration surface; 12 uses it for calls, callbacks, iterators, and loading, including safe standalone-call `this` |
| R5: removing the host-escape marker may move complexity into graph policy plumbing | 9D-A proves one narrow boundary design on indexing/query and export before the full cutover; retain one private marker only if direct classification fails the stated simplification test |
| R6: native iterator finalization conflicts with a blanket ban on host work after fatal | 12 permits only already-owned local iterator finalization, defines its failure/Promise ownership, and removes scheduler-wide abort machinery separately |
| R7: tuple deduplication is not a log of dynamic failure occurrences | 9D-A defines unique cause/source/kind membership, including repeated source execution and absent causes; 12 aligns compiler source reuse and diagnostics |
| R8: diagnostic route retention, safe inspection, and formatter fallback are underspecified | 12 captures immutable routes at causal work creation and implements hook-free views with an explicit successful fallback |

Retain the strengths that already solve the problem: Phase 9C's first-fatal commit before reporting, removable pending outward obligations, local operation lifetimes, placement versions, and purpose-specific readiness records. Preserve `WrappedMethodResult` or its semantic equivalent where it distinguishes completed receiver publication from an independently pending result; it is not a redundant generic Error wrapper. Phase 11 remains a preservation check, not another Promise rewrite. Cascada's compact causal sources and bounded diagnostic presentation remain useful; its broad catches, cause-only collection, and mutable poison control fields are replaced. Its existing single root fatal race is not evidence of a current per-operation retention leak; the bounded-obligation design is needed when generalizing outward completion to every pending public operation.

Recoverable **Error values** are synchronously recognizable `PoisonError` and `CompoundPoisonError` instances whose thenable outcome is rejection with that same instance. Ready Error values can be inspected and processed without awaiting; native `await` and Promise assimilation reject identically on ready and pending routes. Required independent-input and export Error collection remains complete. Phase 9D-B installs the rejecting behavior atomically with the internal publication and collection changes it requires.
...
Keep one small CI test that imports the actual package surface and classifies every export as an execution-bound semantic operation, non-blocking construction, contextless configuration, recognition/data, a delegating alias, or the trusted higher-runtime integration entrypoint. A new export must be classified, and focused route tests prove that each execution-bound outward result is exposed exactly once, construction uses the public-entry and shared boundary checks without an outward wrapper, pending results register exactly once, integration calls register nothing, and aliases add no wrapper or registration.
...
- `combineErrors` flattens `CompoundPoisonError` inputs and deduplicates leaves by raw cause, source-error-context identity, and kind using the common collection rule. A leaf without a cause uses its own identity for that component; an explicit null or undefined cause remains a cause. Child order is unspecified. Return one leaf unchanged or combine several; each caller supplies the boundary message.
...
Phase 9D-B supersedes this phase's blanket `enter` callback-Promise rejection rule: an admitted poison rejection follows ordinary entry completion, while a raw or fatal rejection remains fatal.
...
One registered-class receiver-category module follows the isolation contract now recorded in [`managed-invocation.md`](managed-invocation.md). Registration rejects prototype-chain accessors before recording the class; Phase 9C extends the same validation to callable `then` so later output transport cannot assimilate an instance. Method-behavior restrictions are trusted except for the receiver and result validation specified below; the boundary adds no snapshots, comparisons, or scheduling instrumentation to detect violations.
...
- Registration rejects prototype accessors, with callable-`then` rejection added in Phase 9C, and a Promise-valued result is rejected without being awaited. Trusted representation, external-state, reentry, and post-return restrictions add no runtime enforcement machinery; ordinary registered state access remains ordinary graph access.
...
- Produces no unresolved language Promise, ArrayView, mirror, metadata, or other internal representation.
...
- A continuation first checks execution fatality and stops if the execution failed. In a live execution, it completes required shared mirror, property-version, refcount, and publication bookkeeping, then performs no operation-specific work after local closure.
...
Both constructors require an operation context. Keep these fixed runtime arguments positional. `Chain` admits existing Cascada data, while a root `ContextChain` imports raw host context data and receives the two compiler path Arrays. These runtime-supplied control facts use the narrowly justified operation-context checks in Phase 9D-A; they need no general defensive shape validation.
...
- Every execution-bound fatal entry receives its registering operation context and uses execution-owned `failExecution`. State declarations outside execution return ordinary validation Errors. Missing required operation context follows Phase 9D-A's narrow fatal-entry contract; operation context is never optional for execution-bound work.
...
The fatal reporter and synchronous host-code re-entry guard remain runtime-wide at the end of this phase. Phase 9C adds execution-owned first-fatal state, centralized fatal checks, and pending-operation-result fatal delivery, captures reporter routing and report idempotence on each execution, and removes mutable global reporter routing. The re-entry guard remains: rejecting re-entry preserves ready synchronous mutation and avoids a second nested ordering path. Immutable method tables, captured primordials, and sentinel Symbols remain module constants. Traversal maps, releases, copies, and other temporary state remain operation-local.
...
| Rejecting-thenable poison and audit of Promise paths that can carry language-value payloads | 9D-B |
...
- `FatalError` directly extends native `Error`. Give its exported constructor a module-private token and semantic brand, and let its trusted factory receive the supplied `errorContext` directly rather than adding repeated defensive checks. `FatalError.prototype` receives an own non-callable `then` and is then frozen, so a fatal Error remains non-thenable even if host code later extends `Error.prototype`. This is targeted protection of Cascada's own Error transport protocol: native Promise assimilation must not turn the fatal branch into the language-thenable branch. It does not broaden the platform contract into general tolerance of modified Array, String, Object, or Error primordials. Fatal Error is never admitted, combined, queried, repaired, or returned as language data.
- Phase 9D-A owns the recoverable hierarchy, factories, brands, immutability, kinds, and attribution: `PoisonError` directly extends native `Error`, and `CompoundPoisonError` extends `PoisonError`.
...
- Freeze each completed `FatalError` after construction and freeze its prototype once its methods are final. It carries immutable source, message, and exact cause reference but no reporting state. Phase 9D-A owns poison-wrapper and compound-child-array immutability; Phase 9D-B owns their final `then` and prototype freezing. Each execution's first write to `fatalError` prevents duplicate reporting within that execution; another execution receiving the same Error closes and reports independently.
...
2. Submit the candidate to package-internal `commitFatal(operationContext.execution, candidate)`.
...
The guard that merely skips a callback after an already-committed fatal does not classify a new exception from that callback. Therefore every asynchronous contextualization, admission, validation, and bookkeeping body runs inside `runInternalStep`. When the language contract requires the resulting poison to reject a Promise, contextualize it inside the envelope and perform only the deliberate poison throw outside. Audit root import, `valueWithOrigin`, managed direct-result handling, and Promise-backed imported-property publication so their ready and pending routes cannot disagree about fatal state.

Declaration validation outside execution returns ordinary host API Errors. A missing required operation context is a fatal integration defect under Phase 9D-A's narrowly justified entry checks. An entry without an execution throws that specific branded `FatalError` without creating an execution or using global reporting state.
...
Construct each `Execution` with an immutable reporter, defaulting to a no-op only when its creator supplies none. Validate an explicitly supplied reporter once at that host-configuration boundary and capture it in one module-private state record keyed by the exact execution; operation work never reads mutable global routing. The same record holds the nullable fatal slot and one initially empty Set of rejection actions for currently pending operation results. Expose the fatal outcome through a normal read-only `execution.fatalError` class getter over that record. Ordinary assignment cannot change it; deliberate property redefinition or prototype replacement is unsupported misuse of a trusted control object rather than a threat that earns per-instance accessor machinery. Package-internal `commitFatal(execution, candidate)` accesses that same record and atomically:
...
Native Promise settlement assimilates a fulfilled object with a callable `then`, while a ready return does not. Preserve sequential equivalence through source invariants rather than a second result transport. A successful non-Promise language-data result must not expose a callable `then` through native lookup: ready assignment returns and publishes `PropertyValidation` for a callable own placement; Promise-backed property publication performs the same validation when fulfillment becomes available; managed host mutation reports `InvalidManagedReceiver`; and managed-class declaration or snapshot adoption rejects a callable `then` anywhere on the retained prototype chain. Managed prototypes remain stable after declaration, standard record/Array prototypes remain stable, and exact Functions and external identities remain read-only after admission. Non-callable `then` data remains ordinary. Imported callable thenables already take the Promise path before admission. Operation return therefore recognizes Error, Function, and fixed admitted category before checking the remaining direct result for supported thenability; it keeps no separate producer fact or Promise-brand path.
...
Phase 12 adds one documented trusted integration subpath containing the same unwrapped core operations, return helper, poison/compound factories, fatal submission, Error kinds, and precise predicates needed by Cascada. It is not another implementation, dynamic mode, or security boundary; its calls register no outward results, the higher runtime must not expose it as its host API, and no second Error factory or authorization token is added.
...
- a language-outcome rejection adapter whose contextualization and bookkeeping run inside that operation transition and whose only outside action is the deliberate poison rejection;
...
- `FatalError` directly extends native `Error`, is branded, immutable, and non-thenable, and no longer uses the transitional `CascadaError` base. Existing fatal instances are submitted unchanged to each receiving execution, whose earlier fatal Error remains authoritative. Transitional poison remains recoverable only when a language-outcome continuation recognizes it; poison escaping a fatal-on-escape transition becomes the exact cause of a new `FatalError`. Phase 9D-A owns the one final poison/base-class cutover.
- `FatalError.prototype` is frozen with its own non-callable `then`; later `Error.prototype` mutation cannot make a fatal Error assimilable.
- The protected `FatalError` constructor rejects public construction without the private token. Its one trusted fatal factory supplies the source, and focused tests verify the resulting Error; do not test malformed internal calls as a supported runtime path. Phase 9D-A extends the same construction pattern and authoritative kind table to poison.
- `runInternalStep` and fatal-on-escape Promise reactions attribute every escaping failure to the causing operation and propagate the execution's first fatal outcome. A checked entry lacking required operation context throws the specific branded `FatalError` before work, including when there is no execution to fail. Setup validation outside execution returns ordinary host API Errors.
...
- Public entry, guarded continuations, host-boundary exit, subscription exit, and scheduler dispatch observe the same nullable `fatalError` value. Operation return relies on these actual boundaries, including queued work delivered inside `then`, and needs no duplicate facade-wide check.
...
- `commitFatal` stores one Error, rejects and clears only current pending outward wrappers, reports, and walks no internal task, gate, phase, aggregate, or owner. The public `Execution` surface exposes only the read-only class getter; ordinary assignment cannot replace the stored outcome, deliberate reflective or prototype tampering is unsupported, and no fatal commit or registration operation is package-exported.
...
- Two concurrent executions use their independently captured reporters without cross-routing. A normal-function reporter receives `undefined` as `this` and cannot reach the private state record. A synchronous reporter throw cannot replace the first fatal outcome; reporter return thenability is not inspected, reporter re-entry sees the committed non-null `fatalError`, and absence of a reporter never causes an asynchronous global throw.
...
9. Consume a possible placement thenable before committing its representation. Use an unpublished staging version for any logical backwrite needed by the resolver. If the resolver transition returns directly, publish its final value directly into runtime-owned storage; if imported storage must retain the physical thenable, commit that final value through the existing fixed-overlay mechanism. Install a distinct Promise mirror only when the resolver transition returns pending. The returned result, not whether staging was written, selects the representation. Removing source-thenable canonicalization does not merge actually pending mirrors or change the rule that each consumer registers at its exact program position.
...
- Finalization returns one continuation that closes the operation, eliminating the separate result-observer helper. The transitional non-thenable poison representation still requires explicit rejection propagation; Phase 9D-B owns the rejecting-thenable cutover.
...
| Property publication | Commit ready custom outcomes directly or through a fixed imported overlay; create a mirror only for a returned pending resolver | A staging or mirror backwrite carries the logical property value and is not a readiness flag |
...
Do not apply automatic consumption where thenable-shaped input or output is itself forbidden. Declaration APIs and synchronous-only callback contracts, such as an Array `sort` comparator, reject a callable thenable without invoking it. The synchronous mutable-external snapshot walk likewise rejects nested thenables, and completed managed-receiver validation rejects retained thenables, even if they could deliver synchronously. Direct host or property results retain their ordinary consumption boundary. Refcount indexing, Array remap, static external-tree construction, and other graph walks inspect already-normalized logical placements or their pending-version facts: they do not invoke a host thenable merely to classify readiness, and a thenable still stored as the logical placement value is an actually pending Promise-backed version. During initial import, discovery uses that segment's staged logical view. Do not delete semantic state such as `receiverReached`, query open/found state, phase membership, mirror values, or mutation publication facts merely because a continuation result also describes readiness.

### Cascada compatibility

Cascada's `ResolvedValue` demonstrates the required direct callback return. Its existing `PoisonedValue` is a precedent for synchronous rejection, but not a conforming implementation: it catches a callback's thrown `PoisonError` and converts it into another poison value. The new contract requires that throw to escape. Phase 9D-B installs the minimal conforming `PoisonError`, and Phase 12 replaces Cascada's legacy wrapper; add no compatibility catch or adapter in the kernel. `RuntimePromise` delegates scheduling to native Promise machinery but retains legacy attribution and wrapping that Phase 12 removes. Compatibility tests must distinguish useful scheduling behavior from these migration differences and exercise callback throws as well as outcomes. The FIFO crossing rule also applies to queued delivery: a later ready subscription cannot overtake an earlier undelivered callback. `IteratorWaitToken` is single-consumer and remains an internal scheduler token; it must not cross the kernel data boundary as a supported thenable.
...
- Direct host-call, import, property-mirror, path, mutation, query, and export routes preserve their boundary-specific success, poison, and fatal behavior for both native Promises and supported custom thenables.
...
- A synchronously consumed custom thenable assigned to runtime-owned storage publishes its final value without a Promise mirror. The same value in imported physical storage remains untouched and receives a fixed logical overlay. Pending native and custom sources still receive distinct mirrors per placement and program position.
...
- Initial context-tree discovery follows a ready custom outcome through its staged fixed overlay and records every relevant finite alias path without a second subscription. A pending outcome creates no leaf on later delivery, and failed discovery commits neither graph facts nor leaves.
...
Update [`AGENTS.md`](../AGENTS.md), [`data-limitations.md`](data-limitations.md), [`error-handling.md`](error-handling.md), [`runtime-spec.md`](runtime-spec.md), state-declaration documentation, and Phase 12's Cascada migration inventory with this final contract.

---

## Phase 9C completion: Propagate fatal state at scheduling and publication boundaries

**Implemented.** All four deliverables below are complete. Phase 9D-0 is next. Preserve Phase 9C's first-fatal authority, execution-local reporting, bounded pending outward obligations, ordinary FIFO subscriptions, staged import, and local lifetimes.

Validation: **946 tests pass** on Node `v24.14.1` under strict unhandled-rejection handling, including the refcount-oracle and stress fixtures. An isolated process that omits `run`'s binding check fails its mismatch test; production source and test hooks are unchanged by that sensitivity check. Fatal construction/reporting also passes the `Error.prepareStackTrace` probe without invoking that hook.
...
- Keep the generic host-code re-entry guard inactive during scheduling and supplied callbacks. Keep later fatal-observing continuations as check-and-return. The active subscribing stack must propagate fatality at its own boundary even when its callback did no work.
...
### 2. Use the existing required-mirror invariant at entry publication

In [`publishEnteredValue`](../src/enter.js), a pending private-root value requires its existing Promise mirror. A missing mirror fails immediately, independently of whether the raw value ever settles.

Use [`requirePromiseMirror`](../src/property-versions.js) and let the existing fatal-on-escape transition submit its exception immediately with the entry operation context. Keep no alternate raw-source subscription for corrupt state. Preserve the ordinary `continuePromiseVersion` publication path and its ownership when the required mirror exists. This reuses an invariant already required by property transitions and adds no new validation pass, recovery mode, or entry-specific pending state.
...
- Catch the synchronous failure from entry and assert it is the execution's stored fatal, with the entry's causal source and a missing-required-mirror cause. Replace the old delayed-publication message expectation with the shared invariant's diagnostic; do not preserve bespoke wording by keeping another branch.
...
- Keep the execution state record module-private and fatal commit/registration package-internal. Internal ESM exports for the Error and facade layers need no additional authorization token or indirection.
- Keep immediate missing-mirror failure distinct from valid pending publication. A known runtime defect requires no scheduling slot or source settlement before it can fail the execution.

### Preserve the remaining phase boundaries

The following mechanisms retain their semantic purpose: `continuationStarted` distinguishes exception origin; `receiverReached`, `pathSelectionComplete`, and `WrappedMethodResult` distinguish dependency handoff from an independently pending result; entry gates and read protection may precede even a synchronous callback. Operation-local aggregates and Boolean-search races are not execution-fatal races. Reporter return values remain uninspected and reporter-owned. Do not delete or normalize these mechanisms solely because another path returns a Promise.

Keep coordinated replacements with their existing implementation owners:

| Remaining implementation work | Owner |
| --- | --- |
| Recoverable index-publication safety and successful-result native `then` restrictions | 9D-0, immediately after this completion step |
| `CascadaError`, mutable/unbranded poison, stored compound `.kinds`, broad recoverable `isError`, cause-only deduplication, transport-specific kinds, exact-cause containment, and complete recoverable attribution/collection | 9D-A; cause containment also has 9F/12 boundary integration |
| Attribution-only `valueWithOrigin`, paired `UserCodeFailure`/`runUserCode` catch paths, competing lifecycle variants/close entry points, generic `runContextlessFatal` and its sentinel | 9D-A's causal-continuation migration, bounded host-boundary trial, lifetime consolidation, and separation of setup validation from missing operation context |
| Non-thenable recoverable poison and deliberate manual rejection outside internal envelopes | 9D-B's atomic representation and Promise-path cutover |
...
| Cascada's legacy Promise wrappers, scheduler, compiler operation contexts, diagnostics, and already-owned iterator finalization | 12 |

Delete `valueWithOrigin` only after its producer attribution lives in an already-required causal continuation. Preserve post-host fatal checks through the host-boundary trial, and preserve live-execution releases through lifetime consolidation. Remove trivial forwarding locals when touching their query/export continuations if doing so improves clarity; they do not warrant a separate rewrite or performance claim.
...
- All four deliverables pass. Phase 9D-0 is next; the recoverable and higher-runtime work above remains required.
...
The review found two current correctness gaps that changing Error wrappers cannot repair. `refcounts.js` can publish a descendant index whose cycle cut reaches an unfinished ancestor, then recover from a later reflection failure while leaving that descendant invalid. Separately, successful output validation misses native `then` properties outside language placements, and exact Functions with callable `then` can survive the ready route but be assimilated on the pending route. Finish this phase before 9D-A so expanded recoverability does not preserve corrupt state and boundary migration does not bless transport-dependent results.

### 1. Make index construction safe to abandon

- Reproduce the retained-cycle failure: index an assignment destination; retain `b` from `a.b = b; b.back = a`; put a later sibling on `a` whose supported reflection throws; assign `a` into the indexed destination. The assignment may return its recoverable mutation Error, but querying or mutating the independently retained `b` afterward must not fail with `Ref counts require a ref-indexed value`.
- Audit `buildRefIndex`, `indexComponent`, `prepareLiveEdge`, and their assignment, query, imported-fulfillment, COW, and indexed-replacement callers. Existing index presence must mean a complete valid index, never an in-progress marker. Downward closure includes every traversable child reached through a bookkeeping cut.
- Build new index, cut, and parent-edge facts in one operation-local staging map. Reuse already-complete indexes. Finish fallible reflection and logical-value preparation for the region before installing its new persistent index facts. Publish the completed region in one synchronous hook-free commit. If publishing smaller regions, each region must already satisfy downward closure including its cut targets; an indexed descendant pointing through a cut to a merely staged ancestor is not complete.
- Stage additions to existing parent/counter facts as well as newly created indexes; abandoning a failed build must leave every previously retained identity valid. Reuse ordinary placement/version capture and publication when normalization or supported-thenable delivery makes synchronous progress. Commit only facts for the versions actually selected by that transition; do not reread changed physical slots, add callback-ran flags, or defer a ready commit to a microtask.
- Keep the destination's normal failure effect and the original causal kind. Ordinary supported reflection failure remains recoverable if the build can be abandoned safely. A bookkeeping defect remains fatal. Do not solve this by making every reflection failure fatal, copying the whole graph, adding a general transaction/rollback manager, or clearing all existing indexes after failure.
- Apply the same publication invariant to every path that can recover after constructing an index. Import's existing staged admission is not sufficient if another helper has already published invalid indexing facts.

### 2. Close the successful-result assimilation gap

- State the supported input condition in `data-limitations.md`: an exact Function or external identity used as a successful non-Promise language value must have a stable native `then` lookup that safely yields a non-callable value for its entire use. Read-only-after-admission alone does not establish this initial condition. Function and Error classification still precede availability recognition; a Function is never consumed as a Promise merely because it has callable `then`. Unsafe exact values are outside this host contract, without a new thenability cache, facade wrapper, or recurring arbitrary reflection probe.
- Distinguish classification precedence tests from supported-output tests. Recognizing an Error or Function without sampling a hostile `then` does not promise that every such Function can be successfully returned through native Promise transport. Errors retain their separate Error semantics.
- Validate the native `then` surface of each completed managed receiver and traversable managed descendant before successful mutation publication. Check relevant own non-enumerable properties and Array non-index properties as well as ordinary placements, and retain the admitted-prototype check for inherited callable/accessor surfaces. Inspect descriptors through the exact supported reflection boundary; reject an accessor without invoking it. A callable own data `then` or otherwise unsafe native lookup yields `InvalidManagedReceiver`, using the existing receiver-validation failure effect. Do not use `isPending`'s admitted-metadata shortcut as proof that a mutated receiver is safe.
- Keep ready assignment and Promise-backed publication's existing `PropertyValidation` rule for an ordinary callable `then` placement. A non-callable data `then` stays ordinary data. Do not expand language graph traversal to all non-enumerables or copy such fields into managed graph results merely to implement this native-transport invariant.
- Audit managed method return/admission, external observation/export, and public result completion for the same invariant. Preserve complete receiver preparation, COW isolation, and immediate publication when only an independent result remains pending. Phase 9F applies this rule to its new external routes and Phase 12 to its host-facing render routes.

### Verification

- Add the retained-cycle regression above with late sibling failure after a descendant and cycle cut have been visited. After recoverable failure, query and mutate all retained aliases, assert the execution remains live, and run the refcount oracle. Exercise ready and deferred failure through assignment, query-triggered indexing, imported fulfillment, and indexed replacement where those routes construct indexes. Do not assert canonical cut positions or exact counter totals.
- Inject an adjacent internal bookkeeping failure separately and verify that it remains fatal; recoverability is not a catch around the entire index walk.
- A managed method that installs a non-enumerable callable `then`, an Array non-index `then`, or a forbidden accessor cannot publish a successful assimilable receiver. Cover nested managed descendants, existing admitted receivers, and direct ready and pending method completion. Assert no accessor invocation and the specified validation failure effect.
- Supported exact Functions and observation-only external identities retain the same identity and value on ready and pending output routes. Test the managed hidden-`then` defect before the fix; keep the hostile exact-Function probe as evidence for the documented host restriction, not a requirement to diagnose all unsupported host behavior.
- Run the full suite under strict unhandled-rejection behavior and the refcount checks exercised by its tests. Update `AGENTS.md`, `error-handling.md`, `data-limitations.md`, managed invocation, and output-boundary documentation with these invariants; keep Phase 9C's completed status and ordinary continuation architecture.
...
Phase 9B carries the final operation context to every boundary, Phase 9C provides one authoritative fatal lane, and the 9C addendum installs the final supported-thenable mechanism. Phase 9D-0 makes recovered index construction and successful output transport safe. Recoverable Error attribution and boundary handling still use the transitional model.

### Outcome

Implement causal recoverable Error handling from [`error-handling.md`](error-handling.md). The exact causal boundary supplies a recoverable failure's source and kind; later consumers preserve it. Preserve the addendum's ordinary supported-thenable subscriptions while installing causal placement/import behavior; poison remains non-thenable until Phase 9D-B performs its rejecting-thenable cutover. Keep synchronous host-code re-entry forbidden. Every internal failure continues through Phase 9C's fatal lane.
...
- **Missing required operation context:** generated/runtime code failed to supply required control state. This is itself a fatal runtime defect, even if the requested work would otherwise succeed or return immediately. Every execution-bound Chain initialization, operation, continuation, and participating helper requires its proper operation context. Never treat its absence as an optional argument, poison outcome, successful no-op, or permission to use a fallback. Chain binding remains execution-specific; each operation supplies its own operation context rather than falling back to the Chain's initialization source. Section 2 specifies the small set of justified checks and fatal delivery when no execution is available.
...
- `PoisonError` is recoverable language data and a direct native Error. It remains non-thenable at this phase boundary; Phase 9D-B adds its final sync-first rejecting `then` only after every internal fulfillment path is safe.
...
Create kernel Errors only through factories in `error.js`. Reuse Phase 9C's module-private construction token, brand, and finalization pattern for the two exported poison classes; `instanceof` or prototype shape alone is not trusted. Direct public construction fails as host API misuse, and a public subclass cannot obtain the token. The token check is the only construction-time validation. Internal factories trust their compiler/runtime-supplied kind and source. Each factory installs all concrete-subclass fields before invoking one private finalizer. That finalizer freezes the complete wrapper; compound child arrays are copied and frozen first, and `errorContext` is an immutable opaque handle or value. Freeze each concrete prototype only after its final methods are installed; Phase 9D-B performs the poison-prototype freeze when it adds `then`. Do not copy methods onto instances or add recurring prototype-integrity checks. The exact cause identity remains diagnostic-only and outside graph traversal; later mutation of that external cause cannot replace the wrapper's cause reference or change its stored message, source, or classification. Do not copy arbitrary enumerable cause properties or eagerly read a cause stack into the wrapper. The protected diagnostic adapter may inspect the exact cause later.

Name the trusted causal factory `createPoisonError(reason, operationContext, kind)`: it submits an existing `FatalError`, preserves existing poison, and otherwise creates one branded occurrence. `combineErrors` remains the trusted compound factory. This phase re-exports both through `./integration`; the host-facing root exports only their recognition/data surfaces. This is package-composition trust, not a security boundary, so add no caller capability or duplicate higher-runtime constructor.

Error outputs must preserve owner isolation, immutable Chain outputs, and external-capability containment. This requirement covers diagnostic references exposed to host code, including `.cause`, as well as ordinary returned graph data. The choice of storage or wrapper mechanism must preserve that guarantee.

The concrete failure is a managed observation that executes `throw this`. Its public poison can expose the actual managed receiver as `failure.cause`; assigning `failure.cause.n = 9` then changes the value read through the Chain. The [review probe](error-handling-review-repros.mjs) demonstrates a change from `1` to `9`. A normal native Error can expose the same reference through `new Error(message, { cause: this })` or another diagnostic field. Freezing the poison only fixes its own properties; it does not detach the object reached through `.cause`. A separate safe `#` view does not protect native code receiving the poison directly.
...
Make the common post-boundary ready-admission choke point enforce the resulting invariant: an Error reaching ordinary graph admission is already branded poison. Submit `FatalError` through the fatal lane, and treat any remaining raw native Error as a fatal missed-boundary defect rather than admitting it as generic Error metadata. Boundary-specific import walking may inspect a raw Error only long enough to create its occurrence wrapper before entering this choke point. Test ready, fulfilled, nested, imported, assigned, and host-result routes so no inbound path can bypass contextualization.

The implementation must distinguish a failure thrown by the exact supported host action from a defect in surrounding runtime work. A private host-escape marker is one possible carrier: it tags the former while it passes through shared helpers to the causal boundary that supplies its source and kind. The marker is not language data or a public API requirement; choose the simplest mechanism that preserves this distinction.

Start with a bounded simplification trial on query-triggered indexing and export before rewriting every boundary. Enumerate the individual hook-capable actions in `enumerableLanguageKeys`, `readLanguageProperty`, and their callers: an entire enumeration/normalization helper is not one host action. Implement direct classification using a fixed causal kind supplied by the owning query or export operation to each exact reflection action. Pair a host trap failure with an adjacent internal traversal/indexing failure and verify that only the former is recoverable, with Phase 9D-0's retained-state invariant intact. Include the non-callable-Error `then` protocol case above. A single extra fixed argument through cohesive operation work is acceptable; mutable policy on operation contexts, identity metadata, placements, or Promises is not.

Use the direct design below if that trial removes the marker without duplicating traversal, adding policy branches to unrelated graph helpers, or broadening a recoverable catch over internal work. If it cannot, retain one private branded supported-host escape marker, produced only around the exact host action and consumed once at the owning causal boundary. Its producer owns the re-entry guard and post-host fatal checkpoint; its consumer supplies the fixed source/kind, handles only that marker, and lets every adjacent internal exception reach the fatal envelope. A marker is an implementation carrier, never language data, an Error kind, or a public return shape. Keep only one chosen mechanism, remove superseded marker/catch variants in this phase, and record the trial result and resulting rule in `error-handling.md` and `AGENTS.md`. Do not leave both designs available as modes or require a new approval step for this bounded implementation choice.

The direct target is one kernel-owned `runHostBoundary(operationContext, kind, action)`. Its `try` contains only the exact synchronous host action. On either return or throw, it first checks `operationContext.execution.fatalError`; if attempted re-entry or reporter work already closed that execution, it throws the authoritative Error without inspecting the host outcome. Otherwise a thrown reason or ready language-result Error is handled outside the action's `try`: an existing `FatalError` is submitted and the authoritative fatal outcome is thrown, existing poison is returned unchanged, and another reason becomes a new poison occurrence. The causal caller sees only success or poison and applies that boundary's graph or result effect outside the catch. A failure in contextualization or failure application escapes to the surrounding fatal-on-escape transition and is fatal. The helper has no failure callback, recoverable/fatal flag, or preparation, export, import, publication, bookkeeping, or cleanup behavior. It owns the post-host check; do not add a second helper or caller check. In the direct design, a lower-level effectful primitive does not catch; its causal caller wraps only the primitive invocation. In the marker design, the same trusted host-action interface composes the single producer/consumer mechanism, and graph callers use that mechanism's exact-action producer and one causal consumer instead of a second recovery path. Application callbacks and effectful host reflection use the selected design. A captured built-in is runtime-owned only on runtime-owned, hook-free inputs; applying it where a Proxy or host trap may run is supported host code. “Native code” alone is not a recoverable category. Preserve the re-entry guard: permitting nested work would require an ordering barrier before otherwise-ready host mutations and would undermine the sync-first contract.
...
- Contextless declaration thenability recognition preserves an Error first and directly rejects a reached callable thenable. Inability to inspect an unsupported identity returns an ordinary declaration-validation Error and atomically records no declarations. Re-entrant declaration use throws an ordinary host error before registry mutation; if it escapes the user code, the surrounding operation contextualizes it as poison. An unexpected implementation exception escapes synchronously, while an already-branded `FatalError` is thrown unchanged; declaration code constructs no fatal outcome. Alias deduplication belongs only to the declaration identity walk, not to a thenability cache. The probe creates no fake rejecting thenable, Promise, poison, kind, operation context, or persistent thenability state.

Require an operation context on successful, failing, immediate-return, and deferred routes alike. Keep ordinary helpers dependent on that invariant; add no per-helper defensive shape validation or malformed-control test matrix. Delete the generic `runContextlessFatal` wrapper: declarations use direct host validation, and missing-operation-context entry checks use the private fatal construction specified below.

Audit a small candidate set of high-impact locations: Chain construction and the first kernel entry used by generated/higher-runtime semantic operations, including the trusted integration subpath. The required positional operation-context argument and the compiler/integration signature cutover are a credible omission risk at those boundaries. Reuse their common entry check where possible; do not duplicate it in every facade, core operation, helper, and continuation. Check only required operation-context presence and the minimum required binding facts before any graph/host access or successful fast return. Do not inspect diagnostic fields such as file/line, deeply validate record shapes, or add a second check to already-covered paths. Any additional check requires a concrete plausible omission route and material impact; record that reason with the implementation. Keep Chain/execution mismatch and closed-entered-Chain checks at their existing meaningful boundaries.

Missing required operation context always has fatal semantics. If a valid enclosing operation context is available, propagate the defect through that execution's ordinary `failExecution` path. At a checked outer entry with no execution available, synchronously throw a branded `FatalError` using the same private construction/finalization machinery, with diagnostic source identifying the runtime entry and the missing operation context. This narrow diagnostic construction neither supplies a replacement operation context nor authorizes work: create no execution, choose no ambient execution, infer no execution from a later consumer, and add no reporter/global failure state. An execution that subsequently receives that `FatalError` submits it unchanged through its normal fatal lane. Keep this exception private to required-operation-context enforcement; it is not a general fatal constructor for setup APIs or arbitrary host failures. `failExecution` is the sole execution-failure/commit route.

Fatal fixtures use explicit executions. Focused missing-operation-context tests exercise only the selected high-impact checks, including otherwise-successful and no-op work; they must prove fatal rejection before work rather than rely on some unrelated operation error to expose the omission. Preserve valid operation contexts through ordinary helper signatures and compiler integration tests rather than checking them repeatedly at runtime.
...
Remove `operationLifecycle.continueResult`, `continueInitial`, `continueInternalResultOrFatal`, `continuePrepared`, `continueAllInternalResultsOrFatal`, `continuePreparedAll`, `resolveInitial`, and `closeWhenDone` as lifecycle-layer variants. In `resolution.js`, remove `observeResultPromise` and replace `continueInternalResultOrFatal` with the same one guarded operation-step registration used everywhere else. Keep initial value admission and later property-version advancement as separately named semantic bodies because only the first classifies a raw rejection; graph-Error publication and complete required-input collection likewise remain explicit semantic bodies. They may all use the common registration entry, but they are not modes or options of it. Make this a fresh collapse of the continuation stack, not adapters that preserve the old helper layers. Retain `thenValue` as the supported-thenable subscription primitive, one owner-aware guarded continuation entry, and an aggregation helper only if multiple semantic callers still need the exact same aggregation. If `resolution.js` has no cohesive responsibility afterward, move its remaining primitives to their natural owners and delete the module rather than retaining a namespace for historical layering.
...
Make query-reflection failure an explicit query-operation outcome. Classify each exact effectful reflection failure as `QueryReflectionFailed` through the selected host-boundary mechanism; never recover an arbitrary traversal exception. Bubble poison through the query body, close its local owner, and discard any partially collected query-only set; do not call `found`, convert it to Boolean, append it to `getErrors`, or throw it through the fatal-on-escape envelope. Phase 9D-0 is a prerequisite: discarding a query-local collection does not repair invalid persistent indexes. A ready query returns poison directly. While poison remains non-thenable in 9D-A, the existing language-outcome Promise reaction deliberately rejects the direct query Promise with that poison outside the fatal-on-escape catch; after 9D-B, returning the thenable poison has the same rejection result and the transitional rejection adapter is removed. Add no query-result wrapper or general result algebra.
...
| Internal query traversal, refcount, mirror, publication, gate, cleanup, or other transition that admits no Error outcome | Failing operation | fatal `FatalError` |
...
- Treat the custom `then` body as the supported scheduling protocol rather than generic host code. Its supplied continuation may enter kernel transitions synchronously without tripping the host re-entry guard; any other synchronous re-entry from the thenable body is unsupported.
...
- Pending Promise mirrors and fixed imported logical overlays already share logical read, replacement, detachment, and captured-version behavior. Reuse the fixed form for both contextualized native Errors and synchronously consumed custom thenables whose imported physical storage must remain unchanged; finish causal Error migration through this common path.
...
- Keep external-tree discovery as a separate occurrence walk over the segment's staged logical placements and admission facts: admission deduplicates identities, while tree construction must preserve every finite alias path. The two walks still commit atomically, without resubscription or early overlay publication.
...
Create the package `./integration` subpath in this phase, while the boundary and continuation primitives are being rewritten. Re-export existing unwrapped core operations, `returnOperationResult`, `createPoisonError`, `combineErrors`, `failExecution`, `ERROR_KIND`, precise Error predicates, `runHostBoundary`, `runInternalStep`, and the one owner-aware guarded continuation entry from section 2 (name it `continueOperation`). Include the existing execution-bound readiness recognition primitive needed by those higher-runtime transitions; reuse `isPending` or its final replacement, with Error/Function/admitted-category precedence and the 9D-0 host contract. Do not expose a raw subscription primitive as an alternative that bypasses the guarded entry. These are re-exports of kernel implementations, not copies, new result wrappers, a public/internal flag, or an executionless Error construction path.

Document the composition protocol with a small standalone-call example and an iterator-advancement example:

- The initial semantic body enters `runInternalStep` with its immutable operation context. It prepares/exports inputs and selects any authorized receiver before `runHostBoundary` invokes the exact host action. A ready success or poison is processed synchronously by the causal body.
...
- A supported custom `then` invokes its supplied continuation under the scheduling contract; the generic host-code guard is not kept active around that continuation. Host Functions, callbacks, iterator methods, and loader hooks use the host-action envelope and its one post-action fatal check. Both repositories share the same guard, fatal brands, and causal factory.
- Internal composition returns the classified result without an outward registration. The single public execution-owning facade calls `returnOperationResult` exactly once. Delegating aliases and callback adapters consume that result without wrapping it again.

Phase 9D-B changes poison transport and reruns these routes; Phase 12 consumes this established surface for all higher-runtime host actions rather than adding private deep imports or a second guard. The package-surface classifier includes these trusted integration exports and verifies that importing/calling them creates no second outward-result boundary.

Keep settled semantic rules in the architecture and principles, and outstanding code-removal and implementation tasks in this plan. Document the marker rule selected by the implementation trial. Phase 12 owns the local iterator-finalization and diagnostic route/view requirements.

Complete the documentation alignment in this phase alongside the implementation: `AGENTS.md` and `error-handling.md` must describe the final poison hierarchy, the separate required-operation-context and outside-execution configuration contracts, and one complete idempotent local close without prescribing its method location. Their fatal-delivery rules and prohibition lists must distinguish internal work and operation owners, which have no registered fatal reject action, from actually pending outward operation results, which retain Phase 9C's required removable reject actions. These actions deliver the execution's already-classified fatal; they do not classify invalid application data as fatal. Examples and tests assert complete Error membership and preserved attribution without requiring Error order. Phase 12 owns the corresponding higher-runtime changes and verification of its existing platform requirements.

### Verification

- `PoisonError` and `FatalError` directly extend native `Error`; `CompoundPoisonError` extends `PoisonError`. All three are native Errors, and `CompoundPoisonError` also has poison semantics through its parent. Remove the runtime `CascadaError` base and export. Poison remains non-thenable until Phase 9D-B.
...
- At each justified operation-context entry check, omission causes `FatalError` before graph/host access even when the requested work would otherwise succeed or immediately return. Cover Chain initialization and the common semantic integration entry without duplicating tests for every internal helper. A valid enclosing execution receives its authoritative fatal outcome; a root entry with no execution throws the branded missing-operation-context fatal without manufacturing state or reporting globally. Passing that fatal into another execution uses ordinary fatal submission. Missing optional source-location fields on a valid operation context remain nonfatal.
...
- Admission classification preserves an uninspectable exact identity as external without constructing poison, while a fatal established during its reflection still wins. Contextless declaration thenability recognition is direct; an unreadable `then` returns ordinary validation and creates no synthetic thenable or persistent state. Neither probe generalizes `runHostBoundary`.
- Probe tests cover a classification Proxy trap throwing an ordinary value or an existing `FatalError`; only the ordinary failure falls back to the exact opaque identity. They retain Phase 9C's nested-fatal post-probe case, in which a hook catches the fatal re-entry result but admission still propagates the execution's authoritative failure. Declaration tests cover alias deduplication, a throwing `then` getter, atomic no-declaration failure, an Error with hostile `then`, and re-entrant declaration use. Expected declaration failures are ordinary synchronous host API Errors and leave the registry unchanged; re-entry becomes poison only when it escapes through a surrounding operation's host boundary. An unexpected declaration implementation exception escapes synchronously without constructing a `FatalError`, while an existing `FatalError` is preserved unchanged. Tests needing an existing fatal first fail an explicit execution. Declaration handling creates no execution, poison kind, Promise, or persistent capture, and asserts no stable behavior for repeated access to an unsupported dynamic getter.
...
- Completed kernel Error wrappers are frozen and compounds expose copied frozen arrays. Public code cannot forge kernel-assigned source or kind. The kernel surface contains no legacy presentation or location fields; a separate diagnostic view formats the opaque context and exact cause without decorating the Error.
...
- The boundary trial covers exact reflection failures, adjacent internal failures, and retained cyclic indexes on query and export routes. Reading a non-callable native Error-valued `then` succeeds as a protocol probe; throwing the same Error is `ThenAccessFailed`. Existing or newly committed fatal state always wins. No policy stored on a source context or graph identity, duplicate traversal, or parallel marker/direct recovery mode survives the cutover.
- Trusted-integration tests exercise a standalone host call and iterator advancement with ready, synchronously delivered custom, and pending outcomes. Host code attempting to re-enter the kernel fails the owning execution before graph work; even if it catches that fatal and returns a normal value, the shared post-host checkpoint propagates the original authoritative fatal. Adjacent trusted continuation failures stay fatal. Each outward facade registers at most one pending result, while its internal integration calls register none.
...
- Verify no loss across automatic invocation and export: mix a ready Error with later rejecting required siblings and nested properties, reverse their settlement order, and compare the complete cause/source/kind sets. Assert that host code is never invoked, each leaf keeps its original source and cause, and a first Error does not close required collection. Use a manually controlled pending sibling to show that the result remains pending after another input fails and completes only after that sibling contributes its outcome. Deduplicate equivalent leaves while retaining different sources or kinds; do not count repeated propagation as another failure. Repeat these cases after the Phase 9D-B assimilation cutover and through Phase 12's compiler-generated call and final-export routes.
...
## Phase 9D-B: Complete the rejecting-thenable poison cutover
...
Complete the recoverable representation in [`error-handling.md`](error-handling.md). `PoisonError` remains synchronously detectable language data and also acts as an ordinary supported sync-first rejecting thenable under the Phase 9C addendum contract: subscriptions are repeatable and ordered, the same Error is the rejection outcome, and a rejection callback's direct result or pending chain determines the `then` result. Publication stores poison before an operation Promise rejects; complete collection keeps it outside every internal fulfillment payload.

Use explicit terminology in the architecture, API documentation, and examples:

- An **Error value**, or **poison**, is a contextualized recoverable `PoisonError` or `CompoundPoisonError`. The same object is both a native Error instance and a rejecting thenable; there is no separate Error-value wrapper. Recognize and process an available Error value synchronously with `isPoisonError`, before ordinary thenability handling. `await errorValue` and `Promise.resolve(errorValue)` reject with that exact instance. A rejection handler can recover by returning another value without changing the original Error value's rejection outcome.
- A **raw native Error** is an unclassified host Error. A causal boundary preserves it as the `.cause` of an attributed Error value. Recognize native Errors before inspecting `then`; arbitrary host `then` properties do not grant poison semantics.
- A **fatal Error** is a `FatalError`, remains non-thenable, and is never admitted as a language Error value. Promise rejection alone does not imply fatality: recoverable Error values also reject.
- A **diagnostic view** is ordinary immutable, non-thenable data used to inspect a failure without propagating it through Promise assimilation. Non-thenable internal collection and phase records likewise carry Error values as fields; they do not change the Error values' own behavior.

This representation is required to preserve both sides of the public contract without an adapter: ready poison remains directly inspectable, while `await operation()` rejects with the same Error whether the operation completed ready or pending. Plain non-thenable poison would make `await` fulfill in the ready case, and wrapping it at operation return would sacrifice sync-first return. Do not replace 9D-B with facade-only normalization.

### 1. Make internal Promise transitions safe

Keep the full suite green after Phase 9D-A, then audit every Promise path affected by thenable poison:

1. Audit every kernel Promise producer, resolver call, fulfillment callback, aggregate branch, gate, phase, and operation-result settlement that can receive or return a language value. Review the final common guarded transition, semantic publication and complete-collection bodies, mutation and entry gates, external phases, and aggregates in `export.js`, `managed-invocation.js`, and `observations.js`. Prove behavior through focused ready/pending route tests and strict unhandled-rejection execution; do not add a source checker, semantic classification manifest, or references to lifecycle helpers removed by Phase 9D-A.
...
Keep purpose-specific non-thenable records for complete collection and external phases. They encode different transitions. Do not introduce a shared result algebra or Boolean policy helper merely to box poison; focused producer audits and route tests cover these paths.

Do not add poison-fulfillment normalization to `returnOperationResult`. After poison becomes thenable, native Promise resolution of the outward wrapper already rejects with it; another outward reaction would be redundant and could not prove that an internal transition applied the required graph effect. Focused route tests guard the internal invariant. Phase 9D-A is a migration checkpoint with transitional non-thenable poison, not a release point for the final pending-result type; use only its explicitly required local language-outcome adapters and remove those in this atomic cutover rather than adding a facade-wide temporary path.
...
- any kernel-local equivalent of `PoisonedValue`, `RuntimePromise`, or `PoisonErrorGroup`; the actual legacy Cascada definitions are removed with their consumers in Phase 12, and are not ported into this package;
...
- Raw import, assignment, and host-result rejections use their introducing boundary. Shared settlement and copied mirrors never substitute the consumer that advances them.
- Failing `then` access or invocation uses the exact recognizing or subscribing operation. Native Promises and supported custom thenables own settlement and FIFO delivery across settlement; each introducing boundary's ordinary continuation closure supplies attribution, and no successful thenability state retains an operation context. `PoisonError` itself follows this supported thenable contract, but semantic Error recognition prevents kernel operations from probing it as an ordinary Promise input.
- Two causal boundaries consuming one custom thenable's raw rejection create separately contextualized occurrences. An access or invocation failure belongs only to the operation that observes it; the runtime does not cache it for unrelated later consumers.
- A `FatalError` in any ready, fulfilled, thrown, rejected, or nested host-result position is submitted unchanged to the current execution and never admitted, stored, or exported as language data.
- The single host-boundary design selected in Phase 9D-A recovers only the exact supported host action's failure and preserves thrown or returned language-result poison or contextualizes another expected reason. Its causal caller applies the graph effect outside recovery. If the bounded trial retained a private host-escape marker, only that marker reaches its one causal consumer; no parallel direct recovery path survives. Language-outcome callback bodies preserve poison only when their result contract admits it; adjacent transitions and callbacks whose contracts admit no Error outcome treat a poison escape as fatal. Protocol intermediates are not automatically language results.
...
- Required independent inputs all settle and combine complete cause/source/kind membership with unspecified Error order, without fulfilling an aggregate branch with a thenable Error. Successful input positions and effects retain sequential order. An unclassified or fatal rejection closes the operation.
- Every Promise producer and resolver affected by the cutover is audited, and every derived Promise is returned, owned, or handled; unused host input is not observed solely to suppress rejection reporting. No internal native Promise fulfills with poison.
- Preserve Phase 9D-0's successful-value invariant alongside the poison cutover: supported exact Functions/external identities behave identically on ready and pending routes, and a managed mutation cannot publish hidden callable/accessor `then`. Purpose-specific independent-result records still permit completed receiver publication without extending its protection through an unrelated Promise.

#### Graph, compounds, and import

- Pending Promise mirrors and fixed imported logical overlays use one placement-version map and common logical access. Fixed versions, including synchronously consumed custom-thenable outcomes, are never treated as pending.
- Compound construction rejects zero inputs, flattens nested compounds, shares cause/context/kind deduplication with `getErrors`, exposes an immutable child array with unspecified order, and derives `.kind` correctly without a stored `.kinds` projection.
- Existing Error queries preserve occurrence identity. `hasError` still exits early; `getErrors` and argument/export collection remain complete. Supported query reflection failure is poison; an internal traversal failure is fatal.
...
- Raw internal failures and existing `FatalError` values continue through Phase 9C's fatal lane; recoverable boundary migration adds no competing fatal state.
- Declaration APIs preserve existing Errors and use direct contextless thenability recognition without cached state. Contextless configuration validation returns ordinary synchronous host API Error and creates no execution, fatal Error, poison, or context sentinel.
- The `./integration` host-action, guarded-continuation, and outward-return routes established in 9D-A pass with rejecting poison; no higher-runtime facade normalization, private deep import, second re-entry guard, or extra outward registration is introduced.
...
The binding policy is settled: reject distinct discovered candidate paths to one identity within a root context during import; reject regular-Chain/off-path access locally; invalidate shared authority for competing otherwise-valid independent contexts. Register atomically after import validation and never rewrite earlier placements or revoke completed results. Implement [duplicate candidate validation](external-context-ordering.md#duplicate-candidate-paths-within-one-context) in this phase alongside registration; the Phase 9C addendum retains its existing inert tree scope.
...
### 1. Store durable binding state by identity

Use the existing execution `WeakMap` for identities registered in static trees. Each entry owns only the context binding/invalid-binding Error and its phase cursor. A valid binding holds the one registered leaf, which already identifies its root context and normalized path. Retain no candidate collection or first-use path-selection state. Do not retain the superseded cross-context first-use proposals, use-history state, separate conflict Boolean, reverse placement index, or cached current phase poison.

Commit context registration with the otherwise-valid initial import segment. A regular Chain's import, admission, or inert storage creates no competing registration and cannot poison an existing or later context. A competing independent root ContextChain registration makes their shared authority invalid, with an immutable `ExternalLocationConflict` Error. Both contexts may remain imported objects whose accesses now fail; no earlier root, placement, outward result, or completed host effect is rewritten. Error attribution to the detecting import is allowed to depend on import timing. Do not order independent imports or select a permanent first-arrival winner.

Stage every registration proposal until validation and discovery finish. Reuse one import-local identity-to-location proposal map to validate uniqueness: repeated discovery of one normalized leaf merges, while a second distinct leaf for that identity returns import-attributed `ExternalLocationConflict` and abandons the complete initial segment. Detect distinct occurrences through managed aliases and conservative scopes using the existing occurrence discovery; do not deduplicate them away with the admission visited set. Read synchronous custom outcomes through staged logical placements. Add no graph-wide alias scan, candidate retention, or first-use arbitration. An abandoned segment changes no prior entry, even if an earlier proposal in the same segment would conflict with an existing independent context. Commit all its ordinary import facts and registration outcomes in one hook-free transition. Registering invalid authority is a stored capability failure, not rollback of a previously completed import. Ordinary repair never clears that failure, chooses a location, or transfers authority.

At access, validate the complete exact selection against current identity bindings before any host work. A regular Chain or unregistered path fails locally without modifying the valid context binding or phase poison. A reserved provisional phase for an unauthorized/unselected boundary completes with unchanged predecessor state. Complete selection validation grants no partial host authority and performs no cross-context first-use commit.

Derive whether a recorded leaf is authoritative from its shared binding. Keep invalid leaves as inert discovery facts; remove conflict-driven tree deletion and add no reverse index or cleanup scan. Broad scopes select only their remaining valid leaves; explicit access to an invalid identity returns its binding Error and never falls back to unindexed observation-only behavior.

An operation delayed before host access rechecks current authority when it resumes. Already-invoked host code cannot be interrupted by later registration; finish ordinary boundary processing and rejection ownership, apply normal output validation, and reject subsequent unauthorized runtime accesses. Do not add a post-hoc failure merely to revoke a completed result. Earlier off-path use of a resource intended for contextual mutation is unsupported host usage even when its later registration was not yet knowable.
...
Retain one completion Promise per phase and subscribe through the common helper at each required position. Use ordinary native completion Promises as the baseline; reuse an existing supported sync-first completion primitive only if it already supplies the full FIFO contract across settlement. Do not introduce another primitive solely to optimize phase readiness. Consumers receive the hook-free state record through that protocol, never through an alternate `record | Promise<record>` cursor or a test of phase membership and queued phase work. A native predecessor subscription may return pending even after settlement; advance all available operation work synchronously when its continuation runs. An existing sync-first primitive may instead return directly when its own ordered delivery permits it. Every completion fulfills with a record equivalent to `{ poison }`, never directly with a rejecting-thenable Error. Completion cannot reject, so omit its rejection-only observer; potentially rejecting derived reactions retain ordinary ownership. Fatal failure follows the execution fatal path instead. Each successor captures its predecessor record. Observations after one exclusive phase share that exclusive predecessor and overlap one another. Every such observation consumes exactly the exclusive predecessor's poison; none consumes containment poison produced by another observation, even if that peer completed before the later observation was issued. Read groups collect all containment outcomes for the next exclusive successor using semantic Error deduplication with unspecified Error order; keep no issuance-ordered slots solely for arranging Errors. That successor waits for every earlier read group since the preceding exclusive phase and receives their final combined poison. This separates concurrent observation results from the durable quarantine that blocks mutation and makes visibility independent of Promise timing.

Fatal failure adds no phase-completion path. A successor that resumes after its predecessor completes reaches the common execution check and returns before host or operation work. A successor behind a never-settling predecessor may remain pending because fatal commit rejects every currently pending operation result through its registered outward rejection action. Fatal Error is never stored as repairable poison or in a phase record.
...
- Exact identity mismatch at a fixed leaf is a fatal invariant failure.

Phase 9E exposes no public repair. Its internal exclusive repair transition may bypass repairable predecessor poison and complete with caller-selected poison, but it cannot clear invalid authority, change a registration, create authority, or create a tree leaf. Repair failure uses the repair operation's context and `ExternalRepairFailed`; exact-identity mismatch is fatal. Phase 9F routes repair-only and repair-and-call.

### Verification

- Distinct discovered normalized paths to one exact external identity reject the complete initial import with `ExternalLocationConflict`, independent of discovery order. Cover direct aliases, aliases through shared managed containers, conservative scopes, and synchronous custom delivery. Duplicate or overlapping requests for one normalized boundary merge, including equivalent Number/String segments and opaque suffixes below it.
- Duplicate-path failure commits no admission, overlay, mirror, tree, or registration; pending callbacks from the abandoned segment cannot publish later. Existing context bindings remain valid even when the rejected segment also proposed a cross-context conflict. Inert aliases outside discovery remain allowed; actually pending delivery adds no leaf.
- Context registration is atomic with otherwise-valid import; regular-Chain aliases never claim authority. Permute independent context registration order and require invalid shared authority once both claims exist, without asserting a winning context or retrospective import rejection.
- Permute context/regular import order with inert, waiting, already-invoked, completed, and already-exposed regular uses. Future off-path host access fails locally, context phase poison remains unchanged, and no old placement or settled result is rewritten.
- Phase consumers retain FIFO subscriptions across settlement and validate current authority before deferred host access. Native predecessor delivery remains pending after settlement; an existing sync-first primitive may deliver directly only under its own full ordering contract. No direct-record shortcut exists.
- Complete exact-selection validation permits no partial host invocation. Failed or abandoned import segments invalidate no existing binding; committed competing context claims invalidate the shared entry.
- Invalid authority cancels no earlier host call or phase, preserves its shared Error, and cannot be repaired. A known invalid identity never becomes freely observable because its tree leaf is inert.
...
- Phase completions fulfill only with non-thenable state records and have no redundant rejection-only observer. Potentially rejecting derived reactions remain owned. Repairable poison never becomes a Promise rejection, preserves child attribution, and is changed only by repair.
- Fatal failure creates no phase record or terminal state. A resumed successor performs no host effect after its execution check; a successor behind a never-settling predecessor may remain pending without delaying any operation result.
- Every observation after one exclusive predecessor sees exactly that predecessor's poison, regardless of peer readiness. Peer-produced containment poison affects only its own observation result and the next exclusive successor, which receives all earlier read-group poison with complete semantic membership and unspecified Error order.
- `ExternalOperationContext` uses one selected-record map, stores repair intent once, and contains only operation-local coordination. Incompatible location selections fail before any host call, without creating an access-time shared conflict.
- Each durable identity entry contains only binding state and `phase`; repairable poison exists only in non-thenable phase-state records.
...
- Add `repairPath(chain, path, operationContext)` for repair-only. It targets an existing valid registered external location, creates no registration or authority, invokes no host code, and repairs no managed graph Error. Add it to Phase 9C's public facade and package-surface classification in the same change, so a pending repair result uses the common outward fatal-rejection registration while ready repair remains direct.
...
3. Create one `ExternalOperationContext` when indexed coordination is selected. Merge records by identity and let mutation win over observation. A valid context tree has one location per identity; validate exact selections against that binding without candidate-path choice or first-use arbitration.
...
5. Wait only as needed for phase predecessors and path resolution. Validate the complete exact selection against current bindings before host access. A later conflict is a shared binding Error; an off-path regular access is operation-local. Neither causes eager graph rewriting, and an unauthorized provisional phase relays predecessor state unchanged.
...
- Host code must not synchronously re-enter Cascada while its invocation remains active. Attempted re-entry is fatal. Independent work may start after the synchronous host call returns with its own explicit operation context.
...
- Initial synchronous custom delivery participates in that same discovery through staged logical overlays. Actually pending delivery creates no later leaf, and failed import leaves no latent subscription able to add admission or authority.
...
- Broad scopes select only live leaves; invalid or otherwise unselected identities grant no host authority.
...
- Consume a possible Promise segment through common Promise/Error preparation, preserving synchronous delivery, then normalize its fulfillment.
...
Centralize the identical direct-or-Promise consumption and String/Number validation. Observation and mutation keep their existing walkers because mutation also owns COW, writeback, gating, and failure publication. Both walkers follow the same one-time prefix-protection and resumption protocol.

### 2. Protect the first pending prefix once

Walk ready leading segments synchronously. A supported custom thenable whose consumed transition returns directly extends that same ready prefix; a completely ready path keeps its existing operation and synchronous behavior.

At the first possible Promise segment, consume it through the common helper and continue traversal in its callback. Each path selection has one local `pathSelectionComplete` fact, initially false and set immediately before invoking the selected target operation. Reuse an already-retained selected target only if its presence expresses exactly that handoff; otherwise retain the Boolean across this path's continuations. Do not wait for the target to return or settle. This fact identifies which work a pending result belongs to and is not a callback-ran or result-readiness flag. Only a returned pending chain while `pathSelectionComplete` is false requires the following protection synchronously before the issuing stack returns:
...
Initialize callback-visible staging, captured versions, and enclosing writeback required regardless of readiness before subscribing, because the callback may run inside `then`. Install pending-only protection after the subscription returns pending and before the issuing stack returns; JavaScript run-to-completion excludes later asynchronous delivery in that interval. Publish all required managed protection and external phase successors before waiting for any predecessor.

Resume every later segment from the same protected prefix:

~~~text
pathSelectionComplete = false  // one fact for this path selection

when handing off to the selected target:
  pathSelectionComplete = true
  return targetOperation(target)

for each reached segment:
  prepare callback-visible staging, captured versions, and unconditional publication
  result = consume the segment and resume the path in its callback
           (apply path failures where they occur; use the handoff above at the target)
  if !pathSelectionComplete and result is a pending chain:
    install prefix protection if no deeper path transition already did
  retain any owned pending reaction without an immediate consumer
  return result
~~~

The returned result determines whether that transition still has required work, while `pathSelectionComplete` determines whether it belongs to path selection or the selected operation. Do not track whether a segment callback ran or whether it backwrote a normalized key. A ready custom key followed by `pop()` of a pending element must leave the completed Array mutation visible immediately; only the removed element's independent result waits. It adds no prefix gate, lease, or provisional phase. An already-acquired prefix scope follows its ordinary capture or publication handoff and is not extended by an independent result after that work completes.
...
- Local operation closure in a live execution does not stop required gate or phase completion. Execution fatality adds no completion transition; a resumed waiter stops at the common check, and a never-resumed internal gate or phase may remain pending without delaying an operation result.

### 4. Protect possible external targets before waiting

On a context Chain with an unresolved suffix:

1. Query the ready prefix in the static external mutation tree. An exact boundary uses its ordinary phase and current binding validation before host access. Repair-only stops at that boundary and selects its repair phase without creating authority. Otherwise collect every live leaf the unresolved suffix may reach.
...
4. After resolution and selected predecessors, apply the consuming operation's normal terminal policy and validate current binding authority before host access. Rejected capability extraction and terminal Error inspection grant no authority. `readPath` inherits its containing policy. Repair requires an existing valid location. An unauthorized regular-Chain use fails locally and any provisional phase relays predecessor state unchanged; it never poisons the valid context.

Use the existing live-descendant tree query; add no candidate-path analysis or external index. Import has already rejected distinct candidate locations for one identity, including within conservative scopes, so each selected identity has one registered location.
...
- It contributes no mutation authority, predecessor poison, or operation Error unless resolution selects that boundary.
...
- A supported ready custom thenable segment extends the synchronous prefix without a lease, gate, phase wait, owner registration, or microtask. The same thenable pending installs exactly one prefix protection after subscription and before the issuing stack returns.
- Target handoff sets `pathSelectionComplete` before invoking the selected operation, even when that operation never settles; an independent pending result creates no prefix protection. Cover a ready custom key reaching `pop()` of a never-settling element, immediate visibility of the shortened Array, and later ready access through the prefix. Existing protection ends at its ordinary capture or publication handoff rather than following an unrelated result.
...
- Compiler discovery contributes the longest static String/Number prefix of a dynamic mutation path, so every possible mutation-capable boundary is present in a valid initial tree without putting Promises in compiler path facts. If that conservative scope discovers distinct paths to one external identity, initial import fails even when the eventual dynamic key would select only one. Repeated discovery of the same normalized boundary still merges; resolution chooses no binding path.
- Before waiting, use the ordinary phase for an exact reached boundary or exclusive provisional phases for uncertain live leaves. Validate current authority at deferred host access; reservation itself creates no registration or authority. Repair-only stops at the boundary.
- Resolution never expands the phase set. Exact external mutation requires a selected live leaf; an unindexed identity remains observation-only. Repair-only requires an already selected fixed location and cannot establish authority; repair-and-call retains ordinary call validation.
- Unselected provisional phases carry no authority or new Error but complete in predecessor order with an unchanged non-thenable poison record. Exact authority is validated before host access; phase FIFO is preserved.
...
The Phase 9C addendum already makes import subscriptions, staging, abandonment, and external discovery correct for synchronous and pending custom delivery. Its rewrite also completed the proposed structural simplification: `import-preparation.js` owns the fulfillment-segment processor and calls the ordinary property-version commit, with no installer callbacks or property-to-import delegation loop.
...
- Treat the Chain/operation-context pairing as trusted compiler/runtime protocol. Scripts cannot supply it directly, but losing a required operation context is always a fatal runtime defect, including on otherwise-successful paths. Reuse Phase 9D-A's narrowly justified operation-entry checks rather than adding checks to all generated commands and helpers; verify that compiler lowering passes the required operation context through all routes. A cross-execution Chain binding or closed entered Chain with a supplied operation context deliberately fails that execution before graph access. A missing-operation-context fatal created before an execution can be selected follows 9D-A's private diagnostic exception, never a fallback execution. A future host API may validate its application inputs, but must not turn omitted runtime operation context into ordinary application poison or add another execution-selection path.
...
- Do not cancel or specially settle native Promises, gates, phases, or synchronous host code. A never-resumed internal wait may remain pending without delaying any pending operation result. Section 3 defines the sole additional host-effect exception: local finalization of an iterator the operation already owns. It does not authorize command dispatch, graph transitions, phase/gate settlement, owner sweeps, or cancellation after fatal.

### 2. Replace the legacy recoverable representation

- Replace `PoisonedValue`, `PoisonErrorGroup`, `RuntimePromise`, `RuntimeError`, `RuntimeContextError`, `valueWithOrigin`, `createPoison`, `poisonOrReport`, `rethrowPoisonOrReport`, `poisonOrReportedFatal`, `poisonOrRethrow`, `isPoison`, `isRuntimePromise`, the broad semantic `isError`, and compiler-generated wrapper assumptions with kernel `PoisonError`, `CompoundPoisonError`, `FatalError`, precise predicates, and the two continuation contracts. Remove poison markers and any resolved-value marker used only for detection; retain no forgeable Error marker or compatibility Error name.
- Adopt the kernel's supported-thenable contract directly. Cascada's `ResolvedValue` demonstrates direct callback return; its legacy `PoisonedValue` requires replacement because it converts a callback's thrown poison instead of letting the throw escape. Use the conforming kernel `PoisonError` without porting that catch. Retained custom thenables must preserve subscription order across settlement, propagate synchronous callback throws, and return a supported chain for pending delivery; verify these properties rather than asserting unchanged legacy compatibility. Retain `ResolvedValue` only at higher-runtime positions that genuinely require a Promise-shaped result despite immediate readiness, and otherwise return the ready transition result directly. Replace `RESOLVED_VALUE_MARKER`, unwrap checks, callback-ran flags, and resolution backwrite tests wherever they serve only to distinguish ready from pending; after consuming a value, the returned result itself makes that distinction. Preserve backwrites that carry actual language data or scheduler state. `RuntimePromise`'s origin-retention responsibility moves into each causal boundary's already-required continuation, which captures its operation context and contextualizes a raw rejection before a later consumer can reattribute it. Replace every `valueWithOrigin` site with that existing continuation path before deleting the wrapper; merely returning its raw native Promise is insufficient. Subsequent semantic operations use their own operation contexts for new failures. An ordinary native Promise supplies the scheduling contract and remains the fallback for a host implementation that cannot provide custom chaining. Keep single-consumer scheduler tokens such as `IteratorWaitToken` internal: they do not satisfy the reusable subscription contract and must not cross the kernel data boundary. Add no adapter, cached sampling, canonical Promise, or second FIFO queue in either repository.
- Rebuild `collectErrors` and `collectThrownError` on the kernel's complete-collection and Error-query contracts rather than wrapper or generic thenable inspection. Rebuild `peekError` and the `#` operator as the diagnostic-view boundary: healthy input returns `null`; poison returns or fulfills with one immutable, non-thenable plain diagnostic view whose `.errors` contains child views and whose cause field contains only a safe diagnostic summary, never a `PoisonError`, native Error, or exact hostile cause. Preserve machine-readable `kind` independently of bounded `message`/`fullMessage` presentation. A kernel `QueryReflectionFailed` outcome propagates as poison through Cascada's `is-error`, collection, and peek paths; it is never coerced to Boolean `true`, inserted into a collected Error list, or converted to a successful diagnostic view. Keep `poisonIfNaN` and `handleLoadFailure` only as thin higher-runtime causal boundaries that construct or preserve direct poison.
...
Audit `runtime/call.js` (`callWrapAsyncInternal`, environment calls, and their wrappers), callback/extension dispatch, iterator acquisition/advancement/close, and render-owned loader hooks as host boundaries. Replacing their catches with factories is insufficient: prepare/export inputs, invoke only the exact host action through the shared envelope, check authoritative fatal state there, and perform result admission or fixed-kind rejection conversion in the existing guarded completion work. Keep compiler-generated macro/runtime callbacks in their declared trusted callback roles rather than treating every callable as host code. A host protocol result such as an iterator step record is validated/selected by that protocol before its language `value` enters admission; arbitrary protocol objects are not direct language results.

Remove the raw `.apply(executionContext, args)`/implicit `context.ctx` receiver path for standalone host Functions. A standalone host call uses `undefined` as its supplied `this`; code requiring the old context receives an explicitly declared receiver or argument exported through the common boundary before invocation. Document this API change and migrate built-in environment adapters at the same time. If an API explicitly declares a receiver, export its managed context snapshot alongside the explicit arguments, or select an exact external receiver through ordinary authority and phases. It must never expose a live unexported context by an implicit calling convention. Do not infer managed mutation or external authority from a host Function's use of `this`.
...
- Keep the shared host-code re-entry prohibition during that finalizer. Its deliberate post-fatal semantics differ from ordinary host-result processing: capture and own the close result/rejection without replacing, re-reporting, or waiting to deliver the authoritative fatal. Implement this one local boundary explicitly, not as an `allowAfterFatal` flag on ordinary graph/host operations. If the normal envelope cannot retain a close Promise before propagating the authoritative fatal, factor out its exact synchronous host-entry guard for reuse by this adapter and expose that narrow primitive through `./integration`; keep the post-host fatal classifier on ordinary calls and verify that no other caller uses the finalization exception.
...
- Standalone Function, environment method, extension/callback, iterator, and render-owned loader routes use the shared host guard and guarded continuation. A host action that catches its attempted kernel re-entry and returns success still yields the execution's first fatal. Cover ready, synchronous custom delivery, and pending completion. Standalone calls supply no raw context as `this`; declared context inputs are exported snapshots, and exact external receivers require ordinary path authority. Runtime-created failure payloads and successful results obey the 9D-A/9D-0 boundary contracts.
