...
Phase 9C, its supported-thenable addendum and completion step, and Phases 9D-0 and 9D-A are implemented. Phase 9D-B is next: preserve ordinary graph Errors, add primitive expression extraction and a separate rejecting value, and supply the public higher-runtime API. Phase 9E then builds the external coordination kernel before Phase 9F routes operations through it.

## Current checkpoint and agreed changes

The current kernel has ordinary FIFO subscriptions, atomic import and index publication, successful-output native-then validation, immutable causal Error wrappers, complete semantic Error collection, one local close transition, and a trusted integration surface. Poison remains ordinary non-thenable Error data; 9D-B supplies the expression boundary and normalizes query failure results. External entries remain inert until Phase 9E.
...
Recoverable graph **Errors** are ordinary non-thenable PoisonError and CompoundPoisonError instances. Ready and pending normalized kernel results carry them as data after required processing. A separate PoisonedValue supplies rejecting transport at primitive expression extraction and explicit higher-runtime failure propagation. Successful Error inspection remains data; collection membership, source attribution, and publication do not depend on expression transport.
...
| Ordinary graph Error results, primitive expression extraction, and a separate rejecting expression value | 9D-B |
...
- All four deliverables pass. The recoverable kernel work is implemented in 9D-0/9D-A; expression boundary work and higher-runtime integration remain assigned to 9D-B and 13.
...
The causal host, Error factory, and guarded-continuation implementations share one composition surface. At the 9D-A checkpoint this surface is the integration subpath; 9D-B promotes the required facilities to the public root, keeps unwrapped graph operations private, and removes the subpath and its consumers. The implementation remains shared, with no duplicated factory, guard, private host-depth protocol, or transport mode.
...
- Supported query reflection failure is the query operation's QueryReflectionFailed outcome, not a found graph Error. hasError does not answer true and getErrors does not insert it into the successful Array. Phase 9D-B removes the checkpoint's pending-query rejection wrapper so ready and pending query failure both complete with ordinary Error data. Internal traversal, indexing, and bookkeeping failures remain fatal.
...
**Status: Not implemented. Next phase.** The source and tests are at the completed 9D-A baseline. Implement this phase as one coherent change before 9E. Architecture documents specify the final contracts below; the existing query rejection wrapper and integration subpath are explicit removal work in this phase.
...
- Normalize raw host and expression rejections once through their existing causal input-consumption continuation. Preserve an already contextualized Error by reference. Import, Chain initialization, assignment, path segments, arguments, and host results consume a supplied PoisonedValue through its supported synchronous rejection delivery and publish the contained Error, never the wrapper. No PoisonedValue becomes an admitted graph identity, fixed placement, pending mirror, or refcounted payload.
...
- `getErrors` succeeds with a flat deduplicated Array of ordinary leaf Errors, or an empty Array. Finding Errors is successful inspection. It completes every required captured branch. A query failure is a separate Error result. `hasError` retains its existence-only early completion. Do not make either query reject merely because the graph contains Errors.
...
- Keep complete collectors and their existing readiness/result storage when that state represents required work. Ordinary Errors can safely fulfill Promises; no record or wrapper is needed solely to defeat Error assimilation. Phase completion records and mutation receiver/result records retain their distinct ordering and effect semantics. Do not introduce a general result algebra, transport policy flag, or second continuation engine.
...
### 3. Implement public primitive extraction

Add `lookupPrimitiveValue(chain, path, operationContext)` alongside `lookupPath`. Reuse the existing observation walker, property versions, supported input consumption, execution checks, and boundary classification. It is a shallow terminal validation of the selected logical value, not another walker or an export operation.

- Accept exactly null, undefined, String, Number, Boolean, BigInt, and Symbol primitives. This boundary performs no coercion or numeric validity check; Cascada operators own their accepted operand combinations and NaN policy. Boxed primitives, Functions, Arrays, records, class instances, and external identities are non-primitives.
- Preserve a selected existing Error. For another non-primitive produce a causeless `ExpectedPrimitive` validation Error attributed to this extraction's operation context. Add that contract-based kind to the authoritative kind table. Do not retain the rejected object as a diagnostic payload, invoke conversion hooks, read its contents, deep-export it, or await unused descendants. The failed observation does not poison or replace its source Chain.
...
Factor only the small repeated Error-to-expression conversion needed by this API and higher-runtime completion. Keep normalized graph result return and expression materialization explicit; do not add a generic transport mode to graph operations or alter the default fatal-on-escape guard to permit an intentional outward rejection. A boundary may return a PoisonedValue for native assimilation without throwing a language Error through a trusted fatal envelope.
...
- Every primitive, non-primitive rejection without conversion hooks, missing final placement, failed prefix, raw rejection, existing leaf/compound, and fatal value through ready, synchronous-custom, and pending primitive lookup. Add pending path cases when Phase 10 supplies them. Validation leaves the Chain unchanged and does not wait for an unused nested Promise.
- Public import, Chain construction, assignment, arguments, and host-result consumption accept expression wrappers and rejected expression Promises, preserve cause/source/kind and Error identity, and store only ordinary Errors. Ready wrappers add no microtask, mirror, gate, or pending-only lease. Two causal uses of one raw rejection retain distinct sources; propagation of an existing Error does not.
- Graph lookup, mutation, invocation, export, and query recoverable outcomes remain ready Errors or Promise fulfillments with Errors after required work. Query failures are distinguishable from successful Error arrays; healthy queries return false or an empty Array. Complete queries and input/export collection wait for controlled failing siblings and retain full membership regardless of settlement order.
...
- Use the public kernel PoisonError, CompoundPoisonError, FatalError, Error factories, and separate PoisonedValue factory/predicate. Remove duplicate Cascada Error construction, grouping, wrapper, fatal, and recognition implementations after their callers use those facilities. Keep ordinary Errors inside operation Chains. Compiler lowering converts graph results to expression values only through lookupPrimitiveValue; expression-created failures use createPoisonedValue on an attributed ordinary Error. Remove compatibility names and markers rather than adapting their old shapes.
- Adopt ordinary native Promises and supported ordered sync-first thenables. The expression PoisonedValue returns its rejection callback result directly and never converts a thrown Error. Keep ResolvedValue only where an actual promise-shaped success is required; remove marker-based readiness normalization, deep-resolution markers, and graph finalization from expression helpers when their consumers move to primitive-only inputs. Retain actual captured graph/scheduler state. Replace RuntimePromise and valueWithOrigin attribution with the existing causal boundary continuation and its explicit operation context before deleting them. Pending rejection must be attributed where produced, not at a later expression or Chain consumer. IteratorWaitToken stays an internal scheduler token. No second subscriber queue, canonical settlement Promise, or transport adapter is introduced.
- Rebuild complete expression-input collection using public Error recognition/combination and the ordinary Error returned by rejecting expression values. Resolve every required independent operand before combining recoverable failures; neither Promise.all on rejecting operands nor sequential early-abort await satisfies this rule. Use public getErrors for complete graph inspection and hasError for existence. A successful query yields ordinary data; QueryReflectionFailed stays a failure and is never truthy proof or a collected leaf. Rebuild peekError and # as diagnostic-view boundaries: healthy returns null, failure details return an immutable safe view with complete child/kind membership, and query failure propagates through expression conversion. Diagnostic objects enter operation Chains; only selected primitive fields enter expressions. Expression-owned producers such as poisonIfNaN and handleLoadFailure use the public attributed-Error and expression factories at their causal source.
...
### 2A. Separate operation Chains from expression values

Lower graph lookup, calls, assignment, deletion, Array work, and graph transfer to public Chain operations. Retain a whole branch or a call result in another Chain with ordinary ownership/COW; never inspect a managed source from generated native code. All results entering primitive expression evaluation pass through lookupPrimitiveValue, including nested calls, condition operands, dynamic path segments, and values selected by conditionals. Do not infer the route from whether today's value happens to be ready or primitive; the compiler chooses it from the consuming operation.

Expression helpers consume only the documented primitives, pending primitive results, and ready PoisonedValues. Preserve synchronous success and rejection shortcuts. Ordinary thenable rejection can skip a successful arithmetic continuation, but complete independent operand collection must retain every required failure. Preserve laziness of &&, ||, conditional branches, and other semantically unselected operands. No graph walk, resolve marker, COW mutation, or native object coercion belongs in an expression helper.

Operators creating a new failure retain their own source and kind and use the public Error/expression factories. Storing an expression result in a Chain passes it through ordinary input consumption, preserving an existing Error and removing the container. Cascada must not wrap a contained Error again or add attribution to the wrapper.

Migrate current operations that can consume objects explicitly: membership and collection operations run through the appropriate Chain operation; host calls/callbacks use the existing exported-input boundary; diagnostic # views become managed Chain data whose fields may be extracted as primitives. No object-valued truthiness, identity comparison, template coercion, or custom conversion hook may silently bypass the primitive contract. Retain such behavior only in an explicitly supported Chain/native boundary operation; otherwise produce its documented validation Error. Test compiler routing as well as result values.

Error queries remain successful inspection. getErrors returns the complete leaf Array, hasError returns Boolean, and query failure remains distinguishable. A result used in a primitive expression crosses ordinary primitive extraction; diagnostic objects and Error lists stay outside primitive evaluation. Explicit propagation combines a successful Error list and calls createPoisonedValue. Do not change getErrors into an implicit throw operation.

Final render/script output may be a graph. Complete public export, including nested availability and all required Errors, before final delivery. A failed export becomes outward native rejection using the same expression failure container or the native settlement reject action. Keep this deliberate conversion outside the trusted fatal-on-escape body; preserve exactly one final render obligation until its true outward settlement, without duplicating an already delegated kernel result. A ready failing boundary can return PoisonedValue synchronously; legacy always-Promise host facades may assimilate that completed result. Native callback adapters deliver the ordinary Error, not the container.

Add compiled integration coverage for ready/pending primitive lookup; object and Function rejection without coercion; call-result Chains; pending dynamic paths; primitive field access on diagnostic views; ready/pending arithmetic failures; both failing operands in reversed settlement order; skipped short-circuit branches; assignment of an expression failure back into a Chain; and final graph export with multiple nested failures. Assert exact source and native cause through each round trip, unchanged source state, and no extra pending registration for ready poison.
...
- Document normalized kernel results as T | PoisonError | Promise<T | PoisonError>, primitive expression results as Primitive | PoisonedValue | Promise<Primitive>, and successful Error queries as Error arrays with a separate query-failure Error result. Expression and final native rejection reasons are ordinary Errors. Final script return accepts fully exported graphs as well as primitives and applies explicit failure conversion after export.
