...
Phases 9C through 9F-A supply the implemented graph/Error, admission, binding, managed rollback, and hierarchical external coordination foundations. Phase 9F-B is next: consolidate placement transitions and capture ownership without changing their semantics. Phase 10 then adds Promise-valued path segments, Phase 11 verifies import preservation, and Phase 13 integrates Cascada.
...
| external-access.js, external-snapshot.js, import-preparation.js, import.js, export.js | Preserve boundary classification, native snapshots, complete collection, atomic admission, and borrowed-result isolation; validate nested registered identities along their selected route. |
...
Preserve placement presence through entry setup and publication. Initialize a missing target as an absent private root placement; normal reads yield undefined, while assignment of undefined establishes presence. A delayed no-op callback must not create a missing record key or Array element. Reuse ordinary placement/version transfer for presence, value, source version, and recovery state; add no entry dirty flag or comparison against the initial value. Extend capture/fork/transfer, logical traversal, publication, and index maintenance together so absent completion cannot become a present undefined edge. Deleting an entered root removes its selected placement, preserving Array holes and length; ordinary Chain-root deletion still produces null. Test record keys, in-range and out-of-range indexes, nested entries, and pending consumers.

Detach settled absent versions from live owner metadata after their last already-issued writer finishes. Readers retain captured versions independently. Bound retained owner slots to actual unfinished publication dependencies, without deferred cleanup queues or settled-placement history. Verify repeated absent entries, COW copies with captured consumers, and queued prefix failure followed by repair for record properties and Array holes. Test replacement/deletion behind a gate that publishes never-settling data, multiple queued replacements with intermediate observations, and failed replacement retaining that pending publication for repair.

Select structural protection before installing a mutating entry gate. An out-of-range Array index uses its owning Array's ordinary structural gate; existing in-range elements and holes keep local protection. Keep the working Array as the entered Chain's private root and retain the requested element as a relative root path. Determine whether an operation replaces its scope before rebasing its captured path. Common route capture prepends that path and adjusts operation scope depth, keeping context-tree coordinates at the backing root and source-static provenance at the exposed reference. Entry-target lowering returns paths relative to the caller's Chain. Run each contained command through the ordinary Array selection/publication path. Creation followed by deletion preserves growth in the working Array itself. A failure before creation poisons the Array and blocks later element replacement, deletion, or repair; after successful creation, failure confined to the element stays element-local. Publish the latest working Array through the ordinary entry gate at closure. Never grow length merely by installing a gate. Length/whole-Array observations and later structural mutations wait for the out-of-range entry and see its completed shape. For an in-range hole, ordinary version transfer must carry pending presence: export, logical enumeration, complete receiver preparation, and controlled Array callbacks must distinguish absent completion from an explicit undefined value. No callback runs for an element that remains absent. Presence consumers follow publication completion without awaiting ordinary published data, including through transferred and nested gates. Dense Array copies map source positions before waiting for retained presence decisions; discarded positions introduce no dependency, and retained absent positions become explicit undefined. Retain receiver and payload leases until delayed copying establishes output ownership. Reuse path COW and structural publication without an entry-specific Array representation, callback-wide baseline, or undo log. Preserve the relationship to the structural owner through contained mutation and repair so required Array poison is not published as a length-growing Error element. The wider gate grants no broader mutation/reference authority and must preserve fixed external locations.
...
- While an out-of-range entry is pending, issue length/whole-Array observations and structural mutations. They wait for the final shape; no placeholder length is observable. In-range elements and holes keep placement-local ordering. Check no-op hole completion versus explicit undefined in export, logical enumeration, indexing, and controlled Array callbacks; no callback runs for a hole that remains absent. Exercise shared/imported ancestors, ArrayViews, fixed external siblings, and deferred root publication. Read-only absent-target entry creates no placement or structural gate. Cover pending owner work followed by a widening operation and a third owner operation, with intermediate captures and without inserted observations. Compare direct, entered-element, entered-owner, and nested routes across three-command failure, repair, creation, and deletion sequences; verify replacement/deletion can discard never-settling ordinary data. Include private/shared/leased/imported Arrays, ArrayViews, supported Proxy storage, and an inner element entry outliving its outer callback. Reject Array-wide poison for a later element-only failure after creation; preserve original Array poison when the element never existed. While dense copies await presence, mutate a retained sibling or payload through another operation and verify the earlier output stays unchanged. Cover ready, synchronous, and deferred payloads, copied/nested gates publishing pending data, discarded unresolved positions, explicit undefined versus holes, and lease release before ordinary element data settles.
...
**Status: planned.** Complete this bounded consolidation before Phase 10 extends the path walkers. Preserve the implemented 9F-A behavior and fixes; this phase changes internal responsibilities, not public semantics. The required result is fewer independent implementations of the placement protocol and explicit capture ownership independent of operation-lifetime inference. Particular helper extractions and the placement-outcome rewrite remain experiments. Rejecting an experiment does not waive a required contract; if no simpler implementation meets the required outcome, report the remaining responsibility and discuss the concrete tradeoff rather than force an abstraction or mark the phase complete.

### 1. Map responsibilities and establish the shared contracts

Inspect every constructor, writer, transfer, and consumer of placement state, including property-versions.js, import-preparation.js, mutations.js, observations.js, managed-traversal.js, entry, Array remaps/views, and boundary result capture. Include outcome producers and consumers in path-operation.js, run.js, external-access.js, managed-invocation.js, and array-invocation.js. Use a compact working inventory to identify who owns source consumption, complete state capture, retention, producer progress, installation, publication, and release. Distinguish duplication from necessary staging, boundary-validation, or lifetime differences; a direct field assignment in unpublished import staging is not by itself evidence of a second protocol.

Start with the current duplication: transferPlacement and continueMutationVersion assemble and advance versions; prepareManagedContainer in import-preparation.js separately forwards presence, recovery, transition, and producer facts; walkMutationPath writes captured version facts; continueCapturedPromiseVersion infers protection from an operation owner. Trace their actual callers before choosing shared helpers. Keep semantic placement contents, version-local control state, and identity metadata distinct: a fork needs its own version and publication authority, preserves its destination's capture obligations, and follows the appropriate source dependency. Copying a complete logical placement does not mean cloning every internal flag, source publisher, lease count, or installation fact.
...
### 4. Evaluate complete mutation targets and outcomes
...
Evaluate whether one complete placement outcome at publication boundaries removes mutatedValue/optional-placement fallbacks, duplicate state reconstruction, or callers that can omit presence or recovery. Do not force lower-level invocation to synthesize placement state when it naturally produces a receiver and an independent result. Preserve successful removal of Error-valued data, ordinary Error assignment, combined independent result/publication failures, and repair's atomic restoration semantics.

Retain the outcome rewrite only if it removes concepts or independent transitions across actual callers. Otherwise revert it and keep the existing representation with its established invariants. Keep the owner-rooted entered Array path and ordinary contained-command execution; add no stand-in element replay, callback-wide rollback, or special Array scheduler.

### 5. Evaluate each change and complete the cutover

Establish the responsibility inventory, contracts, and ownership regression matrix below before changing production code. Implement one complete consolidation at a time; capture ownership and version publication may change together when protection depends on both. Do not force a section order that requires temporary adapters or leaves protection until after publication. Gate each capture/retention change on the ownership matrix and each lifecycle change on the relevant publication, import, ordering, and liveness checks before proceeding. Evaluate the optional outcome rewrite after those contracts are established.
...
Implement [Promise-valued paths](promise-path-segments.md) through the existing observation and mutation walkers after Phase 9F-B establishes their shared placement and capture-ownership contracts. Reuse those primitives rather than redesigning them while adding pending keys. Managed paths and observation-only external paths support dynamic inputs. Mutable external resources require static selection; only their already-selected native suffix can contain dynamic input keys. This phase adds no candidate-resource reservations.
...
- The importer still owns fulfillment-segment processing and import policy; shared placement code owns its generic version lifecycle and commits prepared logical state. No installer callbacks, import-policy forwarding, property-to-import delegation loop, or duplicate processor have returned.

Keep [`import-preparation.md`](import-preparation.md) aligned with the retained Phase 9F-B boundary; update it here only if a newly justified change is retained.
