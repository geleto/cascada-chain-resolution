...
Phases 9C through 9F-B supply the implemented graph/Error, admission, binding, managed rollback, hierarchical external coordination, and shared placement/capture foundations. Phase 10 is next: add Promise-valued path segments using those mechanisms. Phase 11 verifies import preservation, and Phase 13 integrates Cascada.
...
- **External boundary/location:** a registered external scope on a static context route, including nested native scopes. Its Symbol-held record holds the canonical path and binding, with node identity distinguishing the context route; first native crossing and selected mutation scope are separate facts.
...
Extend compiler-guided initial discovery to requested native child data placements below an external node. A runtime node can contain both child keys and a Symbol-held external scope record. Keep canonical path, binding, reservation, and poison facts inside internal metadata rather than reserving property names. Continue through the finite compiler paths only; do not enumerate native interiors or add an endpoint-used flag.
...
Publish the entry's outer reservation before waiting, then wait for its captured predecessors and recheck selected binding and strict-ancestor external own poison before invoking the callback. The private view must not bypass those prerequisites. Poison at the selected target or in its external descendants permits contained inspection/repair; a strict-ancestor blocker or invalid selected binding skips the callback and completes the ordinary selection-failure path. Empty prerequisites add no Promise/microtask. Apply the same rule in nested views and both entry modes; do not recapture later managed namespace state or wait for unrelated managed descendants.

Use section 3's reservation turn. Each contained command with selected external coverage acquires its one reservation and entry lifetime membership at issuance, before managed gates can wait. Its completion waits for predecessors plus required local effects and boundary processing. Remove completed membership promptly. Callback closure stops issuance and captures unfinished reservations, including already-issued nested entries. Independent nested results do not extend access. Publish the managed root separately through ordinary versions, preserving ready sibling access while native child effects remain pending. Structural widening of an element entry's managed gate does not widen its native coverage.
...
- Queue read-only and mutating entries behind a pending external mutation; an immediate contained read must see the completed predecessor state. Repeat in nested views, with ready/synchronous delivery, predecessor failure at the selected scope, strict-ancestor poison, and selected-binding conflict. Target poison permits callback inspection/repair; ancestor poison and invalid binding prevent callback/native work. Verify selection failure releases ordinary reservation membership without adding descendant poison or taking later managed gates.
...
Entry is a reference and ordering optimization. Given the required entry coverage and operations through its entered references, moving a command into entry must preserve the corresponding direct command's values, presence, own-key order, Array structure, observable alias/cycle relationships, effects, Error kind and causal operation context, and repair behavior. It may let unrelated work proceed earlier; it must not invent a mutation, suppress conditional work, introduce a dependency cycle into covered work, or add callback-wide rollback. Compare both against the sequential execution of the same issued commands. A branch that issues no data command leaves the data unchanged, including when its requested reference cannot currently be traversed.
...
- Entry captures protection and a reference. Protection alone changes no logical data, presence, own-key order, Array length, or alias/cycle relationship. Actual contained commands retain ordinary managed value semantics, including path-local COW and its permitted alias separation. Multiple paths to one identity do not add an owner; that accounting rule does not require mutation through one path to update every alias.
- Preserve concurrency as well as topology. For a directly selectable managed placement, protection must leave unrelated sibling reads and mutations able to progress while entry remains open. Sharing, import protection, leases, or representation changes alone do not authorize a wider gate. Earlier owners remain usable independently. Do not trade these guarantees for a simpler topology mechanism.
...
Mutable entry uses ordinary placement gates while preserving earlier owners; gate installation is protection, not a logical mutation that authorizes extra alias separation. Ancestor representation copies must preserve the logical graph until a contained command performs the corresponding ordinary mutation. Readonly managed entry leases its captured anchor. External reservations cover the selected registered subtree and use the existing private reservation view. Contained operations receive no authority beyond their original paths. Preserve fatal-on-entry behavior, Chain/execution isolation, and readonly capability checks without adding validation of trusted compiler records.

A no-op entry at `a.x` in `{a: child, b: child}` preserves the exported `a === b` relationship. The same rule applies across different containers: entry at `x.a.k` in `{x: {a: child}, y: {b: child}}` preserves `x.a === y.b`, even though `y` is outside the copied path. Preserve both self-cycles and cycles spanning containers. This requires the same relationships within the observable graph, not export identity equality with the original host objects. Actual contained mutations keep ordinary COW semantics and may separate aliases exactly as their direct counterparts do; entry setup must add no separation of its own. Comparing only leaf values or final presence is insufficient.

Replacing a whole-Array out-of-range entry gate with an element gate must preserve aliases that the existing enclosing gate preserves; owner isolation alone is insufficient. Complete the protection and structural-publication evaluation below before that switch. Independent placement/publication corrections and the growth oracle need not wait. A no-op entry must not gain a new storage failure merely because protection introduced a fallible write to restore its unchanged source. Required source settlement and captures remain intact; final value equality alone proves neither a no-op nor absence of structural effects.
...
- Imported Arrays and imported backing remain physically unchanged, including length. A logical mutation may copy at issuance under ordinary COW; there is no zero-copy guarantee for no-op entry, but any representation copy must preserve its required logical topology. Shared views may reuse only storage that the existing representation permits them to change.
...
Use an ordered linked sequence of potential/committed growth contributions and pending length/range watchers as the implementation candidate. A growth contribution records its bound and shares the source's one creation-or-no-growth outcome. A watcher records a position and the question it asks. Ready questions return synchronously without a watcher. Ordinary index assignment can contribute committed growth; it does not create an unresolved entry contribution.
...
Do not require aggressive deletion of dominated potentials across range watchers. First measure retained state and update work after the bounded cleanup above. Evaluate more selective pruning only if those costs remain material, prove that it preserves each captured answer with the oracle, and revert it if its bookkeeping costs more complexity than it removes.

On COW or whole-value capture, preserve the source's current length knowledge and its unfinished source contributions at that capture position. The destination has its own future sequence and publication authority. Do not copy operation-specific watchers into it or share a mutable tail that admits later source operations into an older capture. Existing watchers retain the original view; copied contribution nodes may share source outcomes as ordinary placement versions do. Reclamation in either view cannot remove an outcome still needed by the other.

The max calculation applies between ordered structural transitions that can shrink or remap the Array. Such transitions retain the existing Array-wide gate and captured-placement rules. Capture affected logical placements through the common candidate layer, including installed versions beyond the published minimum or physical length. A truncation to `n` must account for unfinished placements at indexes at least `n`; scanning only the current physical/published range or unresolved growth contributions is insufficient. Enumerate candidates, not every integer up to a predicted maximum, and preserve ordinary pending payload without waiting for values the mutation does not consume.

Starting with `[0, 1, 2]`, entry at index 5 followed by `length = 1` must leave `[0]`, even if the entry creates its element only later. Also cover creation already committed to length 6 while the element gate remains open: exact length does not establish transition completion. Capture conflicting placement transitions before suspension and honor them through publication; later source publication cannot resurrect truncated elements or growth in the resulting Array. A length/range proof may remove a shape wait, but does not by itself remove an effect dependency, even when the requested length is at least the captured maximum. Reuse ordinary gates and captured publication authority, not a second effect scheduler or a shrink contribution in the max sequence. Root replacement and rollback retain old captured histories while establishing their own resulting Array state.

Implement incremental updates over the affected frontier; do not rescan the Array or keep settled history merely for convenience. Callback order among genuinely dependent continuations follows the existing guarded FIFO machinery. Verify the sequence and bounded reclamation with the oracle specified in the implementation evaluations before integration.

### 5. Keep logical placements authoritative independently of physical storage

An undecided Array placement is part of the logical value even when it is beyond the known minimum. Install its ordinary gate version in the logical owner's `placementVersions`; do not reserve physical backing tail, attach a special view, or extend physical length merely to store uncertainty. Protection must both isolate earlier owners and preserve the topology required by section 1. Use the verified protection/publication mechanism from that experiment; copying the Array merely to isolate gate installation does not by itself satisfy entry equivalence. Switch out-of-range entry to element gates only together with, or after, that mechanism.

Physical backing length is storage information, not the maximum for a captured length question. It may include another view's storage, later reservations, or capacity left by a no-op. Length bounds come from the ordered captured growth state. No extra tail-owner flag, second ArrayView window, or storage-shrink cleanup is required for pending entry. The ArrayView's private storage bounds do not override committed logical length.

Re-evaluate ArrayView attachment, extension, and derivation eligibility against the captured logical surface. Existing physical-end checks alone are insufficient when logical length differs from storage: appending one element to logical length 6 with physical length 3 must produce length 7, not a window of length 4. Reuse backing only when its bounds and placement overlays represent the required range without losing logical state or exposing another view's retained tail; otherwise use existing materialization. Keep logical length on the native Array owner when attaching its projection. A separately published view owns its result's length and dependencies, derived from the selected operation rather than blindly copying the source baseline. Apply this rule to empty suffixes and zero-count operations too. Do not expand ArrayView with another window or storage mode merely to preserve a fast path.

Centralize logical-placement candidate capture as the union of ordinary stored candidates and installed logical versions, whether pending or settled. Deduplicate keys and preserve ordinary reflection/error handling and logical enumeration order. An installed version supplies value, presence, transition, and recovery without requiring a physical descriptor. A candidate is not proof of settled presence: an undecided placement remains a required dependency, known absence remains absent, and a resolved `undefined` with present=true is still a property. Keep this rule in the shared property layer rather than making each query or copier supplement physical enumeration itself.

Make `hasLanguageProperty` independent of physical gate storage before removing those writes. For an ordinary placement, consult its installed version on the logical owner before physical fallback: `present === false` means known absence; otherwise the version identifies a present or undecided candidate (`version.present !== false` in the existing representation). Only a placement without a version falls back to its physical descriptor. A true candidate answer does not establish final presence: presence-sensitive consumers resolve the captured placement transition before treating it as an element, while ready-only consumers reject unresolved presence. Preserve ordinary key classification and intrinsic-property behavior. Reuse `PropertyPlacement` capture and presence resolution rather than adding another presence representation or consumer-specific physical fallback.

Audit the callers together with this change: `getPropertyPlacement`, lookup/mutation selection, logical traversal and receiver preparation, the controlled Array remap `has` trap, searches, and `join`. An undecided hole must remain a captured dependency even without a physical slot; after completion each caller applies its ordinary hole versus present-undefined semantics. Land the common predicate/capture correction and its caller checks no later than removal of physical gate mirroring, not after metadata-only entry has been integrated.
...
- Exact length: public length reads, negative-index calculations, append positions, shape-sensitive bounds, and intrinsic methods when their result/effects genuinely require it.
...
Complete consumers finalize from captured structural state, never current live length. The shape-completion evaluation below determines whether their ordinary required traversal already proves exact shape, allowing export and complete managed receiver preparation to omit a separate length watcher/Promise join. Output discarded because of Error needs no shape completion; any output-only watcher must release without leaving the Error-collection result waiting on its abandoned internal Promise. Do not solve this with cancellation of shared growth sources. Length-only observations and bounded consumers may still need watchers to obtain answers before complete element traversal. Internal captures that deliberately preserve pending data retain unfinished shape instead of waiting.

Native receiver readiness also requires physical representation of the complete logical surface. Resolved values alone do not establish this: an absent physical property and a logically present undefined compare equal by value, an absence overlay may hide physical data, and logical length or key order may differ after all Promises settle. Extend observational receiver materialization to account for presence, Array shape, and record order, including nested containers. Reuse exact native storage only when it represents that logical surface; otherwise copy affected containers through the existing materialization path, preserving aliases/cycles. Use existing placement and structural facts rather than per-method fixes or another registry. Complete mutation isolation must copy the same logical surface. Establish this eligibility before introducing each new overlay or structural state, independently of the proof that traversal has finished waiting.

Internal import/result capture and COW can preserve unfinished length knowledge as managed state; do not delay a host method's independent result merely to allocate such a copy. Preserve metadata-only undecided placements in these logical copies rather than installing physical placeholders that would inflate their published minimum. Native managed invocation still waits until its complete receiver is ready. Error queries and indexing need all relevant logical dependencies but do not acquire an exact-length wait when length cannot affect their answer. Export finishes complete captured shape before delivery; mutable-external property snapshots require it to be ready synchronously and reject unresolved state under section 5 rather than waiting.

Audit callers that never read length as well as existing length call sites. Ready/pending presence handling, sparse enumeration, ArrayView iterators and descriptors, refcount oracles, retained remaps, and physical growth paths are part of this sweep. Reuse existing logical traversal and placement publication rather than adding a second Array-only boundary walker.

### 7. Preserve external authority through runtime anchoring

Retire public `selectEntryPath` and its compiler handoff once entry performs shared reference capture. Keep the user-visible reference at the requested target. Reserve the appropriate static external coverage before managed path suspension, using the common external reservation view and predecessor capture; contained commands reuse that view.
...
An entered native leaf has no independent poison/recovery state. Assignment/deletion of an ordinary native property remains allowed; replacing or deleting a fixed registered binding remains forbidden. Contained operations cannot bypass selected or ancestor poison or invalid bindings, and copying or rebasing a reference creates no repair authority. No-op reference setup adds no new data poison or host effect. Replace the eager `externalBlocker(false)` / binding-validation exit and route-failure mutation publication that currently suppress callback invocation in `enter.js`; actual commands retain their ordinary access checks and failure effects. An invalid dynamic suffix grants no authority to reserve or use a guessed native target. A malformed trusted control record or unexpected runtime failure retains the ordinary fatal boundary.

### Implementation evaluations

The requirements above are settled. These evaluations select mechanisms and prove optional simplifications; they do not permit weakening entry equivalence, sibling concurrency, earlier-owner independence, or ordinary mutation semantics. Compare total concepts, state, lifetimes, graph work, ready/pending costs, and superseded code removed. Keep prototypes separate until their real callers can use them without adapters or unused machinery. Revert changes that merely move complexity, broaden protection to avoid copying, or add a second scheduler, repeated graph scans, or retained history.

#### A. Protection and structural publication

**Question:** can common placement capture/publication preserve topology and sibling concurrency during protection, and convey contained creation/deletion to the captured container without an entry-specific journal or consumer-repair mechanism?

Start with the identity decision faced by a capture made while entry is open. Given `{x: {a: child, b: child}}` and another owner retaining `child`, enter `x.a.k` and issue export of `x` before deciding the callback's work. A no-op must preserve `a === b`; a contained write that requires ordinary COW must produce the same separated aliases as the direct write. An eager capture of either the original child or a protection copy cannot simply fix that relationship before the command is known. Any candidate must show how common capture/publication delivers the correct identity at that capture position without delaying unrelated siblings or rewriting an already delivered output. This counterexample rejects premature identity commitment; it does not prove that all representation approaches require wider gates.

Evaluate separating protection from logical mutation through the existing capture/publication path, including discarding unnecessary COW copies only if that decision also reaches already-issued captures correctly. Reuse an original only when the copy represents the same captured logical value, with no committed logical change or consumer requiring its separate identity. Preserve presence, recovery, Array structure, and the captured Promise frontier. Decide per copied node through its captured publication authority; never restore a remembered whole ancestor. Independent sibling writes, root replacement, actual contained commands, and committed growth followed by deletion must remain intact. Derive reuse from common transition facts, not deep equality, a callback-wide dirty flag, or an execution-wide copy history. Add no separate identity registry or consumer-repair path merely to rescue eager copies.

Include captures while entry is open from the first prototype, using both `{x: {a: child, b: child}}` and `{x: {a: child}, y: {b: child}}`, self-cycles, and cycles spanning containers. An earlier owner must remain usable while the entry is open and receive neither its waits nor its writes. Reject a candidate as soon as it fails one of these obligations. Redirecting aliases only within copied containers does not establish the cross-container guarantee; installing gates directly on the shared source does not isolate owners. Restoring the live root alone cannot repair an export or another Chain that already captured a copy. Earlier consumers must receive the correct topology at their capture position without rewriting delivered outputs or adopting later effects. Test nested entry, pending publication, actual mutations, and source settlement. Protection must also work when optional target inspection cannot consume an unreadable suffix.

Include concurrency in those same tests from the first prototype. Hold a narrow entry open, begin a whole-value capture, and require unrelated ready sibling reads and sibling mutations/entries to progress before it closes. Repeat for imported/shared roots, an unshared ancestor held by a read lease, nested entries, and ArrayView or descriptor-driven representation copies; the sticky shared flag is not the only reason a container's identity may change. Test repeated no-op entries and independent Array-element entries as well as a single operation. Capture through an earlier owner must remain independent, and the later whole-value capture must retain its own preceding effects without absorbing still later sibling work. Do not satisfy topology tests by gating the topmost shared/leased container or serializing independent entries; passing final-output comparisons alone is insufficient.

Evaluate record creation/deletion in this same publication path alongside Array creation facts. Preserve relative order with independent sibling creation and apply captured order in every consumer, not only export. This obligation remains even for an unshared record whose local gate requires no protection copy; topology preservation alone does not solve it. Share the commit boundary without forcing both structures into one event history. Success requires correct final and captured graphs, ordinary mutation semantics, unchanged sibling progress, and no extra fallible storage write merely to complete protection. Reuse the result in other representation-only paths only where it removes an existing mechanism.

**Fallback:** reject candidates that require broader blocking, violate owner isolation, or fail captures during entry. Do not keep a special no-capture solution alongside an unresolved general case. If the remaining topology or structural-order mechanism needs substantial new machinery or whole-graph work merely to protect a path, report the concrete counterexample, measured costs, and alternatives before integration; do not silently weaken concurrency or topology. Keep the dependent entry replacement unintegrated until a mechanism satisfies both. The topology-safe mechanism must precede or accompany the switch from whole-Array out-of-range entry gates to element gates; independent corrections can proceed.

#### B. Growth sequence and bounded reclamation

**Question:** does the linked sequence implement the specified captured bounds and reclaim settled state with less machinery than the alternatives? Prototype it against an exhaustive small sequential oracle covering issuance/registration, predecessor waits, callback activation, creation, final publication, exact/range watchers, dominated growth, no-growth, COW captures, and synchronous re-entry. Verify arbitrary legal completion orders, not only bounds over an already assembled sequence. Include queued same-index entries, captures between and after their issuance, inherited presence, and a never-completing head followed by a long settled run with and without retained boundaries. Success requires correct early answers, terminal outcomes for completed sources, and bounded retained history; oracle verification is mandatory, not an optional optimization.

**Fallback:** compare a shared captured-growth-chain representation only if it removes watcher-barrier bookkeeping without adding subscription or scheduling machinery. Keep the smaller correct design. Additional pruning and a separate missing-slot index remain measurement-driven optimizations under sections 4 and 5, not new semantic requirements or prerequisites.

#### C. Complete traversal establishes shape

**Question:** does a successful complete logical traversal capture every unresolved growth contribution relevant to its output and finish only after each contribution's structural fact is available? Prove both coverage and publication ordering, including metadata-only placements, nested entry, COW/forked placements, absence, failed publication, and synchronous re-entry. No required structural fact may still await a notification callback when traversal completes.

If that invariant holds, finalize export and complete receiver preparation from their captured structural state without a separate shape watcher or Promise join. Keep this simplification only if it reduces total machinery. **Fallback:** a supported counterexample must identify the independent dependency and its narrow lifetime before retaining a separate wait. Discarded output still releases shape-only work without stranding required Error collection; bounded and length-only consumers retain their own earlier-readiness rules.

### 8. Verification and removal of superseded paths

Use differential integration tests: issue identical commands directly, through an entered target, through an entered enclosing owner, and through nested entry. Compare resulting data, own-key order, alias/cycle relationships, length/holes, Error kind/source/cause and recovery, native effects, imported storage, and earlier retained outputs. Compare topology within exported graphs and through supported identity-sensitive operations rather than expecting copies from separate runs to have equal JavaScript identities. Scheduling expectations compare dependency requirements and completion of otherwise completing work rather than requiring independent work to complete in identical order.
...
- Ready and delayed no-op entry into aliased records, Arrays, and cyclic graphs preserves observable topology. Cover mutable and readonly entry, nested entry, imported/shared/leased owners, captures during the entry, later independent sibling writes, and whole-root replacement. Compare actual contained mutations with direct mutations too: entry must neither add alias separation nor suppress ordinary COW required by the command. Sibling writes remain visible after entry completes; no restoration of an old ancestor may overwrite them.
- Hold a directly selectable narrow entry open while reading an unrelated ready sibling and publishing a sibling mutation through another entry. Require both to progress before release, including when an imported root is shared, an unshared ancestor is leased, or representation copying is required. Repeat no-op entries and nested entries, with a whole-value capture concurrently awaiting the protected branch. Verify both capture topology and progress; whole-Chain or whole-Array serialization must not substitute for protection of the selected placement.
- Begin export and retain the value in another Chain while a no-op entry at `x.a.k` is open in `{x: {a: child, b: child}}` and `{x: {a: child}, y: {b: child}}`. After completion, the captured outputs and current graph all preserve their required aliases; repeat with self-cycles and cycles spanning containers. An owner retained before entry stays usable while entry is open and receives none of its writes. Restoring only the current root or redirecting aliases only inside copied containers must not pass these tests. Add an independent sibling write and root replacement while entry is open: current state keeps those effects, earlier captures exclude them, and copy elimination neither replays obsolete ancestors nor changes committed Array growth.
...
- Plain, view-backed, imported, and shared Arrays; nonzero view offsets and backing with unrelated retained tail. Imported physical length/elements never change. No-op entry causes no speculative physical growth. Element publication uses ordinary supported reflection failure handling.
...
Delete replaced entry rejection/widening branches, duplicated caller-side anchor selection, and any temporary tail-storage or length-scanning implementation in the same change. Retain common mutation-scope selection, gate publication, presence handling, and external reservation machinery where still required. Do not leave adapters for the previous public entry selector.

Replace obsolete test expectations alongside each semantic change and keep the suite passing at every implementation checkpoint. In particular, replace eager entry-validation and blanket out-of-range Array-poison assertions when their replacement behavior lands; do not defer them to final cleanup or preserve incompatible expectations as compatibility tests. Retain coverage of still-current behavior until its replacement is integrated.

Extend the existing `enter.test.js`, `scope-recovery.test.js`, `supported-thenables.test.js`, publication-failure tests, and refcount verifier for destination validation and absence retirement. Reuse their public-operation fixtures; assertions must cover scope and recovery in addition to Error kind so direct and entered routes cannot agree by sharing the same delayed-publication defect.

### 9. Contract updates and implementation checkpoints

Revise the authoritative runtime documents with the implementation. The end state above replaces eager application-target failure during entry setup, blanket Array widening for out-of-range element failure, whole-Array out-of-range entry gates, and compiler-selected native entry anchors. Keep the current implementation's contracts explicit until that change lands.

Update AGENTS.md's Design Method, Language Graph, Ownership and Copy-on-Write, Representation and Materialization, Property Versions, Transition Gates, Read Leases, External Entry, Scope Poison and Repair, and capture/completion rules. State entry equivalence including liveness, own-key order, topology, original reference identity, common structural publication before completion, committed growth versus entry completion, captured length knowledge, pending and settled logical overlays, deferred readonly capture protection, and command-owned attribution. Distinguish representation changes for protection from logical mutation; preserve ordinary COW semantics for actual commands. State that protection lives in logical versions without requiring physical gate writes and still isolates earlier owners. Keep intrinsic structural waits outside graph placements, identify the logical Array/ArrayView as the length-state owner, and give operation-specific watchers explicit work ownership. Preserve the semantic distinction between valid source operations and malformed trusted integration calls; deferred target validation does not make setup unconditionally infallible.

Specifically replace the Design Method bullet's native-entry exception beginning "The requested target of native entry is a semantic operation input" and its instruction "Check this at entry target selection". Entry setup captures protection without consuming an unused suffix; actual contained commands perform ordinary validation at their own operation contexts. Keep the surrounding rule that trusts compiler/runtime control facts. Update the Array structural-poison rules to distinguish placement-local creation/growth from genuinely Array-wide mutations.

Replace the External Entry rule that strict-ancestor poison or a binding conflict suppresses callback invocation after predecessor completion. Preserve those blockers at actual contained access and leave their Error identity, repair authority, and native effect ordering unchanged. Document the non-consuming fallback for supported managed inspection failure without changing failure classification at ordinary read/write boundaries. Preserve and explicitly test the existing export own-key-order contract.

Update `data-limitations.md`, `enter.md`, `error-handling.md`, `mutation-and-observation.md`, `external-context-ordering.md`, `array-view.md`, `runtime-spec.md`, `integration.md`, and `promise-path-segments.md`. Remove public-selector documentation/exports/examples and revise future compiler-integration instructions. Preserve static mutable-resource selection, host storage protection, readonly capability, ordinary repair semantics, and operation work lifetimes.

Document logical destination preservation and the distinction between required deferred validation and ordinary pending data in the reference, mutation-lifetime, and rollback contracts. Keep command attribution and selected-scope recovery independent of Promise timing. State the storage-consistency precondition for absent-overlay retirement wherever detachment is specified; completion alone cannot make stale physical fallback authoritative.

Document structural dependency registration before predecessor waits and terminal outcomes for completed growth sources. Separate logical readiness from storage eligibility: ArrayView reuse and native receiver delivery must preserve the complete logical surface, including presence, length, and key order. Keep these conditions in their shared representation/materialization boundaries rather than duplicating rules in each method.

State that ownership or representation facts alone never widen an otherwise selectable entry; topology preservation and unrelated sibling progress are simultaneous requirements. Document candidate presence versus settled presence in the common property contract. Keep ready-only mutable-external snapshots explicit in the consumer inventory: unresolved required presence/shape fails locally without a subscription or watcher, while ready logical shape overrides physical storage. Preserve the distinction between that boundary, ordinary result import, and waiting export.

Implement and verify in bounded steps:

1. Implement independent corrections: authoritative logical-placement capture and indexed publication through pending and settled outcomes, including failed writeback and safe absence retirement; command-owned destination validation retaining scope/recovery through deferred completion; placement-local Array failure; and single-element direct assignment. Correct `hasLanguageProperty` and its candidate/presence consumers before removing physical gate writes. Include mutable-external snapshots in shared candidate capture and reject unresolved presence without subscribing. Establish committed logical-length capture/publication in the section 4 metadata, with ready-length consumers and copying using it, before or alongside the first metadata-only growth outcome. Update ArrayView reuse eligibility, native observation-receiver materialization, and ready-only snapshot shape handling before exposing states their physical/value-only checks cannot represent; failed writeback beyond physical length cannot land without these prerequisites. Carry logical destination rules through existing reference rebasing so direct and entered validation use the same mechanism. Preserve ordinary non-blocking Promise assignment, imported/view-backed storage, and bounded physical enumeration. Update affected tests and contracts with each change. These corrections do not require the topology experiment or pending growth watchers. Keep the existing whole-Array out-of-range entry gate until topology-safe element protection is ready.
2. Establish single-path reference-equivalence tests and run evaluation A through the common protection/publication path, testing cross-container aliases/cycles, earlier-owner isolation, and independent sibling progress from the first prototype. Include the identity decision for no-op versus actual mutation in captures during entry, ownership/representation variants, own-key order, supported inspection fallback, and command-owned external blockers. Reject wider gating as a simplification; if the remaining mechanism is substantial, report its measured cost before integration. Independently run evaluation B over contribution registration and source lifetimes as well as bounds/reclamation. Classify consumers by their actual information needs, including nonnegative `at` as a placement-value consumer and mutable-external snapshots as ready-only complete consumers. Use evaluation C to establish whether complete traversal already proves shape before adding separate waits; that proof does not replace native-storage eligibility or authorize a snapshot wait. Apply each evaluation's success/fallback criteria; independent corrections and growth analysis do not wait for topology, and multi-path coordination is not a prerequisite.
3. Integrate topology-safe element entry together with shared reference anchoring/publication, logical destination and original reference identity, deferred readonly protection, and command-owned validation/context. Preserve unrelated sibling concurrency through protected ancestors; do not introduce a wider ownership-based gate. Replace conflicting eager-validation branches and whole-root deletion inference without losing destination validation or pending mutation recovery. Extend the committed logical-length state from step 1 with captured pending growth and watchers, register known contributions before predecessor waits, and complete every finished source's contribution including queued no-op entries inheriting presence. Readonly entries contribute no growth. Convert dependent length/range consumers as a coherent change and remove unnecessary shape queries from nonnegative `at`. Integrate affected-placement capture for shrink/remap beyond physical or published length in this same change; an answered shape question cannot release conflicting element dependencies. Extend representation eligibility and ready-only snapshot rejection to the newly introduced shape/order states; snapshots allocate no watcher or nested subscription. Preserve early length notification, structural publication before completion, own-key order, bounded reclamation, presence/recovery transfer, and ordinary element effect ordering. Integrate pending intrinsic reads and watcher ownership through the existing continuation/lifetime mechanisms. Independently correct anchoring or consumer changes may land earlier when they neither require unfinished machinery nor regress topology or concurrency.
4. Complete the combined differential/ordering matrix and documentation/public-API pass, and remove any remaining superseded paths. Tests and contract changes already accompany the behaviors they verify; final cleanup is not a period of deferred broken expectations.

Replacing whole-Array out-of-range entry gates with element gates requires verified topology-preserving protection. Single-path reference capture uses the fixed-anchor rule above; Phase 13's mutation/observation-list coordination is not a dependency. Developing the growth oracle and consumer analysis independently does not require landing unused production mechanisms before their entry contributions exist. Do not block all phase work behind the topology experiment or split the phase solely to bypass it.

Completion requires an explicit outcome for evaluations A–C and removal of superseded production paths for retained designs. An inconclusive prototype does not satisfy a required invariant; discuss material contract tradeoffs before changing the settled semantics.
