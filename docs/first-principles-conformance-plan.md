...
| Successful ready and pending outputs obey the same native `then` surface restriction | Phase 9D-0; 9D-B assimilation verification; 9F/13 boundary integration |
...
| No required Error is lost: complete collection is part of determinism; Error order and only the enumerated reporting/detection races may vary | Existing 9C fatal semantics; 9D-A collection/query tests; 9D-B assimilation cutover; 9E/9F invalid-binding tests; Phase 13 integration |
...
| One trusted integration surface for core operations, exact external-action handling, guarded continuations, Error factories, and outward completion | Phase 9D-A surface and route tests; 9D-B transport cutover; Phase 13 consumers |
| Final Promise-backed placement terminology and one explicit prepare/install/continue/publish/commit/detach lifecycle | Phase 12 atomic terminology migration |
| Replace Cascada's legacy poison wrapper, thrown-poison interception, duplicated fatal routing, and source sorting with the kernel contracts | Phase 9D-A/9D-B kernel surface; Phase 13 Cascada integration |
...
Phase 13 consumes the documented trusted integration subpath containing the same unwrapped core operations, return helper, poison/compound factories, fatal submission, Error kinds, and precise predicates needed by Cascada. It is not another implementation, dynamic mode, or security boundary; its calls register no outward results, the higher runtime must not expose it as its host API, and no second Error factory or authorization token is added.
...
| Atomic Promise-mirror to Promise-version terminology and lifecycle cleanup | 12 |
...
- All four deliverables pass. The recoverable kernel work is implemented in 9D-0/9D-A; rejecting poison transport and higher-runtime integration remain assigned to 9D-B and 13.
...
Implement causal recoverable Error handling from [`error-handling.md`](error-handling.md). The exact causal boundary supplies a recoverable failure's source and kind; later consumers preserve it. Preserve the addendum's ordinary supported-thenable subscriptions while installing causal placement/import behavior; poison remains non-thenable until Phase 9D-B performs its rejecting-thenable cutover. Keep synchronous same-execution external-code re-entry forbidden without coupling another execution. Every internal failure continues through Phase 9C's fatal lane.
...
- Derived ArrayViews retain every fixed logical placement in the existing version map; actually pending placements receive fresh mirrors at the derivation's program position. Apply this centrally to slice/concat/pop/shift views, bounded derivatives, COW, and growth. Propagation keeps the original poison and source, without raw-Error reinterpretation or an Error cache.
- Placement replacement/removal prepares the new logical edge, completes physical storage work, then replaces/detaches the old version and commits refcount facts. Under the [managed-storage Proxy contract](data-limitations.md#proxies-in-managed-storage), each primitive write, definition, or deletion implements the requested property/Array operation on success and leaves the represented graph unchanged on failure. The old fixed value or pending mirror therefore remains available through every surviving alias after supported failure. Use these common primitives for ordinary assignment/deletion, Array replay, and enclosing publication; add no Proxy detection, speculative writes, per-caller rollback, or recovery guarantee for violating traps. This restriction applies to individual storage operations, not whole managed methods or opaque external mutations. Runtime-internal Array remapping Proxies remain available.
...
- A pending mutation's completion observes its captured gate's committed logical value. Register that continuation at issuance, immediately after shared publication and before later consumers: registering after receiver readiness would let later mutations advance the mirror before the earlier operation captures its result. Gate fulfillment alone is not publication success. Preserve the already collected result through ready ancestor writeback and pending ancestor mirror advancement, without adding another result representation or scheduling queue.
...
- Trusted-integration tests exercise a standalone host call and iterator advancement with ready, synchronously delivered custom, and pending outcomes. External code attempting to re-enter the same execution fails it before graph work; even if it catches that fatal and returns a normal value, the post-external-action checkpoint propagates the original authoritative fatal. The same external action may synchronously enter another execution without changing either one's state. Adjacent trusted continuation failures stay fatal. Each outward facade registers at most one pending result, while its internal integration calls register none.
...
- Publication tests combine a rejected placement value with descriptor or writeback failure, preserve every Error discovered across retries, and deduplicate an existing poison thrown again. Cover indexed and unindexed owners and synchronous custom settlement, then query the retained mirror and run the refcount oracle. Array removed-result tests cross ready/pending `pop` and `shift` results with receiver replay failure: the receiver publishes its own failure immediately and the independent result combines both without extending the gate.
...
Carry the complete-collection, placement-version, and publication regressions through this cutover, including `array-complete-collection.test.js`, `placement-version-boundaries.test.js`, and `publication-failures.test.js`. Change final poison outcomes to rejection assertions while preserving every required sibling wait and cause/source/kind membership. Audit Array placement capture, fixed-version retention and receiver materialization, sorting conversion/export, flat recursion, property-publication retries, and target/gate/ancestor mutation completion: an intermediate poison rejection must neither bypass later preparation nor become fatal. Verify that an already completed receiver publication remains visible while an independent result waits, on success and failure, that failed storage work preserves surviving aliases, and that later operations cannot change an earlier gate outcome. Use existing collectors and mutation outcomes rather than adding transport adapters.
...
Port the minimal useful part of Cascada's sync-first `then` behavior directly onto `PoisonError`; `CompoundPoisonError` inherits it. Without a rejection callback it returns `this`; otherwise it directly returns that callback's result and lets a callback throw propagate normally. Native assimilation supplies a non-throwing rejection function, and kernel code recognizes Error before thenability, so the callback catch and special thrown-poison branch add no required behavior. Leave the shared prototypes ordinary; deliberate mutation is outside the programming API contract and does not justify freezing or integrity machinery. Omit per-instance method copies, integrity polling, wrapper conversion, `catch`, `finally`, Promise-compatible chaining, and `RuntimePromise` machinery.

Delete the remaining transitional machinery in the same change:

- any kernel-local equivalent of `PoisonedValue`, `RuntimePromise`, or `PoisonErrorGroup`; the actual legacy Cascada definitions are removed with their consumers in Phase 13, and are not ported into this package;
...
- Both poison types are native Errors, sync-first rejecting thenables, and reject with their exact identity through `await` and assimilation. `FatalError` is a non-thenable native Error under the stable Error prototype contract. Factory-created instances and compound child arrays are frozen, but shared Error prototypes are ordinary and deliberate modification is unsupported. No shared runtime base or legacy wrapper remains.
...
- Error combination rejects zero inputs, expands direct compounds one level, and shares `flattenAndDeduplicateErrors` cause/context/kind deduplication with `getErrors`. A sole distinct leaf returns unchanged without constructing a compound. Multiple leaves produce an immutable leaf-only child array with unspecified order and derive `.kind` correctly without a stored `.kinds` projection. Verify composition through the trusted factory rather than direct constructor calls.
...
- The `./integration` external-action, guarded-continuation, and outward-return routes established in 9D-A pass with rejecting poison; no higher-runtime facade normalization, private deep import, second re-entry guard, or extra outward registration is introduced.
...
- Existing predecessor poison contributes an Error at the receiver position. Required preparation finishes, external code is skipped, and the phase preserves it.
...
- Exercise several selected leaves and peer observations where one failure is ready and another is revealed later. Complete all required predecessor and read-group work; retain each contributing poison for the exclusive successor without accepting poison from a peer as an observation input. An unselected provisional leaf relays its predecessor state without contributing it to this operation. A ready failure cannot truncate the selected phase set or release it early. Verify exact source/kind membership and vary completion order.
...
- Add `repairPath(chain, path, operationContext)` for repair-only. It targets an existing valid registered external location, creates no registration or authority, invokes no external code, and repairs no managed graph Error. Add it to Phase 9C's public facade and package-surface classification in the same change, so a pending repair result uses the common outward fatal-rejection registration while ready repair remains direct.
...
- Cross argument-export, host-call, result-admission/snapshot, and post-call containment failures at the same operation. Retain every independent failure the operation is required to collect, while applying the documented managed-placement, external-phase, and operation-result effects separately. A safely rejected pre-call selection preserves state when required; an independent result failure does not erase a completed valid mutation, and phase publication cannot overwrite an already discovered operation Error. Verify ready, synchronous-custom, and pending routes, unchanged Error attribution, and no host invocation after required preparation fails.
...
## Phase 12: Rename Promise mirrors to Promise versions

### Problem

`Promise mirror` describes one implementation effect but obscures the more useful invariant: the object is the exact logical property version created for one placement and program position when consuming a supported thenable remains pending. The current helper names also mix lifecycle stages and call shapes: `continuePromiseVersion` operates on an already captured mirror, `continuePropertyValue` is the ordinary placement-facing continuation, `placePromiseVersion` actually forks a version, and publication/commit helpers are named after a value rather than the version they advance. Renaming these pieces incrementally would temporarily make the model less coherent and would bury a repository-wide terminology migration inside unrelated semantic work.

### Outcome

Rename the concept atomically across source, tests, architecture, and implementation documentation. A **Promise version** is a placement version created only when the first ordered consumption of a supported thenable returns pending. It stores the logical value observed at that program position as ordered settlement work advances it. It is not the source Promise and `Promise version` is not a synonym for `pending`: the version remains the same object after source settlement and may remain captured after detachment from its placement. A synchronously delivered thenable creates no Promise version; it publishes directly or uses a fixed placement version when physical storage must remain unchanged.

This phase changes terminology and helper boundaries, not Promise, placement, ordering, import, ownership, or failure semantics. Phase 13 consumes only the final vocabulary.

### Promise-version lifecycle

1. **Prepare — `preparePropertyVersion`.** Create callback-visible staging before subscribing. Consume the candidate through the ordinary sync-first thenable path. Direct completion produces an ordinary ready value or fixed placement version; only a returned pending continuation proceeds into the Promise-version lifecycle.
2. **Install — `installPlacementVersion`.** Mark the staged record as Promise-backed and attach that exact version to the placement before later delivery can run. Rename the discriminator from `promise` to `promiseBacked`; it identifies the version's representation and does not claim that its current logical value is still pending.
3. **Continue — `continuePromiseVersion`.** The ordinary placement-facing helper looks up the required Promise version and registers the continuation at the caller's exact FIFO position. A module-private `continueCapturedPromiseVersion` handles work that already owns an exact captured version. Both read `promiseVersion.value` when their callback runs rather than treating the source callback payload as the placement's authoritative logical value.
4. **Publish — `publishPromiseVersion`.** Validate and admit the newly available language value, apply retention when required, and prepare publication through the common property path. Publication rejects a still-pending replacement because that requires a fresh Promise version.
5. **Commit — `commitPromiseVersion`.** Atomically update the version's logical value together with any live physical writeback and refcount edge. If the version has been detached, update only the captured version; never republish it into a placement that has moved on.
6. **Fork — `forkPromiseVersion`.** Copying or retaining a pending placement creates a fresh destination Promise version at the copier's program position and advances it from the captured source version. The destination does not share the source version object.
7. **Detach — `detachPlacementVersion`.** Replacement or deletion removes the placement's association with the version. Existing continuations may still advance their captured version, but can no longer change that placement.

Fixed placement versions share the general placement-version install/read/detach machinery but do not enter the Promise-specific continue, publish, commit, or fork lifecycle.

### Atomic rename and cleanup

- Rename `getPromiseMirror`, `requirePromiseMirror`, and `isLivePromiseMirror` to `getPromiseVersion`, `requirePromiseVersion`, and `isLivePromiseVersion`.
- Rename the current captured-version `continuePromiseVersion` to module-private `continueCapturedPromiseVersion`; rename the placement-facing `continuePropertyValue` to exported `continuePromiseVersion`. Convert callers such as mutating `enter` to the placement-facing form where they already have the owner and key.
- Rename `placePromiseVersion` to `forkPromiseVersion`, `publishPromiseValue` to `publishPromiseVersion`, and `commitPromiseValue` to `commitPromiseVersion`.
- Delete the one-line `advancePromiseVersion` alias and call `publishPromiseVersion` directly.
- Rename `mirror`, `sourceMirror`, and corresponding fields to `promiseVersion`, `sourceVersion`, and `promiseVersion`. Rename the version discriminator from `promise` to `promiseBacked`.
- Replace `Promise mirror` terminology throughout code, tests, fixtures, diagnostics, and documentation, including prose about gates, import, COW, ArrayViews, refcounts, paths, and export. Use `placement version` where the statement also covers fixed versions; use `Promise version` only where pending-origin behavior matters.
- Update Phase 13's Cascada migration inventory to use the final names. Leave no compatibility export, delegating alias, temporary adapter, or mixed old/new terminology.

Keep the generic `getPlacementVersion`, `preparePropertyVersion`, `installPlacementVersion`, `installFixedPlacementVersion`, and `detachPlacementVersion` names. Do not invent `continueNonPromiseVersion`, a lifecycle object, a class hierarchy, or persistent version records for ready runtime-owned properties. The lifecycle is the existing state transition expressed coherently, not another mechanism.

### Verification

- Ready native values and synchronously delivered custom thenables create no Promise version and preserve synchronous return behavior.
- Pending native and custom thenables install one Promise version per placement and program position; repeated continuations retain FIFO ordering and observe the version's latest logical value.
- Publication, callable-`then` validation, refcount commit, physical writeback, imported-storage no-writeback, detachment, COW forks, Array copies, gates, path resumption, and mutating `enter` preserve their pre-rename behavior.
- A fork receives a distinct version, and a detached version may finish its captured work without changing the replacement placement.
- Missing required-version and fatal-on-publication paths still fail at the same boundary and with the same operation context.
- Focused property-version, import, path, entry, Array, refcount, custom-thenable, and Error-publication tests pass, followed by the complete suite and refcount oracle.
- A repository-wide audit finds no remaining `mirror`, `Promise mirror`, old helper name, or old discriminator in implementation or final architecture documentation. This phase's migration record and an explicitly quoted historical external name may retain the removed term where necessary. Do not add a permanent source analyzer for this check.

Update [`AGENTS.md`](../AGENTS.md), [`data-classes.md`](data-classes.md), [`counters-implementation.md`](counters-implementation.md), [`enter.md`](enter.md), [`cycles-as-data.md`](cycles-as-data.md), [`array-view.md`](array-view.md), [`import-preparation.md`](import-preparation.md), [`error-handling.md`](error-handling.md), [`managed-invocation.md`](managed-invocation.md), [`managed-and-external-state.md`](managed-and-external-state.md), [`outbound-export.md`](outbound-export.md), [`run.md`](run.md), [`promise-path-segments.md`](promise-path-segments.md), [`runtime-spec.md`](runtime-spec.md), code comments, tests, and fixtures.
...
- Keep the execution-scoped external-action prohibition during that finalizer. Its deliberate post-fatal semantics differ from ordinary host-result processing: capture and own the close result/rejection without replacing, re-reporting, or waiting to deliver the authoritative fatal. Implement this one local boundary explicitly, not as an `allowAfterFatal` flag on ordinary graph/host operations. If the normal envelope cannot retain a close Promise before propagating the authoritative fatal, factor out its exact synchronous active-action bracketing for reuse by this adapter and expose that narrow primitive through `./integration`; keep the post-external-action fatal classifier on ordinary calls and verify that no other caller uses the finalization exception. The adapter marks only its execution and restores the Boolean in `finally`.
