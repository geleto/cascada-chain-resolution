# Subtree counters, `normalize`, `hasError`, `getErrors` — step-by-step implementation
...
counter plan. `import(value, errorContext)` recursively marks the external graph and
registers its promise tree, while validation remains lazy at counting time as
`validateCountable` — see `docs/lazy-import.md`. normalize/hasError/getErrors details
are aligned with issues.md items 6, 7, and 10: there is no ambient settling record;
...
surface is normalize/hasError/getErrors and their shared observational resolver:
...
| `import` | — | recursively mark imported/shared nodes and register promises; no validation, mirrors, counters, or writeback |
| lookup/normalize/hasError/getErrors resolver | — | observational, context-threading path walk |
| exports | — | `+ normalize, hasError, getErrors` |
...
ref-indexed parents. Import marks recursively but does not validate, and non-ref-indexed writeback can float until
...
//
// import(value, context) — index.js/meta.js. Walk the language-visible graph,
//   mark every tracked node imported/shared, and register every unique promise.
//   Settled branches re-enter the same walk. No validation, mirror minting,
//   counter work, or property writeback.
//
...
lookup, normalize, hasError, and getErrors share a lookup-shaped path walk that observes only:
...
The observational walker is `walkObservationPath(chain, path, onResolved)`. The final path segment is the
optional target, while every preceding segment is required; an empty path targets the
root. Promises continue through mirrors and existing Errors stop the walk. A
missing/null/undefined or primitive required value becomes a language Error; only the
target may be absent. `__proto__` and own non-enumerable keys follow the same rule:
missing at the target, Error if another segment attempts to dereference them. The COW
mutation walker receives the same complete target path but remains separate because
it installs copied branches while traversing.
...
```js
function normalize(chain, segments = [], sharedOwnership = true, plainCopy = false) {
    return walkObservationPath(chain, segments, (value, importContext) => {
...
for the value reached at the path. `hasErrorInIndexedBranch` answers directly from
`errorCount` and `promiseCount` when possible; only a pending branch allocates the final
boolean race and starts collecting promise waits.

`walkObservationPath(chain, path, onResolved)` treats the last path segment as the
optional target. A missing target reaches hasError as `undefined` and answers false;
a broken required prefix reaches it as an existing or generated Error and answers
true. Otherwise the reached primitive, promise, or tracked branch is probed normally.
Import-context threading, property reads, and promise handling all remain in the resolver.
...
`hasErrorInIndexedBranch` returns directly when `errorCount > 0` or
`promiseCount === 0`, without allocating a promise. Otherwise the pending frontier is
collected by descending only ref-indexed nodes whose counters still contain promises.
Waits are registered after `buildRefIndex` has minted mirrors/writebacks. The shared
collector returns `Promise.all(waitPromises)`, where each wait is
`onValueResolve(childPromise, () => probeResolvedPromiseForErrors(mirror.currentValue))`;
behind a promise boundary, `probeIndexedBranchForErrors` checks counters and returns
any nested readiness tree.
...
One operation-local `visited` WeakSet survives the recursive promise tree. Only nodes with
their own shared status are checked and added: valid reused identities are marked by
the ownership, import, or COW boundaries that created the reuse. An unmarked node is
owned and cannot recur in valid data; reaching it again through an ancestor would be a
cycle already rejected by ref-indexing or promise-writeback validation. Weak visitation
does not retain settled branches while another captured promise remains pending.

### 4e. getErrors

getErrors resolves the full path observationally, including generated path-access
Errors for broken required prefixes, builds the generic ref index only at the reached
path-value boundary, and accumulates distinct Error identities in one Set.
It shares `collectErrorSearchWaits` and the operation-local marked-node `visited`
WeakSet with hasError: both follow counter-pruned promise regions, while supplying
the Set additionally walks counted Error regions and records Error-valued keys. Valid
non-extensible children are skipped before counter access.

Each promise wait consumes the already-prepared `mirror.currentValue`, and its callback
returns any nested readiness promise. The shared helper therefore returns `undefined`
for an entirely synchronous frontier or a recursive `Promise.all` for pending work.
getErrors has no early race: it materializes `[...errors]` only after that complete wait
tree settles. It never marks or creates a normalize settlement wait.
...
Settlement / normalize / error queries:
...
- `hasError`: empty path probes root; validation failure → true; error at target or mid-path → true; broken path (intermediate
  ...
  writes.
- `getErrors`: any reached Error-free terminal (including missing, primitive, or
  valid frozen) → `[]`; Error-blocked path returns that
  Error; distinct identities are collected through DAGs and recursively exposed
  promises; later installs are ignored, while captured revoked mirrors still count.
