...
- A mutation installs the ordinary transition gate at the reached managed prefix placement and continues against its private working value. Later operations through that prefix wait; unrelated paths continue. Later mutable external work passes preceding managed gates before reserving in its original entry view. Record its one effect completion at issuance and reuse it at reservation so an enclosing entry cannot finish early. No uncertain external candidates are reserved.
...
Once a static path selects one mutable external boundary, reserve its ordinary observation or mutation phase before waiting for any dynamic native-suffix input key. Pass preceding managed gates and required managed selection first, retaining the issuing entry view; a selected native suffix waits only after this reservation. Keep that selected phase through required boundary completion. A dynamic suffix cannot select a registered descendant, and no deferred key grants static authority.
...
A dynamic path may select a managed parent containing registered descendants without granting authority to those descendants. Entry still protects its covered state. Keep its parent-prefix gate through target selection and reservation setup, so later canonical native access cannot reserve ahead of the unresolved entry. After selection, publish the normal narrow entry gate and release unrelated managed siblings through ordinary path publication. Test the pending-key mixed-entry cycle described in [external ordering](external-context-ordering.md#routing-and-capture-points); add no provisional reservations or late static-authority promotion.
